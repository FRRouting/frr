...
static bool ospf_opaque_lsa_hooks_registered = false;
...
static int ospf_opaque_lsa_update_hook(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_OPAQUE_LSA)
		zlog_debug("%s: LSA [Type %d: %pI4] %sLSA Update Hook ", __func__, lsa->data->type,
			   &lsa->data->id, IS_LSA_MAXAGE(lsa) ? "MaxAged " : "");
	/*
...
static int ospf_opaque_lsa_delete_hook(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_OPAQUE_LSA)
		zlog_debug("%s: LSA [Type %d: %pI4] %sLSA Delete Hook ", __func__, lsa->data->type,
			   &lsa->data->id, IS_LSA_MAXAGE(lsa) ? "MaxAged " : "");
	/*
