...
static int ospf_route_match_delete(struct vty *vty, const char *command,
				   const char *arg)
{
	...
	if (ret) {
		...
		case RMAP_COMPILE_ERROR:
			vty_out(vty, "%% OSPF Argument is malformed.%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
...
static int ospf_route_match_add(struct vty *vty, const char *command,
				const char *arg)
{
	VTY_DECLVAR_CONTEXT(route_map_index, index);
	int ret;
