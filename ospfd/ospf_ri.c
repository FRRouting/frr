	...
	if (ntohs(top->address.type) == PCE_ADDRESS_TYPE_IPV4) {
		if (vty != NULL)
			vty_out(vty, "  PCE Address: %s%s",
				inet_ntoa(top->address.value), VTY_NEWLINE);
		else
					      ...
					      struct ri_tlv_header *tlvh)
{
	...
	if (ntohs(top->type) == PCE_DOMAIN_TYPE_AREA) {
		tmp.s_addr = top->value;
		if (vty != NULL)
			vty_out(vty, "  PCE neighbor Area: %s%s",
				inet_ntoa(tmp), VTY_NEWLINE);
		else
					      ...
					      struct ri_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  PCE Capabilities Flag: 0x%x%s",
			ntohl(top->value), VTY_NEWLINE);
	else
...
static void ospf_router_info_config_write_router(struct vty *vty)
{
	...
	if (OspfRI.status == enabled) {
		if (OspfRI.scope == OSPF_OPAQUE_AS_LSA)
			...
			vty_out(vty, " router-info area %s%s",
				inet_ntoa(OspfRI.area_id), VTY_NEWLINE);
		...
		if (pce->pce_cap_flag.header.type != 0)
			vty_out(vty, "  pce flag 0x%x%s",
				ntohl(pce->pce_cap_flag.value), VTY_NEWLINE);

		for (ALL_LIST_ELEMENTS_RO(pce->pce_domain, node, domain)) {
			if (domain->header.type != 0) {
				if (domain->type == PCE_DOMAIN_TYPE_AREA) {
					tmp.s_addr = domain->value;
					vty_out(vty, "  pce domain area %s%s",
						inet_ntoa(tmp), VTY_NEWLINE);
				} else {
					vty_out(vty, "  pce domain as %d%s",
						ntohl(domain->value),
						VTY_NEWLINE);
				}
			}
       ...
       "PCE address in IPv4 address format\n")
{
	...
	if (!inet_aton(argv[idx_ipv4]->arg, &value)) {
		vty_out(vty, "Please specify PCE Address by A.B.C.D%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "32-bit Hexadecimal value\n")
{
	...
	if (sscanf(argv[idx_bitpattern]->arg, "0x%x", &scope) != 1) {
		vty_out(vty, "pce_path_scope: fscanf: %s%s",
       ...
       "AS number in decimal <0-65535>\n")
{
	...
	if (sscanf(argv[idx_number]->arg, "%d", &as) != 1) {
		vty_out(vty, "no_pce_domain: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "AS number in decimal <0-65535>\n")
{
	...
	if (sscanf(argv[idx_number]->arg, "%d", &as) != 1) {
		vty_out(vty, "no_pce_neighbor: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Router Information\n")
{
	...
	if (OspfRI.status == enabled) {
		vty_out(vty, "--- Router Information parameters ---%s",
			VTY_NEWLINE);
