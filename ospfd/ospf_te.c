				      ...
				      struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Link: %u octets of data%s",
			ntohs(top->header.length), VTY_NEWLINE);
	else
					      ...
					      struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Link-ID: %s%s", inet_ntoa(top->value),
			VTY_NEWLINE);
	else
						   ...
						   struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Local Interface IP Address(es): %d%s", n,
			VTY_NEWLINE);
	else
	...
	for (i = 0; i < n; i++) {
		if (vty != NULL)
			vty_out(vty, "    #%d: %s%s", i,
				inet_ntoa(top->value[i]), VTY_NEWLINE);
		else
			zlog_debug("      #%d: %s", i,
						   ...
						   struct te_tlv_header *tlvh)
{
	...
	n = ntohs(tlvh->length) / sizeof(top->value[0]);
	if (vty != NULL)
		vty_out(vty, "  Remote Interface IP Address(es): %d%s", n,
			VTY_NEWLINE);
	else
						 ...
						 struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Maximum Reservable Bandwidth: %g (Bytes/sec)%s",
			fval, VTY_NEWLINE);
	else
					    ...
					    struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL) {
		vty_out(vty, "  Local  TE Router ID: %s%s",
			inet_ntoa(top->local), VTY_NEWLINE);
		vty_out(vty, "  Remote TE Router ID: %s%s",
			inet_ntoa(top->remote), VTY_NEWLINE);
	} else {
					  ...
					  struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Inter-AS TE Remote AS number: %u%s",
			ntohl(top->value), VTY_NEWLINE);
	else
						...
						struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  Delay Variation: %d (micro-sec)%s", jitter,
			VTY_NEWLINE);
	else
					       ...
					       struct te_tlv_header *tlvh)
{
	...
	if (vty != NULL)
		vty_out(vty, "  %s Link Loss: %g (%%)%s",
			anomalous ? "Anomalous" : "Normal", fval, VTY_NEWLINE);
	else
