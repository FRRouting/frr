...
static void path_zebra_add_srv6_policy_internal(struct srte_policy *policy)
{
	...
	if (zclient_send_rnh(zclient, cmd, &lookup.nh, SAFI_UNICAST, false, false, VRF_DEFAULT) ==
...
void path_nht_removed(struct srte_candidate *candidate)
{
	...
	if (zclient_send_rnh(zclient, ZEBRA_NEXTHOP_UNREGISTER, &lookup.nh, SAFI_UNICAST, false,
