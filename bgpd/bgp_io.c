 ...
 * messages whenever that value is left at the standard size on this connection
 * (e.g. when our OPEN had not yet been built when the peer's OPEN was parsed,
 * a state then preserved across collision resolution), dropping every >4096
 * message with NOTIFICATION Message Header Error / Bad Message Length.
 */
