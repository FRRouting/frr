...
static void evpn_delete_vni(struct bgp *bgp, struct bgpevpn *vpn)
{
	...
	if (!is_vni_live(vpn)) {
		/*
		 * Config-only VNI: no RD/RT unconfigure re-import will run below,
		 * so withdraw the remote End.DT2U/End.DT2M underlay /128 routes
		 * here (import RTs still present -> routes still match this VNI)
	...
	/*
	 * SRv6 L2 EVPN remote underlay cleanup, done LAST: the import-RT
	 * unconfigure above runs uninstall -> regenerate -> reinstall, which
	 * re-imports the remote Type-2/Type-3 routes and re-installs their
	 * plain IPv6 /128 underlay routes.  Withdraw them here, after that
	 * reinstall, so the /128s toward the remote End.DT2U/End.DT2M SIDs are
	 * actually removed from the kernel on "no evi N" instead of being left
	 * as dangling nexthops.  Idempotent; the netdevs are untouched.
	 */
	bgp_evpn_srv6_uninstall_remote_underlay(bgp, vpn);
}
