...
DEFUN (bgp_evpn_advertise_type5,
       ...
       "advertise " BGP_AFI_CMD_STR "" BGP_SAFI_CMD_STR " [enable] [gateway-ip] [route-map RMAP_NAME]",
...
#if CONFDATE > 20260227
	if (argv_find(argv, argc, "enable", &idx))
		enabled = true;
	CPP_NOTICE("Remove 'advertise ipv4-unicast <cr>' and 'advertise ipv6-unicast <cr>' command")
#else
	enabled = true;
#endif
...
DEFUN (no_bgp_evpn_advertise_type5,
       ...
       "no advertise " BGP_AFI_CMD_STR "" BGP_SAFI_CMD_STR " [enable] [gateway-ip] [route-map [WORD]]",
       NO_STR
       "Configure prefix routes advertisements\n"
       BGP_AFI_HELP_STR
       BGP_SAFI_HELP_STR
       ...
       "Name of the route map\n")
{
	...
	if (argv_find(argv, argc, "enable", &idx))
		disabled = true;
	CPP_NOTICE(
		"Remove 'no advertise ipv4-unicast <cr>' and 'no advertise ipv6-unicast <cr>' command")
#else
	disabled = true;
#endif
				...
				safi_t safi)
{
	...
	if (CHECK_FLAG(bgp->af_flags[AFI_L2VPN][SAFI_EVPN], BGP_L2VPN_EVPN_ADV_IPV4_UNICAST_ENABLE))
		vty_out(vty, "  advertise ipv4 unicast enable\n");
	...
	if (CHECK_FLAG(bgp->af_flags[AFI_L2VPN][SAFI_EVPN], BGP_L2VPN_EVPN_ADV_IPV6_UNICAST_ENABLE))
		vty_out(vty, "  advertise ipv6 unicast enable\n");
