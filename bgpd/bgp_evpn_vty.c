...
static void evpn_show_route_rd_prefix(struct vty *vty, struct bgp *bgp, struct prefix_rd *prd,
				      const struct prefix *ip_prefix, json_object *json)
{
	struct prefix_evpn p;
	struct bgp_dest *rn;
	struct bgp_path_info *pi;
...
DEFPY(show_bgp_l2vpn_evpn_route_rd_prefix,
      ...
      "show bgp l2vpn evpn route rd ASN:NN_OR_IP-ADDRESS:NN$rd_str prefix <A.B.C.D/M|X:X::X:X/M>$prefix [json$uj]",
      SHOW_STR
      BGP_STR
      L2VPN_HELP_STR
      EVPN_HELP_STR
      ...
      JSON_STR)
{
	...
	evpn_show_route_rd_prefix(vty, bgp, &prd, prefix, json);
...
ALIAS_HIDDEN(
	...
	"show bgp evpn route rd ASN:NN_OR_IP-ADDRESS:NN$rd_str prefix <A.B.C.D/M|X:X::X:X/M>$prefix [json$uj]",
	SHOW_STR BGP_STR EVPN_HELP_STR
	"EVPN route information\n"
	"Route Distinguisher\n"
	"ASN:XX or A.B.C.D:XX\n"
