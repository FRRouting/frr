 ...
 * BGP_DEST_AUTOUNLOCK
 *
 * Declaration attribute for a dest reference that is released on every exit
 * from the enclosing scope, so that early returns and gotos cannot leak it:
 *
 ...
 */
#define BGP_DEST_AUTOUNLOCK __attribute__((cleanup(_bgp_dest_auto_unlock)))
