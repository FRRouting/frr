...
static inline struct list *bgp_evpn_evi_get_vrf_export_rtl(struct bgp_evpn_evi *vpn)
{
	if (!vpn->bgp_vrf)
		return NULL;
...
static inline struct list *bgp_evpn_evi_get_vrf_import_rtl(struct bgp_evpn_evi *vpn)
{
	if (!vpn->bgp_vrf)
		return NULL;
...
extern void bgp_evpn_handle_rd_change(struct bgp *bgp, struct bgp_evpn_evi *vpn,
				      int withdraw);
void bgp_evpn_handle_global_macvrf_soo_change(struct bgp *bgp,
					      struct ecommunity *new_soo);
extern int bgp_evpn_install_routes(struct bgp *bgp, struct bgp_evpn_evi *vpn);
extern int bgp_evpn_uninstall_routes(struct bgp *bgp, struct bgp_evpn_evi *vpn);
...
extern void bgp_evpn_derive_auto_rt_export(struct bgp *bgp,
					   struct bgp_evpn_evi *vpn);
extern void bgp_evpn_derive_auto_rd(struct bgp *bgp, struct bgp_evpn_evi *vpn);
