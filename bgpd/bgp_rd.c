...
uint16_t decode_rd_type(unsigned char *pnt)
{
	uint16_t v;
...
void encode_rd_type(uint16_t v, unsigned char *pnt)
{
	*((uint16_t *)pnt) = htons(v);
}

/* type == RD_TYPE_AS */
void decode_rd_as(unsigned char *pnt, struct rd_as *rd_as)
{
	rd_as->as = (uint16_t)*pnt++ << 8;
	rd_as->as |= (uint16_t)*pnt++;

	rd_as->val = ((uint32_t)*pnt++ << 24);
	rd_as->val |= ((uint32_t)*pnt++ << 16);
	rd_as->val |= ((uint32_t)*pnt++ << 8);
	rd_as->val |= (uint32_t)*pnt;
}

/* type == RD_TYPE_AS4 */
void decode_rd_as4(unsigned char *pnt, struct rd_as *rd_as)
{
	rd_as->as = (uint32_t)*pnt++ << 24;
	rd_as->as |= (uint32_t)*pnt++ << 16;
	rd_as->as |= (uint32_t)*pnt++ << 8;
	rd_as->as |= (uint32_t)*pnt++;
...
/* type == RD_TYPE_IP */
void decode_rd_ip(unsigned char *pnt, struct rd_ip *rd_ip)
{
...
void decode_rd_vnc_eth(unsigned char *pnt, struct rd_vnc_eth *rd_vnc_eth)
{
