...
static uint32_t srv6_l3vpn_hash_key_make(const void *p)
{
	...
	key = jhash_3words(l3vpn->sid_flags, l3vpn->endpoint_behavior,
			   l3vpn->loc_block_len, key);
	key = jhash_3words(l3vpn->loc_node_len, l3vpn->func_len, l3vpn->arg_len,
			   key);
	key = jhash_2words(l3vpn->transposition_len,
			   l3vpn->transposition_offset, key);
	return key;
}
