...
static bool overlay_index_same(const struct attr_extra *ae1,
			       const struct attr_extra *ae2)
{
	if (!ae1 && ae2)
		return false;
	if (!ae2 && ae1)
	...
	if (!ae1 && !ae2)
		return true;
	return !memcmp(&(ae1->evpn_overlay), &(ae2->evpn_overlay),
...
void bgp_attr_deep_dup(struct attr *new, struct attr *orig)
{
	...
	if (orig->extra) {
		...
		if (orig->extra->encap_subtlvs)
			new->extra->encap_subtlvs =
				encap_tlv_dup(orig->extra->encap_subtlvs);
#if ENABLE_BGP_VNC
		if (orig->extra->vnc_subtlvs)
			new->extra->vnc_subtlvs =
				encap_tlv_dup(orig->extra->vnc_subtlvs);
#endif
	}
}
...
#define MIX(val)	key = jhash_1word(val, key)
	...
	if (extra) {
		...
		key = jhash(extra->mp_nexthop_global.s6_addr, IPV6_MAX_BYTELEN,
			    key);
		key = jhash(extra->mp_nexthop_local.s6_addr, IPV6_MAX_BYTELEN,
			    key);
	}
...
int attrhash_cmp(const void *p1, const void *p2)
{
	...
	if (attr1->flag == attr2->flag && attr1->origin == attr2->origin
		...
		if (ae1 && ae2 && ae1->aggregator_as == ae2->aggregator_as
		    && ae1->aggregator_addr.s_addr
			       == ae2->aggregator_addr.s_addr
		    && ae1->weight == ae2->weight && ae1->tag == ae2->tag
	...
	if (attr->extra) {
		...
		if (attre->lcommunity) {
			if (!attre->lcommunity->refcnt)
				attre->lcommunity =
					lcommunity_intern(attre->lcommunity);
			else
		...
		if (attre->encap_subtlvs) {
			if (!attre->encap_subtlvs->refcnt)
				attre->encap_subtlvs =
					encap_intern(attre->encap_subtlvs,
						     ENCAP_SUBTLV_TYPE);
			else
		...
		if (attre->vnc_subtlvs) {
			if (!attre->vnc_subtlvs->refcnt)
				attre->vnc_subtlvs = encap_intern(
					attre->vnc_subtlvs, VNC_SUBTLV_TYPE);
			else
...
void bgp_attr_unintern_sub(struct attr *attr)
{
	...
	if (attr->extra) {
		...
		if (attr->extra->encap_subtlvs)
			encap_unintern(&attr->extra->encap_subtlvs,
				       ENCAP_SUBTLV_TYPE);

#if ENABLE_BGP_VNC
		if (attr->extra->vnc_subtlvs)
			encap_unintern(&attr->extra->vnc_subtlvs,
				       VNC_SUBTLV_TYPE);
#endif
	}
}
	...
	case BGP_ATTR_NHLEN_IPV4:
		...
		if (attr->nexthop.s_addr == 0)
			memcpy(&attr->nexthop.s_addr,
			       &attre->mp_nexthop_global_in, IPV4_MAX_BYTELEN);
		break;
			  ...
			  u_char *startp)
{
	...
	while (length >= 4) {
		...
		if (!attre) {
			...
			if (BGP_ATTR_ENCAP == type) {
				for (stlv_last = attre->encap_subtlvs;
				...
				for (stlv_last = attre->vnc_subtlvs;
				     stlv_last && stlv_last->next;
				     stlv_last = stlv_last->next)
					;
				if (stlv_last) {
				    ...
				    struct bgp_nlri *mp_withdraw)
{
	...
	if (attr->extra) {
		...
		if (attr->extra->transit)
			attr->extra->transit =
				transit_intern(attr->extra->transit);
		if (attr->extra->encap_subtlvs)
			attr->extra->encap_subtlvs = encap_intern(
				attr->extra->encap_subtlvs, ENCAP_SUBTLV_TYPE);
#if ENABLE_BGP_VNC
		if (attr->extra->vnc_subtlvs)
			attr->extra->vnc_subtlvs = encap_intern(
				attr->extra->vnc_subtlvs, VNC_SUBTLV_TYPE);
#endif
	}
...
size_t bgp_packet_mpattr_start(struct stream *s, afi_t afi, safi_t safi,
			       afi_t nh_afi,
			       ...
			       struct attr *attr)
{
	...
	if (afi == AFI_L2VPN)
		...
		nh_afi =
			BGP_NEXTHOP_AFI_FROM_NHLEN(attr->extra->mp_nexthop_len);
	/* Nexthop */
	...
	case AFI_IP6:
		...
		case SAFI_MULTICAST: {
			...
			stream_put(s, &attre->mp_nexthop_global,
				   IPV6_MAX_BYTELEN);
			if (attre->mp_nexthop_len
		...
		case SAFI_ENCAP:
			...
			stream_put(s, &attr->extra->mp_nexthop_global,
				   IPV6_MAX_BYTELEN);
			break;
		default:
			break;
	...
	case AFI_L2VPN:
		...
		case SAFI_EVPN:
			if (attr->extra->mp_nexthop_len
				...
				stream_putl(s, 0); /* RD = 0, per RFC */
				stream_putl(s, 0);
				stream_put(s, &attr->extra->mp_nexthop_local,
					   IPV6_MAX_BYTELEN);
			}
			break;
			break;
				...
				u_int32_t addpath_tx_id)
{
	...
	if (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_AGGREGATOR)) {
		...
		if (use32bit) {
			...
			if (attr->extra->aggregator_as > 65535) {
				...
				stream_putw(
					s,
					(u_int16_t)attr->extra->aggregator_as);
		}
	 ...
	 * Large Community attribute.
	 */
	if (attr->extra && CHECK_FLAG(peer->af_flags[afi][safi],
		...
		stream_put(s, attr->extra->lcommunity->val,
			   attr->extra->lcommunity->size * 12);
	}
...
void bgp_packet_mpunreach_prefix(struct stream *s, struct prefix *p, afi_t afi,
				 ...
				 u_int32_t addpath_tx_id, struct attr *attr)
{
	return bgp_packet_mpattr_prefix(s, afi, safi, p, prd, tag,
					addpath_encode, addpath_tx_id, attr);
}
