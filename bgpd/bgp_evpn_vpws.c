...
#include "bgpd/bgp_fsm.h" /* peer_established hook */
...
{
	/*
	 * VPWS is inherently SRv6 (End.DX2); readiness is established by the
	 * per-VPWS SRv6 state (a SID allocated from its bound locator), not by
	 * a BGP-instance-wide encapsulation mode.
	 */
	return vpws && vpws->bgp && vpws->ac_ifindex_valid &&
	       vpws->sid_allocated && vpws->sid_locator;
}
...
static void vpws_request_sid(struct bgp_evpn_vpws *vpws)
{
	...
	if (vpws->sid_requested || vpws->sid_allocated) {
		if (debug)
			zlog_debug("VPWS %s: req_sid skipped (requested=%d allocated=%d)",
				   vpws->name, vpws->sid_requested, vpws->sid_allocated);
		return;
	}
...
static void vpws_release_sid(struct bgp_evpn_vpws *vpws)
{
	...
	if (!vpws->sid_requested && !vpws->sid_allocated)
		return;
	/*
	 * Remove the kernel End.DX2 decap for the CURRENT SID before we drop
	 * local_sid below.  The uninstall keys off local_sid and is suppressed
	 * once it is unspecified, and the install path is suppressed while
	 * decap_installed is set - so if we zero local_sid here without first
	 * uninstalling, the old /128 decap is orphaned in the dataplane and a
	 * later re-allocation (e.g. `locator` change from the instance locator to
	 * a per-instance one) never installs the new SID's decap.
	 */
...
int bgp_evpn_vpws_set_locator(struct bgp_evpn_vpws *vpws, const char *locname)
{
	...
	if (want)
		snprintfrr(vpws->locator_name, sizeof(vpws->locator_name), "%s", locname);
	else
...
/*
 * Force every VPWS instance whose End.DX2 SID is drawn from @locname to
 * release and re-request that SID.  Called from the ZEBRA_SRV6_LOCATOR_ADD
 * handler when a locator's format/encoding changes (legacy <-> uSID): the
 * locator NAME is unchanged, so bgp_evpn_vpws_set_locator() would no-op, but
 * the SID must be reallocated from the new function block (0xe000+ for uSID)
 * and re-advertised with the correct uDX2 behaviour.  This mirrors
 * zebra_srv6_evi_on_locator_update() for the per-EVI DT2U/DT2M SIDs.
 */
void bgp_evpn_vpws_reissue_locator_sids(struct bgp *bgp, const char *locname)
{
	struct bgp_evpn_vpws *vpws;

	if (!bgp || !bgp->evpn_vpws_inited || !locname || !locname[0])
		return;

	frr_each (evpn_vpws_list, &bgp->evpn_vpws_list, vpws) {
		const char *eff = (vpws->locator_name[0] != '\0')
					  ? vpws->locator_name
					  : bgp->srv6_locator_name;

		if (!eff || !eff[0] || strcmp(eff, locname) != 0)
			continue;

		zlog_debug("VPWS %s: locator %s encoding changed, reissuing End.DX2 SID",
			   vpws->name, locname);
		vpws_release_sid(vpws);
		vpws_request_sid(vpws);
	}
...
static void vpws_build_attr(const struct bgp_evpn_vpws *vpws, struct attr *attr)
{
	...
	/*
	 * An SRv6 underlay is IPv6, so advertise a locally owned IPv6 address
	 * as the nexthop: an IPv4 router-id is unreachable there and a strict
	 * receiver (e.g. IOS-XR) will not build a remote EAD from it. Derive it
	 * the same way as the SRv6 L2VNI / L3VNI EVPN routes do (local address
	 * of an established IPv6 session).  Until such a session is up, fall
	 * back to the IPv4 router-id; bgp_evpn_vpws_peer_established()
	 * refreshes the route once one is.
	 */
	if (bgp_evpn_srv6_local_originator(bgp, &originator)) {
		bgp_evpn_vtep_ip_to_attr_nh(&originator, attr);
	} else {
	...
	/*
	 * The export RTs are the only extended communities. No Encapsulation
	 * Type community is attached: VPWS is always SRv6 (see
	 * vpws_xc_sid_ready()), the registry has no SRv6 tunnel type, and the
	 * SRv6 binding is carried by the Prefix-SID attribute below. Tagging it
	 * VXLAN contradicted that and is not what a native SRv6 peer sends.
	 */
	if (vpws->export_rtl)
		bgp_attr_set_ecommunity(attr, ecommunity_dup(vpws->export_rtl));
	 ...
	 */
	srv6_l2vpn->endpoint_behavior = CHECK_FLAG(xc_loc->flags, SRV6_LOCATOR_USID)
						? SRV6_ENDPOINT_BEHAVIOR_UDX2
...
/*
 * peer_established hook: when a peer with an IPv6 local address reaches
 * Established, the EAD-EVI nexthop may move from the IPv4 router-id fallback to
 * that IPv6 address.  Re-originate advertised VPWS instances so routes built
 * before the IPv6 session was up get the correct nexthop.  (In the IRB tree
 * this call lives in bgp_evpn_handle_peer_established(); the community tree has
 * no such handler, so drive it from a dedicated hook here.)
 */
static int bgp_evpn_vpws_peer_established_hook(struct peer *peer)
{
	struct bgp *bgp_evpn = bgp_get_evpn();

	if (bgp_evpn)
		bgp_evpn_vpws_peer_established(bgp_evpn);
	return 0;
}

int bgp_evpn_vpws_init(struct bgp *bgp)
{
	static bool peer_established_hooked;
	...
	if (!peer_established_hooked) {
		hook_register(peer_established,
			      bgp_evpn_vpws_peer_established_hook);
		peer_established_hooked = true;
	}
	return 0;
}
...
int bgp_evpn_vpws_originate(struct bgp_evpn_vpws *vpws)
{
	...
	if (!vpws_xc_sid_ready(vpws)) {
		if (debug)
			zlog_debug("VPWS %s: cross-connect SID not ready, skip originate",
				   vpws->name);
		return 0;
	}
...
/*
 * A peer with an IPv6 local address reached Established, so the EAD-EVI
 * nexthop chosen by vpws_build_attr() may now move from the IPv4 router-id
 * fallback to that IPv6 address. Re-run origination for every advertised
 * instance: it updates the path in place only when the attributes differ, so
 * an already-correct route is not flapped.
 */
void bgp_evpn_vpws_peer_established(struct bgp *bgp)
{
	struct bgp_evpn_vpws *vpws;
...
static void vpws_config_write_one(struct vty *vty, const struct bgp_evpn_vpws *vpws)
{
	...
	if (vpws->ac_ifname[0])
		vty_out(vty, "   interface %s%s bridge %s\n", vpws->ac_ifname,
			vpws->sid_auto ? " sid auto" : "", vpws->bridge_ifname);
...
/*
 * Real SID of a remote VPWS EAD.
 *
 * With SRv6 SID transposition (RFC 9252) a peer leaves the function bits out
 * of the SID address and carries them in the high-order bits of the route's
 * label instead (e.g. IOS-XR: len 16, offset 48). Rebuild the SID from both;
 * using the address alone gives the bare locator, and encapsulating to that
 * never reaches the End.DX2 SID.
 *
 * This is the same transposition the SRv6 L3VPN paths already apply (see
 * bgp_nht.c): transpose_sid() with the 20-bit label from decode_label(),
 * which reads the 3 wire-order bytes kept by bgp_update() independent of host
 * byte order. transposition_len/offset were bounded by the attribute parser.
 * A path with no label keeps the SID as received.
 */
void bgp_evpn_vpws_peer_sid(const struct bgp_attr_srv6_l3service *svc, mpls_label_t label,
			    struct in6_addr *sid)
{
	*sid = svc->sid;
	...
	if (label == MPLS_INVALID_LABEL) {
		zlog_warn("VPWS: SID %pI6 uses transposition (len %u offset %u) but the path has no label, using it as received",
			  &svc->sid, svc->transposition_len, svc->transposition_offset);
		return;
	}

	transpose_sid(sid, decode_label(&label), svc->transposition_offset,
		      svc->transposition_len, BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_LABEL);
}
...
void bgp_evpn_vpws_handle_remote_ead(struct bgp *bgp, const struct prefix_evpn *p,
				     ...
				     struct peer *peer, mpls_label_t label)
{
	struct bgp_evpn_vpws *vpws;
	struct in6_addr real_sid;
	uint32_t eth_tag;
	int debug = BGP_DEBUG(zebra, ZEBRA);
	...
	bgp_evpn_vpws_peer_sid(svc, label, &real_sid);
	 ...
	 *
	 * Compare against the reconstructed SID, the same form stored in
	 * peer_sid: the raw SID differs from it whenever the peer uses
	 * transposition, which would withdraw and reinstall a correct /128 on
	 * every update.
	 */
	if (vpws->peer_present && !IN6_IS_ADDR_UNSPECIFIED(&vpws->peer_sid) &&
	    memcmp(&vpws->peer_sid, &real_sid, sizeof(vpws->peer_sid)) != 0 &&
	    vpws->peer_attr_snap) {
		if (debug)
			zlog_debug("VPWS %s: peer SID changed (%pI6 -> %pI6), withdrawing old underlay /128",
				   vpws->name, &vpws->peer_sid, &real_sid);
		bgp_evpn_program_srv6_ipv6_route(bgp, &vpws->peer_sid, vpws->peer_attr_snap,
						 vpws->peer_peer_snap, false);
	}
	...
	if (debug)
		zlog_debug("VPWS %s: peer SID %pI6 (received %pI6, transposition len %u offset %u)",
			   vpws->name, &real_sid, &svc->sid, svc->transposition_len,
			   svc->transposition_offset);

	vpws->peer_sid = real_sid;
	...
	if (debug)
		zlog_debug("VPWS %s: peer SID learned (eth_tag=%u behavior=0x%04x)", vpws->name,
			   eth_tag, svc->endpoint_behavior);
}
				...
				const char *bridge_ifname)
{
	...
	if (!vpws || !ifname || !ifname[0])
		return -1;
	/* The operator-created bridge is mandatory: zebra no longer creates a
	 * per-instance bridge; it enslaves the AC + srl2 to this bridge.
	 */
	if (!bridge_ifname || !bridge_ifname[0])
		return -1;
	...
	strlcpy(vpws->bridge_ifname, bridge_ifname, sizeof(vpws->bridge_ifname));
				     ...
				     const struct srv6_locator *locator, bool allocated)
{
	...
	if (behavior != ZEBRA_SEG6_LOCAL_ACTION_END_DX2) {
		if (debug)
			zlog_debug("VPWS notify-dispatch: skipped (behavior 0x%04x != END_DX2)",
				   behavior);
		return false;
	}
	if (oif == 0) {
	...
	if (allocated) {
		...
		if (debug)
			zlog_debug("VPWS %s: SRv6 SID released (oif=%u evi=%u)", vpws->name, oif,
				   evi);
		bgp_zebra_send_vpws_local_del(vpws->name);
	}
	return true;
