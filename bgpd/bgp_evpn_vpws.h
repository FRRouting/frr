...
struct bgp_evpn_vpws {
	...
	char bridge_ifname[IFNAMSIZ]; /* operator bridge the AC is bound to */
...
/* Reissue End.DX2 SIDs drawn from @locname on a locator format/encoding
 * change (legacy <-> uSID); called from the SRV6_LOCATOR_ADD handler.
 */
extern void bgp_evpn_vpws_reissue_locator_sids(struct bgp *bgp, const char *locname);
...
/*
 * An IPv6-addressed peer reached Established: refresh advertised EAD-EVI
 * routes so their nexthop is the local IPv6 session address, not the IPv4
 * router-id fallback used while no IPv6 session was up.
 */
extern void bgp_evpn_vpws_peer_established(struct bgp *bgp);

/*
 * Real SID of a remote VPWS EAD: the SID address with any SRv6 SID
 * transposition applied (function bits carried in the route's label).
 * Use this, not svc->sid, wherever the peer SID is programmed or compared.
 */
extern void bgp_evpn_vpws_peer_sid(const struct bgp_attr_srv6_l3service *svc, mpls_label_t label,
				   struct in6_addr *sid);
...
extern void bgp_evpn_vpws_handle_remote_ead(struct bgp *bgp, const struct prefix_evpn *p,
					    ...
					    struct attr *attr, struct peer *peer, mpls_label_t label);
...
extern int bgp_evpn_vpws_set_interface(struct bgp_evpn_vpws *vpws, const char *ifname,
				       bool sid_auto, const char *bridge_ifname);
