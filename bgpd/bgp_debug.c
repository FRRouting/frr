				     ...
				     struct list *list)
{
	...
	if (list && !list_isempty(list)) {
		for (ALL_LIST_ELEMENTS(list, node, nnode, filter)) {
			if (filter->host) {
				vty_out(vty, "%s %s%s", desc, filter->host,
					VTY_NEWLINE);
				write++;
			}
...
int bgp_dump_attr(struct peer *peer, struct attr *attr, char *buf, size_t size)
{
	...
	if (CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_AGGREGATOR)))
		snprintf(buf + strlen(buf), size - strlen(buf),
			 ", aggregated by %u %s", attr->extra->aggregator_as,
       ...
       "BGP Neighbor Events\n")
{
	...
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP neighbor-events debugging is on%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (bgp_debug_list_has_entry(bgp_debug_neighbor_events_peers, host,
				     NULL)) {
		vty_out(vty,
			"BGP neighbor-events debugging is already enabled for %s%s",
			host, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (vty->node == CONFIG_NODE)
		...
		TERM_DEBUG_ON(neighbor_events, NEIGHBOR_EVENTS);
		vty_out(vty, "BGP neighbor-events debugging is on for %s%s",
			host, VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "Neighbor Events\n")
{
	...
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP neighbor-events debugging is off%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (found_peer)
		vty_out(vty, "BGP neighbor-events debugging is off for %s%s",
			host, VTY_NEWLINE);
	else
		vty_out(vty,
			"BGP neighbor-events debugging was not enabled for %s%s",
			host, VTY_NEWLINE);
       ...
       "BGP nexthop tracking events\n")
{
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP nexthop tracking debugging is on%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP nexthop tracking events\n")
{
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP nexthop tracking debugging is off%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (bgp_debug_list_has_entry(bgp_debug_keepalive_peers, host, NULL)) {
		vty_out(vty,
			"BGP keepalive debugging is already enabled for %s%s",
			host, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (vty->node == CONFIG_NODE)
		...
		TERM_DEBUG_ON(keepalive, KEEPALIVE);
		vty_out(vty, "BGP keepalives debugging is on for %s%s", host,
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (found_peer)
		vty_out(vty, "BGP keepalives debugging is off for %s%s", host,
			VTY_NEWLINE);
	else
		vty_out(vty,
			"BGP keepalives debugging was not enabled for %s%s",
			host, VTY_NEWLINE);
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_list_has_entry(bgp_debug_bestpath_prefixes, NULL,
				     argv_p)) {
		vty_out(vty,
			"BGP bestptah debugging is already enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (vty->node == CONFIG_NODE) {
		...
		vty_out(vty, "BGP bestpath debugging is on for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	}
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_bestpath_prefixes
		...
		if (list_isempty(bgp_debug_bestpath_prefixes)) {
			if (vty->node == CONFIG_NODE) {
				...
				vty_out(vty,
					"BGP bestpath debugging (per prefix) is off%s",
					VTY_NEWLINE);
			}
		}
	}
	...
	if (found_prefix)
		vty_out(vty, "BGP bestpath debugging is off for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	else
		vty_out(vty, "BGP bestpath debugging was not enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
       ...
       "Outbound updates\n")
{
	...
	if (vty->node == CONFIG_NODE) {
		...
		if (strncmp("i", argv[idx_in_out]->arg, 1) == 0) {
			TERM_DEBUG_ON(update, UPDATE_IN);
			vty_out(vty, "BGP updates debugging is on (inbound)%s",
				VTY_NEWLINE);
		} else {
			TERM_DEBUG_ON(update, UPDATE_OUT);
			vty_out(vty, "BGP updates debugging is on (outbound)%s",
				VTY_NEWLINE);
		}
	}
	return CMD_SUCCESS;
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (inbound) {
		if (bgp_debug_list_has_entry(bgp_debug_update_in_peers, host,
					     NULL)) {
			vty_out(vty,
				"BGP inbound update debugging is already enabled for %s%s",
				host, VTY_NEWLINE);
			return CMD_SUCCESS;
		}
	}
		...
		if (bgp_debug_list_has_entry(bgp_debug_update_out_peers, host,
					     NULL)) {
			vty_out(vty,
				"BGP outbound update debugging is already enabled for %s%s",
				host, VTY_NEWLINE);
			return CMD_SUCCESS;
		}
	}
	...
	if (vty->node == CONFIG_NODE) {
		...
		if (inbound) {
			TERM_DEBUG_ON(update, UPDATE_IN);
			vty_out(vty,
				"BGP updates debugging is on (inbound) for %s%s",
				argv[idx_peer]->arg, VTY_NEWLINE);
		} else {
			TERM_DEBUG_ON(update, UPDATE_OUT);
			vty_out(vty,
				"BGP updates debugging is on (outbound) for %s%s",
				argv[idx_peer]->arg, VTY_NEWLINE);
		}
	}
	return CMD_SUCCESS;
       ...
       "Outbound updates\n")
{
	...
	if (strncmp("i", argv[idx_in_out]->arg, 1) == 0) {
		...
		if (vty->node == CONFIG_NODE) {
			...
			vty_out(vty, "BGP updates debugging is off (inbound)%s",
				VTY_NEWLINE);
		}
	} else {
		bgp_debug_list_free(bgp_debug_update_out_peers);
		...
		if (vty->node == CONFIG_NODE) {
			...
			vty_out(vty,
				"BGP updates debugging is off (outbound)%s",
				VTY_NEWLINE);
		}
	}
       ...
       "BGP neighbor on interface to debug\n")
{
	...
	if (inbound && bgp_debug_update_in_peers
		...
		if (list_isempty(bgp_debug_update_in_peers)) {
			if (vty->node == CONFIG_NODE)
				...
				TERM_DEBUG_OFF(update, UPDATE_IN);
				vty_out(vty,
					"BGP updates debugging (inbound) is off%s",
					VTY_NEWLINE);
			}
		}
	}
	...
	if (!inbound && bgp_debug_update_out_peers
		...
		if (list_isempty(bgp_debug_update_out_peers)) {
			if (vty->node == CONFIG_NODE)
				...
				TERM_DEBUG_OFF(update, UPDATE_OUT);
				vty_out(vty,
					"BGP updates debugging (outbound) is off%s",
					VTY_NEWLINE);
			}
		}
	...
	if (found_peer)
		if (inbound)
			vty_out(vty,
				"BGP updates debugging (inbound) is off for %s%s",
				host, VTY_NEWLINE);
		else
			vty_out(vty,
				"BGP updates debugging (outbound) is off for %s%s",
				host, VTY_NEWLINE);
	else if (inbound)
		vty_out(vty,
			"BGP updates debugging (inbound) was not enabled for %s%s",
			host, VTY_NEWLINE);
	else
		vty_out(vty,
			"BGP updates debugging (outbound) was not enabled for %s%s",
			host, VTY_NEWLINE);
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_list_has_entry(bgp_debug_update_prefixes, NULL, argv_p)) {
		vty_out(vty,
			"BGP updates debugging is already enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (vty->node == CONFIG_NODE) {
		...
		vty_out(vty, "BGP updates debugging is on for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	}
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_update_prefixes
		...
		if (list_isempty(bgp_debug_update_prefixes)) {
			if (vty->node == CONFIG_NODE) {
				...
				vty_out(vty,
					"BGP updates debugging (per prefix) is off%s",
					VTY_NEWLINE);
			}
		}
	}
	...
	if (found_prefix)
		vty_out(vty, "BGP updates debugging is off for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	else
		vty_out(vty, "BGP updates debugging was not enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_list_has_entry(bgp_debug_zebra_prefixes, NULL, argv_p)) {
		vty_out(vty, "BGP zebra debugging is already enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (vty->node == CONFIG_NODE)
		...
		TERM_DEBUG_ON(zebra, ZEBRA);
		vty_out(vty, "BGP zebra debugging is on for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	}
       ...
       "IPv6 prefix\n")
{
	...
	if (bgp_debug_zebra_prefixes
		...
		if (list_isempty(bgp_debug_zebra_prefixes)) {
			if (vty->node == CONFIG_NODE)
				...
				TERM_DEBUG_OFF(zebra, ZEBRA);
				vty_out(vty, "BGP zebra debugging is off%s",
					VTY_NEWLINE);
			}
		}
	}
	...
	if (found_prefix)
		vty_out(vty, "BGP zebra debugging is off for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
	else
		vty_out(vty, "BGP zebra debugging was not enabled for %s%s",
			argv[idx_ipv4_ipv6_prefixlen]->arg, VTY_NEWLINE);
       ...
       "BGP update-groups\n")
{
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP update-groups debugging is on%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       "BGP update-groups\n")
{
	if (vty->node == CONFIG_NODE)
		...
		vty_out(vty, "BGP update-groups debugging is off%s",
			VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
       ...
       BGP_STR)
{
	...
	TERM_DEBUG_OFF(zebra, ZEBRA);
	TERM_DEBUG_OFF(allow_martians, ALLOW_MARTIANS);
	vty_out(vty, "All possible debugging has been turned off%s",
		VTY_NEWLINE);
       ...
       BGP_STR)
{
	...
	if (BGP_DEBUG(as4, AS4_SEGMENT))
		vty_out(vty, "  BGP as4 aspath segment debugging is on%s",
			VTY_NEWLINE);
	...
	if (BGP_DEBUG(nht, NHT))
		vty_out(vty, "  BGP next-hop tracking debugging is on%s",
			VTY_NEWLINE);
...
const char *bgp_debug_rdpfxpath2str(struct prefix_rd *prd,
				    union prefixconstptr pu, int addpath_valid,
