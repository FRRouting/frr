...
int rfapiGetL2o(struct attr *attr, struct rfapi_l2address_option *l2o)
{
	if (attr && attr->extra) {
		...
		for (pEncap = attr->extra->vnc_subtlvs; pEncap;
		     pEncap = pEncap->next) {
...
int rfapiGetVncTunnelUnAddr(struct attr *attr, struct prefix *p)
{
	...
	if (attr && attr->extra) {
		for (pEncap = attr->extra->encap_subtlvs; pEncap;
		     pEncap = pEncap->next) {
...
static int rfapiVpnBiNhEqualsPt(struct bgp_info *bi, struct rfapi_ip_addr *hpt)
{
	...
	switch (family) {
	case AF_INET:
		if (bi->attr->extra->mp_nexthop_global_in.s_addr
		    != hpt->addr.v4.s_addr)
			return 0;
		break;

	case AF_INET6:
		if (IPV6_ADDR_CMP(&bi->attr->extra->mp_nexthop_global,
				  &hpt->addr.v6))
			return 0;
		break;
			    ...
			    struct route_node *rn) /* req for L2 eth addr */
{
	...
	if (bi->attr) {
		...
		if (bi->attr->extra) {
			...
			switch (BGP_MP_NEXTHOP_FAMILY(
				bi->attr->extra->mp_nexthop_len)) {
			case AF_INET:
			...
			for (pEncap = bi->attr->extra->vnc_subtlvs; pEncap;
				...
				case BGP_VNC_SUBTLV_TYPE_LIFETIME:
					/* use configured lifetime, not attr
					 * lifetime */
					break;

				default:
					zlog_warn(
						"%s: unknown VNC option type %d",
...
#if DEBUG_ENCAP_MONITOR
			vnc_zlog_debug_verbose(
				"%s: line %d: have_vnc_tunnel_un=%d", __func__,
				     ...
				     safi_t safi)
{
	...
	for (prev = NULL, next = rn->info; next;
		...
		if (!bgp || (!CHECK_FLAG(info_new->flags, BGP_INFO_REMOVED)
		    ...
		    || bgp_info_cmp_compatible(bgp, info_new, next, afi, safi)
...
void rfapiNexthop2Prefix(struct attr *attr, struct prefix *p)
{
	...
	switch (p->family =
			BGP_MP_NEXTHOP_FAMILY(attr->extra->mp_nexthop_len)) {
	case AF_INET:
			  ...
			  int (*timer_service_func)(struct thread *))
{
	...
	if (lifetime > UINT32_MAX / 1001) {
		...
		bi->extra->vnc.import.timer = thread_add_timer(
			bm->master, timer_service_func, wcb, lifetime);
	} else {
		...
		bi->extra->vnc.import.timer = thread_add_background(
			bm->master, timer_service_func, wcb, lifetime_msec);
	}
...
static int rfapiGetNexthop(struct attr *attr, struct prefix *prefix)
{
	...
	default:
		vnc_zlog_debug_verbose(
			"%s: unknown attr->extra->mp_nexthop_len %d", __func__,
