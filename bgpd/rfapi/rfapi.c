		   ...
		   struct rfapi_nexthop *lnh, int kill)
{
	...
	for (bpi = (bn ? bgp_dest_get_bgp_path_info(bn) : NULL); bpi;
	     bpi = bpi->next) {
		vnc_zlog_debug_verbose(
			"%s: trying bpi=%p, bpi->peer=%p, bpi->type=%d, bpi->sub_type=%d, bpi->extra->vnc.export.rfapi_handle=%p, local_pref=%" PRIu32,
			__func__, bpi, bpi->peer, bpi->type, bpi->sub_type,
			(bpi->extra ? bpi->extra->vnc->vnc.export.rfapi_handle : NULL),
			bgp_attr_exists(bpi->attr, BGP_ATTR_LOCAL_PREF) ? bpi->attr->local_pref
									: 0);
