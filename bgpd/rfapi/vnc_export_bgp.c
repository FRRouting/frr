				 ...
				 struct prefix *use_nexthop)
{
	...
	case AF_INET6:
		if (!new->extra) {
			new->extra = XCALLOC(MTYPE_ATTR_EXTRA,
...
static struct ecommunity *vnc_route_origin_ecom(struct route_node *rn)
{
	...
	for (bi = rn->info; bi; bi = bi->next) {
		...
		case AF_INET:
			...
			roec.val[1] = 0x03;
			memcpy(roec.val + 2,
			       &bi->attr->extra->mp_nexthop_global_in.s_addr,
			       4);
			roec.val[6] = 0;
			roec.val[7] = 0;
					 ...
					 /* if rn is 0, use route's nexthop */
{
	...
	if (new_nexthop) {
		...
		if (orig_nexthop.family == AF_INET) {
			...
			orig_nexthop.u.prefix4 =
				orig->extra->mp_nexthop_global_in;
	...
	if (new->extra->ecommunity) {
		if (ecom_ro) {
			new->extra->ecommunity = ecommunity_merge(
				ecom_ro, new->extra->ecommunity);
		}
	} else {
				 ...
				 struct prefix *prefix, struct peer *peer)
{
	...
	if (!eti->timer && eti->lifetime <= INT32_MAX) {
		eti->timer = thread_add_timer(
			bm->master, vncExportWithdrawTimer, eti, eti->lifetime);
		vnc_zlog_debug_verbose(
