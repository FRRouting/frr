				 ...
				 struct vty *vty, u_int8_t flags)
{
	...
	if (output_count != 0)
		vty_out(vty, "%sTotal number of prefixes %ld%s", VTY_NEWLINE,
			output_count, VTY_NEWLINE);
}
	...
	for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
		for (ri = rn->info; ri; ri = ri->next)
			...
			if (CHECK_FLAG(ri->flags, BGP_INFO_SELECTED)
				...
				if (subgroup_announce_check(ri, subgrp, &rn->p,
							    &attr))
...
void subgroup_announce_all(struct update_subgroup *subgrp)
{
	 ...
	 */
	if (!subgrp->t_coalesce) {
		THREAD_TIMER_MSEC_ON(bm->master, subgrp->t_coalesce,
				     subgroup_coalesce_timer, subgrp,
				     subgrp->v_coalesce);
	}
}
