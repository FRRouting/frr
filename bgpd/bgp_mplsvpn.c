		       ...
		       struct bgp_nlri *packet)
{
	 ...
	 * srv6_service (and other attr->extra members) for prefixes
					 ...
					 const struct bgp_path_info *rmap_bpi)
{
	...
	for (bpi = bgp_dest_get_bgp_path_info(bn); bpi; bpi = bpi->next) {
		if (!bpi->extra || !bpi->extra->vrfleak ||
			...
			continue;
		if (bgp_attr_get_srv6_service(static_attr) &&
		    !bgp_attr_get_srv6_service(bpi->attr) && !bgp_labels_is_implicit_null(bpi))
			/* SRv6 path can not overwrite MPLS path */
			continue;
		if ((!bgp_attr_get_srv6_service(static_attr) && num_labels == 1 &&
		     decode_label(&label[0]) != MPLS_LABEL_NONE) &&
		    bgp_attr_get_srv6_service(bpi->attr))
					...
					mpls_label_t *label)
{
	...
	if (from_bgp->vpn_policy[afi].tovpn_sid_locator && from_bgp->vpn_policy[afi].tovpn_sid) {
		...
		if (CHECK_FLAG(from_bgp->vpn_policy[afi].flags, BGP_VPN_POLICY_TOVPN_SID_FUNC_WIDE))
			srv6_service->func_len = BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_BGP;
		else
		...
		if (srv6_service->func_len > BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_LABEL) {
			srv6_service->transposition_len = 0;
			srv6_service->transposition_offset = 0;
			IPV6_ADDR_COPY(&srv6_service->sid, from_bgp->vpn_policy[afi].tovpn_sid);
		} else {
			...
			srv6_service->transposition_offset =
				from_bgp->vpn_policy[afi].tovpn_sid_locator->block_bits_length +
				from_bgp->vpn_policy[afi].tovpn_sid_locator->node_bits_length;
			IPV6_ADDR_COPY(&srv6_service->sid,
	...
	if (from_bgp->tovpn_sid_locator && from_bgp->tovpn_sid) {
		...
		srv6_service->loc_block_len = from_bgp->tovpn_sid_locator->block_bits_length;
		srv6_service->loc_node_len = from_bgp->tovpn_sid_locator->node_bits_length;
		if (CHECK_FLAG(from_bgp->vrf_flags, BGP_VRF_TOVPN_SID_FUNC_WIDE))
			srv6_service->func_len = BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_BGP;
		else
			srv6_service->func_len = from_bgp->tovpn_sid_locator->function_bits_length;
		srv6_service->arg_len = from_bgp->tovpn_sid_locator->argument_bits_length;
		if (srv6_service->func_len > BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_LABEL) {
			srv6_service->transposition_len = 0;
			srv6_service->transposition_offset = 0;
			IPV6_ADDR_COPY(&srv6_service->sid, from_bgp->tovpn_sid);
		} else {
			srv6_service->transposition_len =
				...
				struct bgp_path_info *path_vrf) /* route */
{
	...
	while (bpi_next) {
		...
		if (!bpi->extra || !bpi->extra->vrfleak || bpi->extra->vrfleak->parent != path_vrf)
			continue;
		if (bgp_attr_get_srv6_service(path_vrf->attr) &&
		    !bgp_attr_get_srv6_service(bpi->attr) && !bgp_labels_is_implicit_null(bpi))
			/* SRv6 path can not overwrite MPLS path */
			continue;
		if ((!bgp_attr_get_srv6_service(path_vrf->attr) &&
	...
	if (aspath_loop_check(path_vpn->attr->aspath, to_bgp->as) > aspath_loop_count) {
		for (bpi = bgp_dest_get_bgp_path_info(bn); bpi;
			...
			if (!bpi->extra || !bpi->extra->vrfleak ||
				...
				continue;
			if (bgp_attr_get_srv6_service(path_vpn->attr) &&
			    !bgp_attr_get_srv6_service(bpi->attr) &&
...
{
	if (pi->attr && bgp_attr_get_srv6_service(pi->attr))
		/* srv6 sid */
		return false;
