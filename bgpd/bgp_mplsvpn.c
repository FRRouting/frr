				 ...
				 afi_t afi)
{
	...
	if (debug)
		zlog_debug("requesting VPN SID with behavior=%s for vrf %s locator %s",
			   seg6local_action2str(ctx.behavior), bgp_vrf->name_pretty,
			   locator_bgp->name);
...
void bgp_evpn_srv6_install_local_decap(struct bgp *bgp, struct bgpevpn *vpn)
{
	...
	if (!bgp || !vpn)
		return;
	if (!is_vpn_srv6(vpn)) /* per-EVI encapsulation */
		return;

	loc = bgp_srv6_locator_lookup(bgp, bgp_get_default());
	/* Per-EVI: derive the decap SID geometry + uSID (NEXT-CSID) flavor from
	 * THIS EVI's own locator (metadata shipped by zebra in VNI_ADD), not the
	 * single BGP-instance locator, so each EVI's local decap matches its own
	 * `behavior usid` / `format`.  Fall back to the instance locator only
	 * when zebra shipped no per-EVI metadata (older zebra).
	 */
	if (vpn->srv6_loc_meta_valid) {
		ctx.block_len = vpn->srv6_loc_block_len;
		ctx.node_len = vpn->srv6_loc_node_len;
		ctx.function_len = vpn->srv6_loc_func_len;
		ctx.argument_len = vpn->srv6_loc_arg_len;
		if (vpn->srv6_loc_is_usid)
			SET_SRV6_FLV_OP(ctx.flv.flv_ops, ZEBRA_SEG6_LOCAL_FLV_OP_NEXT_CSID);
	} else if (loc) {
