...
static int bgp_zebra_process_srv6_locator_delete(ZAPI_CALLBACK_ARGS)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bm->bgp, node, bgp_vrf)) {
		...
		tovpn_sid_locator_explicit = bgp_vrf->tovpn_sid_locator_explicit;
		if (tovpn_sid_locator_explicit) {
			tmp_prefi.family = AF_INET6;
			tmp_prefi.prefixlen = IPV6_MAX_BITLEN;
			tmp_prefi.prefix = tovpn_sid_locator_explicit->prefix.prefix;
			if (prefix_match((struct prefix *)&loc.prefix,
					 (struct prefix *)&tmp_prefi)) {
...
int bgp_zebra_srv6_manager_get_locator_static_sids(const char *name)
{
	 ...
	 */
	return srv6_manager_get_locator_static_sids(zclient, name);
}
