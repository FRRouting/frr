...
static int update_group_show_walkcb(struct update_group *updgrp, void *arg)
{
	...
	vty_out(vty, "  MRAI value (seconds): %d%s", updgrp->conf->v_routeadv,
		VTY_NEWLINE);
	...
	UPDGRP_FOREACH_SUBGRP(updgrp, subgrp)
	{
		...
		vty_out(vty, "  Update-subgroup %" PRIu64 ":%s", subgrp->id,
			VTY_NEWLINE);
		...
		if (subgrp->split_from.update_group_id
			...
			vty_out(vty, "    Split from group id: %" PRIu64 "%s",
				...
				VTY_NEWLINE);
			vty_out(vty,
				"    Split from subgroup id: %" PRIu64 "%s",
				subgrp->split_from.subgroup_id, VTY_NEWLINE);
		}

		vty_out(vty, "    Join events: %u%s", subgrp->join_events,
			VTY_NEWLINE);
		vty_out(vty, "    Prune events: %u%s", subgrp->prune_events,
			VTY_NEWLINE);
		vty_out(vty, "    Merge events: %u%s", subgrp->merge_events,
			VTY_NEWLINE);
		vty_out(vty, "    Split events: %u%s", subgrp->split_events,
			VTY_NEWLINE);
		...
		vty_out(vty, "    Version: %" PRIu64 "%s", subgrp->version,
			VTY_NEWLINE);
		...
		vty_out(vty, "    Packet queue high watermark: %d%s",
			...
			VTY_NEWLINE);
		vty_out(vty, "    Adj-out list count: %u%s", subgrp->adj_count,
			VTY_NEWLINE);
...
void update_group_show_stats(struct bgp *bgp, struct vty *vty)
{
	...
	vty_out(vty, "Join events: %u%s", bgp->update_group_stats.join_events,
		VTY_NEWLINE);
	vty_out(vty, "Prune events: %u%s", bgp->update_group_stats.prune_events,
		VTY_NEWLINE);
	vty_out(vty, "Merge events: %u%s", bgp->update_group_stats.merge_events,
		VTY_NEWLINE);
	vty_out(vty, "Split events: %u%s", bgp->update_group_stats.split_events,
		VTY_NEWLINE);
	vty_out(vty, "Update group switch events: %u%s",
