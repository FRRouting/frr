			...
			mpls_label_t *label, uint32_t num_labels)
{
	...
	bmp_per_peer_hdr(hdr, peer->bgp, peer, flags, peer_type,
			 peer_distinguisher,
...
static bool bmp_wrqueue_ribin(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	bmp_monitor(bmp, peer, 0, peer_type_flag, &bqe->p, prd,
...
static int bmp_nht_path_valid(struct bgp *bgp, struct bgp_path_info *path, bool valid)
{
	...
	return bmp_process_ribinpre(bgp, table->afi, table->safi, dest,
				    path->addpath_rx_id, path->peer, true);
}
...
static void bmp_stats_per_instance(struct bgp *bgp, struct bmp_targets *bt)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bgp->peer, node, peer)) {
		...
		bmp_per_peer_hdr(s, bt->bgp, peer, 0, peer_type_flag,
				 peer_distinguisher, &tv);
				   ...
				   struct bgp_path_info *new_route)
{
	...
	if (old_route && new_route && old_route != new_route) {
		new_head = bmp_process_one(bt, &bt->mon_loc_updhash,
				&bt->mon_loc_updlist, afi, safi, bn,
				old_route->addpath_rx_id, old_route->peer,
				BMP_MON_LOC_RIB);
	}
