...
static int bmp_send_peerup_vrf(struct bmp *bmp)
{
	...
	s = bmp_peerstate(bmpbgp->bgp->peer_self, bmpbgp->vrf_up == vrf_state_down);
...
bool bmp_bgp_update_vrf_status(struct bmp_bgp *bmpbgp, enum bmp_vrf_state force)
{
	...
	bgp = bmpbgp->bgp;
	old_state = bmpbgp->vrf_up;

	vrf = bgp_vrf_lookup_by_instance_type(bgp);
	bmpbgp->vrf_up = force != vrf_state_unknown ? force
			 : vrf_is_enabled(vrf)	    ? vrf_state_up
						    : vrf_state_down;

	changed = old_state != bmpbgp->vrf_up;
	if (changed) {
