 ...
 *
 * `sid` must already be the reconstructed (post-transposition) SID - see
 * evpn_l2vni_remote_sid() - and is compared here against every other
 * candidate path's OWN reconstructed SID, not its raw attribute value;
 * two paths whose raw SIDs differ only because of a since-changed label
	...
	for (rd_dest = bgp_table_top(bgp_evpn->rib[AFI_L2VPN][SAFI_EVPN]); rd_dest;
		...
		for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
				struct bgp_attr_srv6_l3service *svc;
				struct in6_addr candidate_sid;

				if (!CHECK_FLAG(pi->flags, BGP_PATH_VALID))
					continue;
				svc = pi->attr ? bgp_attr_get_srv6_l2vpn(pi->attr) : NULL;
				if (!svc)
					continue;
				evpn_l2vni_remote_sid(svc, pi, &candidate_sid);
				if (memcmp(&candidate_sid, sid, sizeof(*sid)) == 0) {
					/* bgp_table_top()/bgp_route_next() hold a
					 * lock on the current node of each loop;
					 * release both before this early return so
					 * the references are not leaked.
					 */
					    ...
					    struct bgp_path_info *pi)
{
	...
	if (p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE) {
...

		{
			 ...
			 */
			if (svc)
				bgp_evpn_program_srv6_ipv6_route(bgp, &dt2u_sid, pi->attr,
								 pi->peer, true);
		}
		 ...
		 * /128 so the local encap can actually leave the wire, and
		 * reuse the same reconstructed SID as the BUM-flood target
		 * passed to zebra below.
		 */
		struct bgp_attr_srv6_l3service *svc = bgp_attr_get_srv6_l2vpn(pi->attr);
		...
		if (svc) {
			...
			bgp_evpn_program_srv6_ipv6_route(bgp, &dt2m_sid, pi->attr, pi->peer, true);
	...
	if (p->prefix.route_type == BGP_EVPN_MAC_IP_ROUTE) {
		 ...
		 */
		{
			...
			if (svc) {
				...
				evpn_l2vni_remote_sid(svc, pi, &dt2u_sid);
				if (!evpn_srv6_underlay_sid_in_use(bgp, &dt2u_sid, p))
					bgp_evpn_program_srv6_ipv6_route(bgp, &dt2u_sid, pi->attr,
									 pi->peer, false);
			}
		}
		...
		if (svc) {
			...
			evpn_l2vni_remote_sid(svc, pi, &dt2m_sid);
			bgp_evpn_program_srv6_ipv6_route(bgp, &dt2m_sid, pi->attr, pi->peer, false);
		}
...
static void bgp_evpn_srv6_oif_install_cb(struct hash_bucket *bucket, void *arg)
{
	...
	if (ctx->is_bum_srl2) {
		if (!vpn->srv6_dt2m_sid_valid)
			return;
		if (vpn->srv6_dt2m_oif == ctx->ifindex || vpn->srv6_dt2m_oif == 0) {
			vpn->srv6_dt2m_oif = ctx->ifindex;
			install = true;
		}
	} else {
		if (!vpn->srv6_dt2u_sid_valid)
			return;
		if (vpn->srv6_dt2u_oif == ctx->ifindex || vpn->srv6_dt2u_oif == 0) {
			vpn->srv6_dt2u_oif = ctx->ifindex;
			install = true;
		}
	}
...
void bgp_evpn_srv6_install_for_ifindex(struct bgp *bgp, ifindex_t ifindex, bool is_bum_srl2)
{
	struct evpn_srv6_oif_ctx ctx = {
		.bgp = bgp,
		.ifindex = ifindex,
		.is_bum_srl2 = is_bum_srl2,
	};
...
void bgp_evpn_srv6_clear_oif_for_ifindex(struct bgp *bgp, ifindex_t ifindex, bool is_bum_srl2)
{
	struct evpn_srv6_oif_ctx ctx = {
		.bgp = bgp,
		.ifindex = ifindex,
 ...
 */
/*
 * SRv6 L2 EVPN teardown: withdraw the remote underlay /128 routes for this VNI.
 *
 * The plain IPv6 /128 underlay route toward a remote End.DT2U/End.DT2M SID is
 * installed by bgp_evpn_program_srv6_ipv6_route() when a remote Type-2/Type-3
 * route is imported, and it is withdrawn only from evpn_zebra_uninstall() - i.e.
 * when the *received* route itself goes away (peer withdraw / path invalid).
 * Un-importing or deleting the VNI ("no evi N") does NOT withdraw it: the peer
 * is still advertising, so the received route stays valid and the /128 is
 * orphaned in the kernel (a dangling nexthop).  The local decap SIDs are
 * cleaned explicitly in evpn_delete_vni(); this is the symmetric cleanup for
 * the remote underlay side.
 *
 * Walk the global EVPN RIB for valid remote MAC-IP / IMET routes that match
 * THIS VNI's import RTs and carry an SRv6 L2 service TLV, and withdraw each
 * underlay /128.  A DT2U/DT2M SID is unique per (remote PE, VNI), so a SID
 * matched here belongs to this VNI and is safe to withdraw; duplicate
 * ROUTE_DELETEs for a shared per-VNI SID are harmless.  Must run BEFORE the RT
 * unconfigure in evpn_delete_vni() so is_route_matching_for_vni() still matches.
 */
void bgp_evpn_srv6_uninstall_remote_underlay(struct bgp *bgp, struct bgpevpn *vpn)
{
	struct bgp *bgp_evpn = bgp_get_evpn();
	struct bgp_dest *rd_dest, *dest;
	struct bgp_table *table;
	struct bgp_path_info *pi;
	uint32_t nwithdrawn = 0;

	if (!bgp || !vpn)
		return;

	/*
	 * Unconditional (not debug-gated) so the operator can confirm from the
	 * log that this teardown path actually ran in the running binary.
	 */
	zlog_info("SRv6 EVI %u: remote-underlay teardown - scanning EVPN RIB for /128s to withdraw",
		  vpn->vni);
	...
	for (rd_dest = bgp_table_top(bgp_evpn->rib[AFI_L2VPN][SAFI_EVPN]); rd_dest;
		...
		for (dest = bgp_table_top(table); dest; dest = bgp_route_next(dest)) {
			...
			for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
				const struct bgp_attr_srv6_l3service *l2vpn;
				vni_t route_vni = 0;

				if (pi->type != ZEBRA_ROUTE_BGP || pi->sub_type != BGP_ROUTE_NORMAL)
					continue;
				if (!pi->attr)
					continue;
				l2vpn = bgp_attr_get_srv6_l2vpn(pi->attr);
				if (!l2vpn)
					continue;

				/*
				 * Match the route to THIS VNI two ways, because the
				 * two route types carry the VNI differently:
				 *  - Type-2 (MAC-IP) carries the VNI in its EVPN MPLS
				 *    label -> label2vni(); this is RT-independent, so
				 *    it still works after "no evi N" has unconfigured
				 *    the import RTs.
				 *  - Type-3 (IMET) here does NOT carry that label, so
				 *    label2vni() yields 0 and misses the End.DT2M SID.
				 *    Fall back to the RT-based match, which the import-
				 *    RT reconfigure re-maps just before we run, so it
				 *    matches the IMET route.
				 * Both are VNI-scoped, so this never touches another
				 * VNI's or the VPWS (Type-1) SID.
				 */
				if (pi->extra && pi->extra->labels &&
				    pi->extra->labels->num_labels > 0)
					route_vni = label2vni(&pi->extra->labels->label[0]);
				if (route_vni != vpn->vni &&
				    !is_route_matching_for_vni(bgp, vpn, pi))
					continue;

				zlog_info("SRv6 EVI %u: withdrawing remote underlay %pI6/128 on EVI teardown",
					  vpn->vni, &l2vpn->sid);
				bgp_evpn_program_srv6_ipv6_route(bgp, &l2vpn->sid, pi->attr,
								 pi->peer, false);
				nwithdrawn++;
			}
		}
	...
	zlog_info("SRv6 EVI %u: remote-underlay teardown done, %u /128 withdraw(s) issued",
		  vpn->vni, nwithdrawn);
}
...
static int install_uninstall_type5_route_in_vpn_rib(struct bgp *bgp, struct prefix_evpn *evp,
						    struct bgp_path_info *pi, int install)
{
	struct attr attr;
	struct attr *attr_new;
...
int bgp_evpn_local_vni_del(struct bgp *bgp, vni_t vni)
{
	...
	/*
	 * SRv6 L2 EVPN: authoritative teardown of this VNI's remote
	 * End.DT2U/End.DT2M underlay /128 routes.  This is the final VNI
	 * destruction (zebra "VNI del"), with no route re-import after it, so
	 * it is the reliable place to make sure the plain IPv6 /128s toward the
	 * remote SIDs are removed from the kernel rather than left as dangling
	 * nexthops.  Run while the VNI (and its import RTs) is still intact so
	 * the received routes still match it.  Idempotent; netdevs untouched.
	 */
	/*
	 * NOTE: call unconditionally, NOT under is_vpn_srv6() - by this point
	 * bgp_evpn_srv6_uninstall_local_decap() may have cleared the DT2U/DT2M
	 * valid flags, making is_vpn_srv6() return false.  The helper is a no-op
	 * for non-SRv6 VNIs (their routes carry no SRv6 L2 TLV).
	 */
	bgp_evpn_srv6_uninstall_remote_underlay(bgp, vpn);
