...
static int update_evpn_type5_route_entry(struct bgp *bgp_evpn, struct bgp *bgp_vrf, afi_t afi,
					 ...
					 struct bgp_path_info *origin_pi, struct attr *attr,
					 ...
					 uint32_t addpath_id)
{
	...
	*route_changed = 0;
	...
	for (local_pi = bgp_dest_get_bgp_path_info(dest); local_pi; local_pi = local_pi->next) {
		...
		if (local_pi->extra->evpn->type5_origin_pi == origin_pi)
			break;
	}
	...
	if (!local_pi) {
		...
		*route_changed = 1;
		...
		pi->extra->evpn->type5_origin_pi = origin_pi;
		...
		*entry = pi;
		...
		if (!attrhash_cmp(tmp_pi->attr, attr)) {
			if (origin_pi != local_pi->extra->evpn->type5_origin_pi)
				zlog_warn(
					"Changing the originator of a type5 route, this is not right");

			local_pi->extra->evpn->type5_origin_pi = origin_pi;
					      ...
					      uint32_t addpath_id)
{
	...
	if (origin_pi) {
		for (pi = bgp_dest_get_bgp_path_info(dest); pi; pi = pi->next) {
			...
			if (pi->extra->evpn->type5_origin_pi == origin_pi)
				break;
		}
	} else
...
static int delete_evpn_type5_route(struct bgp *bgp_vrf, const struct bgp_path_info *origin_pi,
				   struct prefix_evpn *evp, uint32_t addpath_id)
{
	...
	pi = delete_evpn_route_entry(bgp_evpn, afi, safi, dest, origin_pi, addpath_id);
	if (pi)
		bgp_process(bgp_evpn, dest, pi, afi, safi);
	bgp_dest_unlock_node(dest);
	return 0;
...
void bgp_evpn_withdraw_type5_route(struct bgp *bgp_vrf, const struct bgp_path_info *origin_pi,
				   ...
				   uint32_t addpath_id)
{
	...
	ret = delete_evpn_type5_route(bgp_vrf, origin_pi, &evp, addpath_id);
	if (ret)
		flog_err(
...
void bgp_evpn_install_uninstall_default_route(struct bgp *bgp_vrf, afi_t afi, safi_t safi,
					      struct bgp_path_info *origin_pi, bool add)
{
	...
	if (add)
		bgp_evpn_advertise_type5_route(bgp_vrf, origin_pi, &ip_prefix, NULL, afi, safi, 0);
	else
		bgp_evpn_withdraw_type5_route(bgp_vrf, origin_pi, &ip_prefix, afi, safi, 0);
}
