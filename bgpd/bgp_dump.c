...
static int bgp_dump_interval_add(struct bgp_dump *bgp_dump, int interval)
{
	...
	if (interval > 0) {
		...
		bgp_dump->t_interval = thread_add_timer(
			bm->master, bgp_dump_interval_func, bgp_dump, interval);
	} else {
		/* One-off dump: execute immediately, don't affect any scheduled
		 * dumps */
		bgp_dump->t_interval = thread_add_event(
			bm->master, bgp_dump_interval_func, bgp_dump, 0);
	}
			...
			const char *interval_str)
{
	...
	if (interval_str) {
		...
		if (interval == 0) {
			vty_out(vty, "Malformed interval string%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	if (bgp_dump_all.filename) {
		...
		if (bgp_dump_all.interval_str)
			vty_out(vty, "dump bgp %s %s %s%s", type_str,
				...
				bgp_dump_all.interval_str, VTY_NEWLINE);
		else
	...
	if (bgp_dump_updates.filename) {
		...
		if (bgp_dump_updates.interval_str)
			vty_out(vty, "dump bgp %s %s %s%s", type_str,
				...
				bgp_dump_updates.interval_str, VTY_NEWLINE);
		else
			vty_out(vty, "dump bgp %s %s%s", type_str,
	...
	if (bgp_dump_routes.filename) {
		if (bgp_dump_routes.interval_str)
			vty_out(vty, "dump bgp routes-mrt %s %s%s",
				...
				bgp_dump_routes.interval_str, VTY_NEWLINE);
		else
