	...
	if (old_select && old_select == new_select
		...
		if (bgp_zebra_has_route_changed(old_select)) {
			...
			if (nhfamily == AF_INET) {
				...
				SET_IPADDR_V4(&vtep_ip);
				vtep_ip.ipaddr_v4 = old_select->attr->nexthop;
			}
	...
	if (new_select && new_select->type == ZEBRA_ROUTE_BGP
		...
		if (nhfamily == AF_INET) {
			...
			if (new_select->attr->mp_nexthop_len == BGP_ATTR_NHLEN_IPV4 ||
			   ...
			   vtep_ip.ipaddr_v4 = new_select->attr->mp_nexthop_global_in;
			else
			   vtep_ip.ipaddr_v4 = new_select->attr->nexthop;
		} else if (nhfamily == AF_INET6) {
			SET_IPADDR_V6(&vtep_ip);
		...
		if (old_select && old_select->type == ZEBRA_ROUTE_BGP &&
			...
			if (nhfamily == AF_INET) {
				SET_IPADDR_V4(&vtep_ip);
				vtep_ip.ipaddr_v4 = old_select->attr->mp_nexthop_global_in;
			} else if (nhfamily == AF_INET6) {
				SET_IPADDR_V6(&vtep_ip);
				IPV6_ADDR_COPY(&vtep_ip.ipaddr_v6,
					       &old_select->attr->mp_nexthop_global);
						    ...
						    struct bgp_path_info *pi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug(
			"del remote %s es %s evi %u ead_ip %pIA vtep %pIA",
			p->prefix.ead_addr.eth_tag ? "ead-es" : "ead-evi",
			esi_to_str(&p->prefix.ead_addr.esi, buf, sizeof(buf)),
			vpn->vni, &p->prefix.ead_addr.ip, &vtep_ip);
