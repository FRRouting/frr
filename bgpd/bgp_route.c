...
void bgp_show_cb(struct vty *vty, void *arg)
{
	...
	/* Proceed with iteration only after all data is sent to control buffer size. */
...
static int bgp_show_table_core(struct vty *vty, struct show_bgp *args)
{
	...
	for (; dest; dest = bgp_route_next(dest)) {
		if (count >= route_output_threshold)
			break;
		++count;
...
static int show_ip_bgp_all(struct vty *vty, bool uj, uint16_t show_flags, afi_t afi, safi_t safi, bool first,
							char *community, int match_p, enum bgp_show_type sh_type, void *output_arg, enum rpki_states rpki_target_state)
{
	struct show_bgp *args = XCALLOC(MTYPE_TMP, sizeof(struct show_bgp));
      ...
      "Increase table width for longer prefixes\n")
{
	...
	if (!all) {
		...
		return show_ip_bgp_all(vty, uj, show_flags, afi, safi, first, community, match_p, sh_type, output_arg, rpki_target_state);
	}
	return CMD_SUCCESS;
}
