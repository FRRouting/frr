			...
			safi_t safi)
{
	...
	if (safi != SAFI_MPLS_VPN)
		goto end_bgp_best_selection;

	if (!CHECK_FLAG(bgp->flags, BGP_FLAG_ENCAPSULATION_SELECTION) || new_select == NULL) {
	...
	if (new_select) {
		if (bgp_attr_get_srv6_l3service(new_select->attr) ||
		    bgp_attr_get_srv6_vpn(new_select->attr))
			pi_srv6 = new_select;
		else if (bgp_mplsvpn_path_uses_valid_mpls_label(new_select))
			pi_mpls = new_select;

		if (pi_mpls || pi_srv6) {
			if (!CHECK_FLAG(new_select->flags, BGP_PATH_ENCAPSULATION_SELECTED))
				bgp_path_info_set_flag(dest, new_select,
						       BGP_PATH_ENCAPSULATION_SELECTED);
		}
	}

	if (pi_mpls == NULL && pi_srv6 == NULL) {
		for (pi1 = bgp_dest_get_bgp_path_info(dest); pi1; pi1 = pi1->next)
			if (CHECK_FLAG(pi1->flags, BGP_PATH_ENCAPSULATION_SELECTED))
				bgp_path_info_unset_flag(dest, pi1,
							 BGP_PATH_ENCAPSULATION_SELECTED);
	...
	for (pi1 = bgp_dest_get_bgp_path_info(dest); pi1; pi1 = pi1->next) {
		...
		if (!bgp_mplsvpn_path_uses_valid_mpls_label(pi1) &&
		    !bgp_attr_get_srv6_l3service(pi1->attr) && !bgp_attr_get_srv6_vpn(pi1->attr)) {
			if (CHECK_FLAG(pi1->flags, BGP_PATH_ENCAPSULATION_SELECTED))
				bgp_path_info_unset_flag(dest, pi1,
							 BGP_PATH_ENCAPSULATION_SELECTED);
			continue;
		...
		if ((pi_mpls && pi_mpls != pi1 && bgp_mplsvpn_path_uses_valid_mpls_label(pi1)) ||
		    (pi_srv6 && pi_srv6 != pi1 &&
		     (bgp_attr_get_srv6_l3service(pi1->attr) || bgp_attr_get_srv6_vpn(pi1->attr)))) {
			if (CHECK_FLAG(pi1->flags, BGP_PATH_ENCAPSULATION_SELECTED))
				bgp_path_info_unset_flag(dest, pi1,
	...
	if (!pi_mpls && pi_tmp && !CHECK_FLAG(pi_tmp->flags, BGP_PATH_ENCAPSULATION_SELECTED))
		bgp_path_info_set_flag(dest, pi_tmp, BGP_PATH_ENCAPSULATION_SELECTED);

	if (!pi_srv6 && pi_tmp && !CHECK_FLAG(pi_tmp->flags, BGP_PATH_ENCAPSULATION_SELECTED))
		bgp_path_info_set_flag(dest, pi_tmp, BGP_PATH_ENCAPSULATION_SELECTED);

end_bgp_best_selection:
