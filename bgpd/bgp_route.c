...
#define NEXTHOP_IS_V6                                                          \
	 ...
	 *
	 * Only perform this check when route-map has modified the nexthop,
	 * to avoid affecting ECMP and other scenarios where the nexthop
	 * was not explicitly set to the peer's address by policy.
	 */
	if (bgp_rmap_nhop_changed(attr->rmap_change_flags) && !transparent &&
	    !CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED)) {
		struct peer_af *paf;
