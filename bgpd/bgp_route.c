...
#define NEXTHOP_IS_V6                                                          \
	...
	if (safi == SAFI_MPLS_VPN &&
	    CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CONFIG_ENCAPSULATION_TX_SRV6) &&
	    !pi->attr->srv6_l3vpn && !pi->attr->srv6_vpn)
		return false;

	if (safi == SAFI_MPLS_VPN &&
	    CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_CONFIG_ENCAPSULATION_TX_MPLS) &&
	    (pi->attr->srv6_l3vpn || pi->attr->srv6_vpn))
		return false;
			...
			if (pi->peer && pi->peer->bgp->peer_self != pi->peer &&
				...
				if (pi->attr->srv6_l3vpn || pi->attr->srv6_vpn) {
					if (peergroup_af_flag_check(peer, afi, SAFI_MPLS_VPN,
								    PEER_FLAG_CONFIG_ENCAPSULATION_RX_SRV6) ||
					    !peergroup_af_flag_check(peer, afi, SAFI_MPLS_VPN,
								     PEER_FLAG_CONFIG_ENCAPSULATION_RX_MPLS))
