...
static struct log_ref ferr_bgp_err[] = {
	...
	{
		.code = EC_BGP_LINKSTATE_INSTALLATION,
		.title = "BGP Link-State Installation/removal Error",
		.description = "The BGP Link-State subsystem has detected that there was a failure for installation/removal/modification of Link-State from the dataplane",
		.suggestion = "Gather log files from the router and open an issue, Restart FRR"
	},
	{
