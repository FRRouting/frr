...
DECLARE_HOOK(bgp_hook_config_write_vrf, (struct vty * vty, struct vrf *vrf), (vty, vrf));
...
DECLARE_HOOK(bgp_inst_config_write, (struct bgp * bgp, struct vty *vty), (bgp, vty));
DECLARE_HOOK(bgp_snmp_traps_config_write, (struct vty * vty), (vty));
DECLARE_HOOK(bgp_config_end, (struct bgp * bgp), (bgp));
DECLARE_HOOK(bgp_hook_vrf_update, (struct vrf * vrf, bool enabled), (vrf, enabled));
...
#define BGP_SELECT_DEFER_DISABLE(bgp) (CHECK_FLAG(bgp->flags, BGP_FLAG_SELECT_DEFER_DISABLE))
...
enum bgp_fsm_rfc_codes {
	...
	BGP_FSM_AutomaticStart_with_DampPeerOscillations_and_PassiveTcpEstablishment = 7,
...
#define PEER_THREAD_READS_ON  (1U << 1)
	...
	union sockunion su;
#define BGP_CONNECTION_SU_UNSPEC(connection) (connection->su.sa.sa_family == AF_UNSPEC)
...
#define PEER_ATTR_INHERIT(peer, group, attr) ((peer)->attr = (group)->conf->attr)
...
#define PEER_ROUTE_ADV_DELAY(peer) (CHECK_FLAG(peer->thread_flags, PEER_THREAD_SUBGRP_ADV_DELAY))
...
extern struct peer *peer_create_bind_dynamic_neighbor(struct bgp *bgp, union sockunion *su,
						      struct peer_group *group);
extern struct prefix *peer_group_lookup_dynamic_neighbor_range(struct peer_group *group,
							       struct prefix *prefix);
extern struct peer_group *peer_group_lookup_dynamic_neighbor(struct bgp *bgp, struct prefix *prefix,
							     struct prefix **listen_range);
extern struct peer *peer_lookup_dynamic_neighbor(struct bgp *bgp, union sockunion *su);
...
extern char *peer_uptime(time_t uptime2, char *buf, size_t len, bool use_json, json_object *json);
...
extern int bgp_get(struct bgp **bgp, as_t *as, const char *name, enum bgp_instance_type kind,
...
extern int bgp_handle_socket(struct bgp *bgp, struct vrf *vrf, vrf_id_t old_vrf_id, bool create);
...
extern void bgp_suppress_fib_pending_set(struct bgp *bgp, bool set, uint16_t adv_delay);
...
extern void bgp_confederation_id_set(struct bgp *bgp, as_t as, const char *as_str);
...
extern void bgp_confederation_peers_add(struct bgp *bgp, as_t as, const char *as_str);
...
extern void peer_as_change(struct peer *peer, as_t as, enum peer_asn_type as_type,
			   const char *as_str);
extern int peer_remote_as(struct bgp *bgp, union sockunion *su, const char *conf_if, as_t *as,
...
extern int peer_af_flag_set(struct peer *peer, afi_t afi, safi_t safi, uint64_t flag);
extern int peer_af_flag_unset(struct peer *peer, afi_t afi, safi_t safi, uint64_t flag);
extern bool peer_af_flag_check(struct peer *peer, afi_t afi, safi_t safi, uint64_t flag);
extern void peer_af_flag_inherit(struct peer *peer, afi_t afi, safi_t safi, uint64_t flag);
...
extern void peer_update_source_addr_set(struct peer *peer, const union sockunion *su);
extern void peer_update_source_unset(struct peer *peer);

extern int peer_default_originate_set(struct peer *peer, afi_t afi, safi_t safi, const char *rmap,
				      struct route_map *route_map);
extern int peer_default_originate_unset(struct peer *peer, afi_t afi, safi_t safi);
extern void bgp_tcp_keepalive_set(struct bgp *bgp, uint16_t idle, uint16_t interval,
				  uint16_t probes);
...
extern int peer_local_as_set(struct peer *peer, as_t as, bool no_prepend, bool replace_as,
			     bool dual_as, const char *as_str);
...
extern int peer_unsuppress_map_set(struct peer *peer, afi_t afi, safi_t safi, const char *name,
				   struct route_map *route_map);

extern int peer_advertise_map_set(struct peer *peer, afi_t afi, safi_t safi,
				  const char *advertise_name, struct route_map *advertise_map,
				  const char *condition_name, struct route_map *condition_map,
				  bool condition);
...
extern void peer_maximum_prefix_out_refresh_routes(struct peer *peer, afi_t afi, safi_t safi);
extern int peer_maximum_prefix_out_set(struct peer *peer, afi_t afi, safi_t safi, uint32_t max);
extern int peer_maximum_prefix_out_unset(struct peer *peer, afi_t afi, safi_t safi);
...
extern int bgp_map_afi_safi_iana2int(iana_afi_t pkt_afi, iana_safi_t pkt_safi, afi_t *afi,
				     safi_t *safi);
extern int bgp_map_afi_safi_int2iana(afi_t afi, safi_t safi, iana_afi_t *pkt_afi,
...
#define BGP_GR_ROUTER_DETECT_AND_SEND_CAPABILITY_TO_ZEBRA(_bgp, _peer_list)                       \
	do {                                                                                      \
		...
		if (gr_router_detected && _bgp->present_zebra_gr_state == ZEBRA_GR_DISABLE) {     \
...
static inline int peer_group_af_configured(struct peer_group *group)
{
	...
	if (peer->afc[AFI_IP][SAFI_UNICAST] || peer->afc[AFI_IP][SAFI_MULTICAST] ||
	    ...
	    peer->afc[AFI_IP][SAFI_UNREACH] || peer->afc[AFI_IP6][SAFI_UNICAST] ||
...
{
	return (peer_dynamic_neighbor(peer) && !CHECK_FLAG(peer->sflags, PEER_STATUS_NSF_WAIT));
}
...
extern struct peer *peer_lookup_in_view(struct vty *vty, struct bgp *bgp, const char *ip_str,
					bool use_json);
...
/* Hooks */
DECLARE_HOOK(bgp_vrf_status_changed, (struct bgp * bgp, struct interface *ifp), (bgp, ifp));
...
DECLARE_HOOK(bgp_snmp_update_stats, (struct bgp_dest * rn, struct bgp_path_info *pi, bool added),
	     (rn, pi, added));
...
extern void bgp_recalculate_afi_safi_bestpaths(struct bgp *bgp, afi_t afi, safi_t safi);
