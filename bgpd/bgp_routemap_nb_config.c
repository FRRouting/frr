...
/*
 * XPath:
 * /frr-route-map:lib/route-map/entry/match-condition/rmap-match-condition/frr-bgp-route-map:prefix-state
 */
int lib_route_map_entry_match_condition_rmap_match_condition_prefix_state_modify(
	struct nb_cb_modify_args *args)
{
	struct routemap_hook_context *rhc;
	const char *state;
	...
	case NB_EV_APPLY:
		/* Add configuration. */
		rhc = nb_running_get_entry(args->dnode, NULL, true);
		state = yang_dnode_get_string(args->dnode, NULL);
		...
		ret = bgp_route_match_add(rhc->rhc_rmi, "prefix-state", state,
					  RMAP_EVENT_MATCH_ADDED, args->errmsg, args->errmsg_len);
		if (ret != RMAP_COMPILE_SUCCESS) {
			rhc->rhc_mhook = NULL;
			return NB_ERR_INCONSISTENCY;
...
int lib_route_map_entry_match_condition_rmap_match_condition_prefix_state_destroy(
	struct nb_cb_destroy_args *args)
{
	switch (args->event) {
	case NB_EV_VALIDATE:
