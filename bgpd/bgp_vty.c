       ...
       "Sid value\n")
{
	...
	if ((sid_auto && bgp->vpn_policy[afi].tovpn_sid_index != 0) ||
	    ...
	    (sid_idx != 0 &&
	     CHECK_FLAG(bgp->vpn_policy[afi].flags, BGP_VPN_POLICY_TOVPN_SID_EXPLICIT)) ||
	    (sid_explicit && bgp->vpn_policy[afi].tovpn_sid_index != 0) ||
	    (sid_explicit && CHECK_FLAG(bgp->vrf_flags, BGP_VPN_POLICY_TOVPN_SID_AUTO))) {
		...
		else if (bgp->vpn_policy[afi].tovpn_sid_index != 0)
