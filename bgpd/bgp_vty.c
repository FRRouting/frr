...
FRR_CFG_DEFAULT_BOOL(BGP_CLIENT_TO_CLIENT, { .val_bool = true });

FRR_CFG_DEFAULT_BOOL(BGP_PREFER_GLOBAL_CLUSTER, { .val_bool = false });

FRR_CFG_DEFAULT_BOOL(BGP_LOOSE_CLUSTER_LIST_CHECK, { .val_bool = false });
			      ...
			      json_object *json_neigh)
{
	...
	if (use_json) {
		...
		if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_CLUSTER_ID))
			json_object_string_addf(json_addr, "clusterId", "%pI4",
						&p->cluster[afi][safi]);
