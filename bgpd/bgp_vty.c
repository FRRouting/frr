...
bool bgp_has_srv6_evpn_state(const struct bgp *bgp)
{
	...
	/* Per-EVI encapsulation: render if there are any EVIs to show or any
	 * bound SRv6 locator (VPN/L3).
	 */
	if (bgp->vnihash && hashcount(bgp->vnihash))
		return true;
				 ...
				 bool is_explicit, const char *behavior, const char *oif_ifname)
{
	...
	if (is_explicit) {
		...
		snprintfrr(modebuf, sizeof(modebuf), "index %u", idx);
		mode = modebuf;
	} else {
		mode = "-";
	}
...
void bgp_show_srv6_evpn_instance(struct vty *vty, struct bgp *bgp)
{
	...
	vty_out(vty, "  EVPN encapsulation: per-EVI (see End.DT2U/DT2M table below)\n");
	 ...
	 */
	{
		...
		if (vpws_present) {
			...
			frr_each (evpn_vpws_list, &bgp->evpn_vpws_list, vpws) {
				/* Reflect the actual advertised behavior: uDX2
				 * when this instance's locator is uSID, else
				 * End.DX2 (mirrors bgp_evpn_vpws_originate()).
				 */
				const char *dx2 =
					(vpws->sid_locator &&
					 CHECK_FLAG(vpws->sid_locator->flags, SRV6_LOCATOR_USID))
						? "uDX2"
						: "End.DX2";

				snprintfrr(label, sizeof(label), "L2 (Type-1) %s:", vpws->name);
