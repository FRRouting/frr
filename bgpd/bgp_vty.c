...
DEFPY(neighbor_addpath_tx_bestpath_per_encapsulation,
      neighbor_addpath_tx_bestpath_per_encapsulation_cmd,
      "[no] neighbor <A.B.C.D|X:X::X:X|WORD>$neighbor addpath-tx-bestpath-per-encapsulation",
      NO_STR NEIGHBOR_STR NEIGHBOR_ADDR_STR2
      "Use addpath to advertise the bestpath per each encapsulation (MPLS, SRv6)\n")
{
	...
	if (no) {
		if (peer->addpath_type[bgp_node_afi(vty)][safi] !=
		    BGP_ADDPATH_BEST_PER_ENCAPSULATION) {
			vty_out(vty,
