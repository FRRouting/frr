			      ...
			      enum bgp_instance_type inst_type)
{
	...
	if (name) {
		 ...
		 */
		THREAD_TIMER_ON(bm->master, bgp->t_startup,
				bgp_startup_timer_expire, bgp,
				bgp->restart_time);
	}
					 ...
					 struct peer *peer)
{
	...
	if (peer_group_active(peer)) {
		...
		if (g_peer->as_type == AS_UNSPECIFIED && !if_ras_printed) {
			if (peer->as_type == AS_SPECIFIED) {
				...
				vty_out(vty,
					" neighbor %s remote-as internal%s",
					addr, VTY_NEWLINE);
			} else if (peer->as_type == AS_EXTERNAL) {
				vty_out(vty,
					" neighbor %s remote-as external%s",
					addr, VTY_NEWLINE);
			}
		}
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_SHUTDOWN)) {
		if (!peer_group_active(peer)
			...
			if (peer->tx_shutdown_message)
				vty_out(vty,
					" neighbor %s shutdown message %s%s",
	...
	if (peer->gtsm_hops != 0) {
		if (!peer_group_active(peer)
			...
			vty_out(vty, " neighbor %s ttl-security hops %d%s",
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)) {
		if (!peer_group_active(peer)
			...
			vty_out(vty, " neighbor %s disable-connected-check%s",
				addr, VTY_NEWLINE);
		}
	}
	...
	if (CHECK_FLAG(peer->config, PEER_CONFIG_ROUTEADV)
		...
		vty_out(vty, " neighbor %s advertisement-interval %d%s", addr,
			peer->v_routeadv, VTY_NEWLINE);
	}
	...
	if (peer->ifp && !CHECK_FLAG(peer->flags, PEER_FLAG_CAPABILITY_ENHE)) {
		if (!peer_group_active(peer)
			...
			vty_out(vty,
				" no neighbor %s capability extended-nexthop%s",
				addr, VTY_NEWLINE);
		}
	}
	...
	if (!peer->ifp && CHECK_FLAG(peer->flags, PEER_FLAG_CAPABILITY_ENHE)) {
		if (!peer_group_active(peer)
		    || !CHECK_FLAG(g_peer->flags, PEER_FLAG_CAPABILITY_ENHE)) {
			vty_out(vty,
				" neighbor %s capability extended-nexthop%s",
				addr, VTY_NEWLINE);
		}
	}
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_DONT_CAPABILITY)) {
		if (!peer_group_active(peer)
			...
			vty_out(vty, " neighbor %s dont-capability-negotiate%s",
				addr, VTY_NEWLINE);
		}
	}
	...
	if (CHECK_FLAG(peer->flags, PEER_FLAG_STRICT_CAP_MATCH)) {
		if (!peer_group_active(peer)
			...
			vty_out(vty, " neighbor %s strict-capability-match%s",
				addr, VTY_NEWLINE);
		}
	}
}
				     ...
				     int *write)
{
	...
	if (peergroup_af_flag_check(peer, afi, safi,
				    PEER_FLAG_ADDPATH_TX_ALL_PATHS)) {
		afi_header_vty_out(vty, afi, safi, write,
				   "  neighbor %s addpath-tx-all-paths%s", addr,
				   VTY_NEWLINE);
	}
	...
	if (peergroup_af_flag_check(peer, afi, safi,
		...
		afi_header_vty_out(vty, afi, safi, write,
				   "  neighbor %s next-hop-self force%s", addr,
				   VTY_NEWLINE);
	}
	...
	if (bgp_option_check(BGP_OPT_CONFIG_CISCO)) {
		if (peergroup_af_flag_check(peer, afi, safi,
			...
			afi_header_vty_out(
				vty, afi, safi, write,
				"  neighbor %s send-community large%s", addr,
				VTY_NEWLINE);
		} else if (peergroup_af_flag_check(
				   peer, afi, safi,
				   PEER_FLAG_SEND_EXT_COMMUNITY)) {
			afi_header_vty_out(
				vty, afi, safi, write,
				"  neighbor %s send-community extended%s", addr,
				VTY_NEWLINE);
		} else if (peergroup_af_flag_check(peer, afi, safi,
						   PEER_FLAG_SEND_COMMUNITY)) {
			afi_header_vty_out(vty, afi, safi, write,
		...
		if (!peer_af_flag_check(peer, afi, safi,
			...
			afi_header_vty_out(
				vty, afi, safi, write,
				"  no neighbor %s send-community all%s", addr,
				VTY_NEWLINE);
			...
			afi_header_vty_out(
				vty, afi, safi, write,
				"  no neighbor %s send-community large%s", addr,
				VTY_NEWLINE);
		} else if (!peer_af_flag_check(peer, afi, safi,
					       PEER_FLAG_SEND_EXT_COMMUNITY)
			   && (!g_peer
		...
		if (!peer_group_active(peer)
			...
			if (peer->weight[afi][safi]) {
				afi_header_vty_out(
					...
					peer->weight[afi][safi], VTY_NEWLINE);
			}
		}
...
int bgp_config_write(struct vty *vty)
{
	...
	if (bm->rmap_update_timer != RMAP_DEFAULT_UPDATE_TIMER)
		vty_out(vty, "bgp route-map delay-timer %d%s",
			bm->rmap_update_timer, VTY_NEWLINE);
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		...
		if (bgp_flag_check(bgp, BGP_FLAG_DISABLE_NH_CONNECTED_CHK))
			vty_out(vty,
				" bgp disable-ebgp-connected-route-check%s",
				VTY_NEWLINE);
		...
		if (bgp->v_maxmed_onstartup
			...
			vty_out(vty, " bgp max-med on-startup %d",
		...
		if (bgp_flag_check(bgp, BGP_FLAG_GR_PRESERVE_FWD))
			vty_out(vty,
				" bgp graceful-restart preserve-fw-state%s",
				VTY_NEWLINE);
		...
		if (bgp_flag_check(bgp, BGP_FLAG_ASPATH_MULTIPATH_RELAX)) {
			if (bgp_flag_check(bgp,
				...
				vty_out(vty,
					" bgp bestpath as-path multipath-relax as-set%s",
					VTY_NEWLINE);
			} else {
				vty_out(vty,
					" bgp bestpath as-path multipath-relax%s",
					VTY_NEWLINE);
			}
		}
