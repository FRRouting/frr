					  ...
					  struct bgp_path_info *path)
{
	...
	if (bgp_attr_get_srv6_service(path->attr) || bgp_attr_get_srv6_vpn(path->attr)) {
			...
			struct bgp *bgp_nexthop, struct bgp_path_info *source_pi)
{
	...
	case AFI_IP:
		...
		break;
	case AFI_IP6: {
		struct bgp_attr_srv6_service *srv6_service = bgp_attr_get_srv6_service(pi->attr);

		p->family = AF_INET6;
		if (srv6_service) {
			tmp_prefix.family = AF_INET6;
			tmp_prefix.prefixlen = IPV6_MAX_BITLEN;
			tmp_prefix.prefix = srv6_service->sid;
		...
		if (local_sid == false && srv6_service) {
			p->prefixlen = IPV6_MAX_BITLEN;
			if (srv6_service->transposition_len != 0 && BGP_PATH_INFO_NUM_LABELS(pi)) {
				IPV6_ADDR_COPY(&p->u.prefix6, &srv6_service->sid);
