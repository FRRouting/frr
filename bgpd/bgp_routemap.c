						 ...
						 void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (bgp_info->attr->extra
		    && lcommunity_list_match(bgp_info->attr->extra->lcommunity,
					     list))
			return RMAP_MATCH;
	}
	return RMAP_NOMATCH;
						 ...
						 void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (ecommunity_list_match(bgp_info->attr->extra->ecommunity,
					  list))
			return RMAP_MATCH;
	}
	return RMAP_NOMATCH;
					  ...
					  route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		return ((bgp_info->attr->extra->tag == *tag) ? RMAP_MATCH
							     : RMAP_NOMATCH);
	}
						      ...
						      void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		list = community_list_lookup(bgp_clist, rule,
					     LARGE_COMMUNITY_LIST_MASTER);
		old = ((binfo->attr->extra) ? binfo->attr->extra->lcommunity
					    : NULL);
						    ...
						    void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (IPV6_ADDR_SAME(&bgp_info->attr->extra->mp_nexthop_global,
				   addr))
			return RMAP_MATCH;

		if (bgp_info->attr->extra->mp_nexthop_len
			    == BGP_ATTR_NHLEN_IPV6_GLOBAL_AND_LL
		    && IPV6_ADDR_SAME(&bgp_info->attr->extra->mp_nexthop_local,
				      rule))
			return RMAP_MATCH;
							...
							void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		/* Set next hop value. */
		(bgp_attr_extra_get(bgp_info->attr))->mp_nexthop_global =
			*address;

		/* Set nexthop length. */
		if (bgp_info->attr->extra->mp_nexthop_len == 0)
			bgp_info->attr->extra->mp_nexthop_len =
						       ...
						       void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		/* Set next hop value. */
		(bgp_attr_extra_get(bgp_info->attr))->mp_nexthop_local =
			*address;
	...
	if (type == RMAP_BGP) {
		...
		if ((CHECK_FLAG(peer->rmap_type, PEER_RMAP_TYPE_IN)
			...
			if (IN6_IS_ADDR_LINKLOCAL(&peer_address)) {
				(bgp_attr_extra_get(bgp_info->attr))
					->mp_nexthop_local = peer_address;
				if (bgp_info->attr->extra->mp_nexthop_len != 32)
					bgp_info->attr->extra->mp_nexthop_len =
						32;
			} else {
				(bgp_attr_extra_get(bgp_info->attr))
					->mp_nexthop_global = peer_address;
				if (bgp_info->attr->extra->mp_nexthop_len == 0)
					bgp_info->attr->extra->mp_nexthop_len =
						16;
			}
			...
			memset(&((bgp_attr_extra_get(bgp_info->attr))
					 ->mp_nexthop_global),
						  ...
						  void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		/* Set next hop value. */
		(bgp_attr_extra_get(bgp_info->attr))->mp_nexthop_global_in =
			*address;
						  ...
						  void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		/* Set next hop value. */
		memcpy(&(bgp_attr_extra_get(bgp_info->attr))->mp_nexthop_global,
		       address, sizeof(struct in6_addr));
		(bgp_attr_extra_get(bgp_info->attr))->mp_nexthop_len =
...
struct route_map_rule_cmd route_set_vpnv4_nexthop_cmd = {
	"ip vpn next-hop", route_set_vpnv4_nexthop,
...
static void bgp_route_map_mark_update(const char *rmap_name)
{
	if (bm->t_rmap_update == NULL) {
		...
		if (bm->rmap_update_timer) {
			bm->t_rmap_update = thread_add_timer(
       ...
       "IPv6 address of next hop\n")
{
	...
	if (IN6_IS_ADDR_UNSPECIFIED(&addr) || IN6_IS_ADDR_LOOPBACK(&addr)
		...
		vty_out(vty, "%% Invalid global nexthop address%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
