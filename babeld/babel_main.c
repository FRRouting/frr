...
int main(int argc, char **argv)
{
	...
	snprintf(state_file, sizeof(state_file), "%s/%s", frr_runstatedir, "babel-state");
...
static void babel_init_random(void)
{
	...
	if (rc < 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL, "read(random): %s", safe_strerror(errno));
		seed = 42;
	}
...
void babel_load_state_file(void)
{
	...
	if (fd < 0 && errno != ENOENT)
		flog_err_sys(EC_LIB_SYSTEM_CALL, "open(babel-state: %s)", safe_strerror(errno));
	rc = unlink(state_file);
	if (fd >= 0 && rc < 0) {
		flog_err_sys(EC_LIB_SYSTEM_CALL, "unlink(babel-state): %s", safe_strerror(errno));
		...
		if (rc < 0) {
			...
			if (rc == 3 && s >= 0 && s <= 0xFFFF) {
				...
				if (rc < 0) {
					...
					debugf(BABEL_DEBUG_COMMON, "Got %s %d %ld from babel-state.",
...
void show_babel_main_configuration(struct vty *vty)
{
	vty_out(vty,
		...
		"id                      = %s\n"
		"kernel_metric           = %d\n",
		state_file, babeld_di.config_file, format_address(protocol_group), protocol_port,
		babel_vty_addr ? babel_vty_addr : "None", babel_vty_port, format_eui64(myid),
		kernel_metric);
}
