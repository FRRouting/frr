...
static const unsigned char tlv_min_length[MESSAGE_MAX + 1] = {
	[MESSAGE_PAD1] = 0,    [MESSAGE_PADN] = 0,    [MESSAGE_ACK_REQ] = 6,	 [MESSAGE_ACK] = 2,
	[MESSAGE_HELLO] = 6,   [MESSAGE_IHU] = 6,     [MESSAGE_ROUTER_ID] = 10,	 [MESSAGE_NH] = 2,
	[MESSAGE_UPDATE] = 10, [MESSAGE_REQUEST] = 2, [MESSAGE_MH_REQUEST] = 14,
};

/* Checks whether an AE exists or must be silently ignored */
static bool known_ae(int ae)
{
	return ae <= 3;
}
...
static int network_prefix(int ae, int plen, unsigned int omitted, const unsigned char *p,
			  const unsigned char *dp, unsigned int len, unsigned char *p_r)
{
	unsigned pb;
...
static bool parse_update_subtlv(const unsigned char *a, int alen, unsigned char *channels)
{
	int type, len, i = 0;
		 ...
		 */
			...
			if (len > DIVERSITY_HOPS) {
				flog_err(EC_BABEL_PACKET,
					 "Received overlong channel information (%d > %d).n", len,
					 DIVERSITY_HOPS);
				len = DIVERSITY_HOPS;
			}
...
static int parse_hello_subtlv(const unsigned char *a, int alen, unsigned int *hello_send_us)
{
...
static int parse_ihu_subtlv(const unsigned char *a, int alen, unsigned int *hello_send_us,
...
static int parse_request_subtlv(int ae, const unsigned char *a, int alen, unsigned char *src_prefix,
	...
	while (i < alen) {
		...
		if (type == SUBTLV_PADN) {
			...
			if (have_src_prefix != 0)
				goto fail;
			rc = network_prefix(ae, a[i + 2], 0, a + i + 3, NULL, len - 1, src_prefix);
			if (rc < 0)
				goto fail;
			if (ae == 1)
				*src_plen = a[i + 2] + 96;
...
static int babel_packet_examin(const unsigned char *packet, int packetlen, int *blength)
{
	int i = 0, bodylen;
		  ...
		  int packetlen)
{
	...
	int have_router_id = 0, have_v4_prefix = 0, have_v6_prefix = 0, have_v4_nh = 0,
	    have_v6_nh = 0;
	unsigned char router_id[8], v4_prefix[16], v6_prefix[16], v4_nh[16], v6_nh[16];
	...
	if (!linklocal(from)) {
		flog_err(EC_BABEL_PACKET, "Received packet from non-local address %s.",
			 format_address(from));
		return;
	}

	if (babel_packet_examin(packet, packetlen, &bodylen)) {
		flog_err(EC_BABEL_PACKET, "Received malformed packet on %s from %s.", ifp->name,
			 format_address(from));
		return;
	}
...
#define BABEL_UNICAST_HELLO 0x8000
		 ...
		 */
			...
			if (message[5] == 0 || (message[2] == 1 ? have_v4_prefix : have_v6_prefix))
				rc = network_prefix(message[2], message[4], message[5], message + 12,
			...
			if (!ignore_update)
				update_route(router_id, prefix, plen, seqno, metric, interval,
					     neigh, nh, channels, channels_len(channels));
			...
			if (!known_ae(message[2])) {
				debugf(BABEL_DEBUG_COMMON,
				       "Received request with unknown AE %d. Ignoring.", message[2]);
				goto done;
			}
			...
			if (message[6] == 0) {
				debugf(BABEL_DEBUG_COMMON,
				       "Received seqno request with invalid hop count 0");
				goto done;
			}
			rc = parse_request_subtlv(message[2], message + 4 + rc, len - 2 - rc,
						  src_prefix, &src_plen);
			if (rc < 0)
				goto done;
			...
			int rc;
			DO_NTOHS(seqno, message + 4);
			rc = network_prefix(message[2], message[3], 0, message + 16, NULL, len - 14,
					    prefix);
			if (rc <= 0)
				goto fail;
			plen = message[3] + (message[2] == 1 ? 96 : 0);
...
fail:
		flog_err(EC_BABEL_PACKET, "Couldn't parse packet (%d, %d) from %s on %s.",
	...
	if (have_hello_rtt && hello_send_us && hello_rtt_receive_time) {
		...
		local_waiting_us = time_us(neigh->hello_rtt_receive_time) - hello_send_us;
...
{
	babel_interface_nfo *babel_ifp = babel_get_if_nfo(ifp);
	if (CHECK_FLAG(babel_ifp->flags, BABEL_IF_TIMESTAMPS) && (babel_ifp->buffered_hello >= 0)) {
...
void flushbuf(struct interface *ifp)
{
	...
	if (babel_ifp->buffered > 0) {
		...
		if (check_bucket(ifp)) {
			...
			flog_err(EC_BABEL_PACKET, "Bucket full, dropping packet to %s.", ifp->name);
		}
	}
...
static void schedule_unicast_flush(unsigned msecs)
{
	...
	unicast_flush_timeout.tv_sec = babel_now.tv_sec + (babel_now.tv_usec / 1000 + msecs) / 1000;
}
...
static void accumulate_bytes(struct interface *ifp, const unsigned char *value, unsigned len)
{
	babel_interface_nfo *babel_ifp = babel_get_if_nfo(ifp);
...
static int start_unicast_message(struct neighbour *neigh, int type, int len)
{
	...
	if (!unicast_buffer) {
		flog_err(EC_BABEL_MEMORY, "malloc(unicast_buffer): %s", safe_strerror(errno));
		return -1;
	}
...
static void accumulate_unicast_bytes(struct neighbour *neigh, const unsigned char *value,
				     unsigned len)
{
...
void send_hello_noupdate(struct interface *ifp, unsigned interval)
{
	...
	debugf(BABEL_DEBUG_COMMON, "Sending hello %d (%d) to %s.", babel_ifp->hello_seqno, interval,
	       ifp->name);
	...
	end_message(ifp, MESSAGE_HELLO, CHECK_FLAG(babel_ifp->flags, BABEL_IF_TIMESTAMPS) ? 12 : 6);
}
...
void flush_unicast(int dofree)
{
	...
	if (check_bucket(unicast_neighbour->ifp)) {
		...
		rc = babel_send(protocol_socket, packet_header, sizeof(packet_header),
				unicast_buffer, unicast_buffered, (struct sockaddr *)&sin6,
				sizeof(sin6));
		if (rc < 0)
			flog_err(EC_BABEL_PACKET, "send(unicast): %s", safe_strerror(errno));
	} else {
		flog_err(EC_BABEL_PACKET, "Bucket full, dropping unicast packet to %s if %s.",
			 format_address(unicast_neighbour->address), unicast_neighbour->ifp->name);
	}
...
static void really_send_update(struct interface *ifp, const unsigned char *id,
			       const unsigned char *prefix, unsigned char plen, unsigned short seqno,
			       unsigned short metric, unsigned char *channels, int channels_len)
{
	babel_interface_nfo *babel_ifp = babel_get_if_nfo(ifp);
	...
	if (v4) {
		...
		if (babel_ifp->have_buffered_prefix) {
			while (omit < plen / 8 && babel_ifp->buffered_prefix[omit] == prefix[omit])
				omit++;
		}
	...
	start_message(ifp, MESSAGE_UPDATE, 10 + (real_plen + 7) / 8 - omit + channels_size);
	...
	end_message(ifp, MESSAGE_UPDATE, 10 + (real_plen + 7) / 8 - omit + channels_size);
	...
	if (babel_ifp->num_buffered_updates > 0) {
		...
		for (i = 0; i < n; i++) {
			...
			if (xroute && (!route || xroute->metric <= kernel_metric)) {
				really_send_update(ifp, myid, xroute->prefix, xroute->plen, myseqno,
				...
				seqno = route->seqno;
				metric = route_interferes(route, ifp)
						 ? route_metric(route)
						 : route_metric_noninterfering(route);

				if (metric < INFINITY)
					satisfy_request(route->src->prefix, route->src->plen, seqno,
							route->src->id, ifp);
				...
				really_send_update(ifp, route->src->id, route->src->prefix,
               ...
               after an xroute has been retracted, so send a retraction. */
				really_send_update(ifp, myid, b[i].prefix, b[i].plen, myseqno,
						   INFINITY, NULL, -1);
			}
		}
...
static void buffer_update(struct interface *ifp, const unsigned char *prefix, unsigned char plen)
{
	babel_interface_nfo *babel_ifp = babel_get_if_nfo(ifp);
	if (babel_ifp->num_buffered_updates > 0 &&
...
again:
	...
	memcpy(babel_ifp->buffered_updates[babel_ifp->num_buffered_updates].prefix, prefix, 16);
	babel_ifp->buffered_updates[babel_ifp->num_buffered_updates].plen = plen;
	babel_ifp->num_buffered_updates++;
}

void send_update(struct interface *ifp, int urgent, const unsigned char *prefix, unsigned char plen)
{
	babel_interface_nfo *babel_ifp = NULL;
...
void send_ihu(struct neighbour *neigh, struct interface *ifp)
{
	...
	if (CHECK_FLAG(babel_ifp->flags, BABEL_IF_TIMESTAMPS) &&
	    ...
	    && timeval_minus_msec(&babel_now, &neigh->hello_rtt_receive_time) < 1000000) {
		send_rtt_data = 1;
	} else {
	...
	if (unicast_neighbour != neigh) {
		...
		if (send_rtt_data) {
			...
			accumulate_unicast_int(neigh, time_us(neigh->hello_rtt_receive_time));
		}
...
void send_request(struct interface *ifp, const unsigned char *prefix, unsigned char plen)
{
	int v4, pb, len;
...
void send_unicast_request(struct neighbour *neigh, const unsigned char *prefix, unsigned char plen)
{
	int rc, v4, pb, len;
...
void send_multihop_request(struct interface *ifp, const unsigned char *prefix, unsigned char plen,
			   unsigned short seqno, const unsigned char *id, unsigned short hop_count)
{
	int v4, pb, len;
...
void send_unicast_multihop_request(struct neighbour *neigh, const unsigned char *prefix,
...
void send_request_resend(struct neighbour *neigh, const unsigned char *prefix, unsigned char plen,
			 unsigned short seqno, unsigned char *id)
{
	...
	record_resend(RESEND_REQUEST, prefix, plen, seqno, id, neigh ? neigh->ifp : NULL,
		      resend_delay);
}

void handle_request(struct neighbour *neigh, const unsigned char *prefix, unsigned char plen,
		    unsigned char hop_count, unsigned short seqno, const unsigned char *id)
{
	struct xroute *xroute;
	struct babel_route *route;
	...
	if (route && memcmp(id, route->src->id, 8) == 0 && seqno_minus(seqno, route->seqno) > 100) {
	...
	send_unicast_multihop_request(successor, prefix, plen, seqno, id, hop_count - 1);
	record_resend(RESEND_REQUEST, prefix, plen, seqno, id, neigh->ifp, 0);
}
