...
static int parse_request_subtlv(int ae, const unsigned char *a, int alen,
	...
	if (babel_ifp->num_buffered_updates > 0) {
		...
		for (i = 0; i < n; i++) {
			...
			if (xroute && (!route || xroute->metric <= kernel_metric)) {
				really_send_update(ifp, myid, xroute->prefix, xroute->plen,
						   myseqno, xroute->metric, NULL, 0);
				...
				really_send_update(ifp, route->src->id, route->src->prefix,
						   route->src->plen, seqno, metric, channels,
						   chlen);
