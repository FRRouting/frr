...
static int main_imsg_send_config(struct ldpd_conf *ldp_conf, bool use_l2vpn_lib);
...
static void ldp_config_reset_l2vpns(struct l2vpn_head *conf);
...
static void merge_l2vpns(struct l2vpn_head *conf, struct l2vpn_head *xconf);
...
main(int argc, char *argv[])
{
	...
	main_imsg_send_config(ldpd_conf, true);
...
static int main_imsg_send_config(struct ldpd_conf *xconf, bool use_l2vpn_lib)
{
	struct iface		*iface;
	struct tnbr		*tnbr;
	...
	if (use_l2vpn_lib)
		l2vpn_tree = &l2vpn_tree_config;
	else
		l2vpn_tree = &xconf->l2vpn_tree;
...
ldp_config_apply(struct vty *vty, struct ldpd_conf *xconf)
{
	...
	if (main_imsg_send_config(xconf, true) == -1)
		return (-1);
...
static void ldp_config_reset_l2vpns(struct l2vpn_head *conf_l2vpn_tree)
{
	struct l2vpn		*l2vpn;
	struct l2vpn_if		*lif;
	struct l2vpn_pw		*pw;
...
static void merge_l2vpns(struct l2vpn_head *dst, struct l2vpn_head *src)
{
