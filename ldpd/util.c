...
void ldp_applymask(int af, union g_addr *dest, const union g_addr *src, int prefixlen)
{
	...
	switch (af) {
	case AF_INET:
		dest->ipv4.s_addr = src->ipv4.s_addr & prefixlen2mask(prefixlen);
		break;
	case AF_INET6:
		memset(&mask, 0, sizeof(mask));
		for (i = 0; i < prefixlen / 8; i++)
		...
		for (i = 0; i < 16; i++)
			dest->ipv6.s6_addr[i] = src->ipv6.s6_addr[i] & mask.s6_addr[i];
		break;
	default:
...
int ldp_addrcmp(int af, const union g_addr *a, const union g_addr *b)
{
	...
	case AF_INET:
		if (a->ipv4.s_addr == b->ipv4.s_addr)
			return (0);
		return ((ntohl(a->ipv4.s_addr) > ntohl(b->ipv4.s_addr)) ? 1 : -1);
	case AF_INET6:
		return (memcmp(&a->ipv6, &b->ipv6, sizeof(struct in6_addr)));
	default:
...
int ldp_addrisset(int af, const union g_addr *addr)
{
	...
	case AF_UNSPEC:
		return (0);
	case AF_INET:
		if (addr->ipv4.s_addr != INADDR_ANY)
			return (1);
		break;
	case AF_INET6:
		if (!IN6_IS_ADDR_UNSPECIFIED(&addr->ipv6))
			return (1);
		break;
	default:
...
int ldp_prefixcmp(int af, const union g_addr *a, const union g_addr *b, uint8_t prefixlen)
{
	...
	case AF_INET:
		...
		aa = htonl(a->ipv4.s_addr) & mask;
		ba = htonl(b->ipv4.s_addr) & mask;
		return (aa - ba);
	case AF_INET6:
		if (prefixlen == 0)
			return (0);
		...
		for (i = 0; i < prefixlen / 8; i++)
			if (a->ipv6.s6_addr[i] != b->ipv6.s6_addr[i])
				return (a->ipv6.s6_addr[i] - b->ipv6.s6_addr[i]);
		i = prefixlen % 8;
		if (i) {
			m = 0xff00 >> i;
			if ((a->ipv6.s6_addr[prefixlen / 8] & m) !=
			    (b->ipv6.s6_addr[prefixlen / 8] & m))
				return ((a->ipv6.s6_addr[prefixlen / 8] & m) -
					(b->ipv6.s6_addr[prefixlen / 8] & m));
		}
		return (0);
	default:
...
int bad_addr(int af, union g_addr *addr)
{
	switch (af) {
	case AF_INET:
		return (bad_addr_v4(addr->ipv4));
	case AF_INET6:
		return (bad_addr_v6(&addr->ipv6));
	default:
