...
static void nhrp_peer_defer_vici_request(struct event *t)
{
	...
	if (p->online) {
		dbg(NHRP_COMMON, "IPsec connection to %pSU already established", &vc->remote.nbma);
	} else {
...
void nhrp_peer_send(struct nhrp_peer *p, struct zbuf *zb)
{
	...
	dbg(NHRP_KERNEL, "PACKET: Send %pSU -> %pSU", &p->vc->local.nbma, &p->vc->remote.nbma);
	...
	if (extensions) {
		...
		while ((ext = nhrp_ext_pull(extensions, &payload)) != NULL) {
			...
			case NHRP_EXTENSION_NAT_ADDRESS:
				 ...
				 */
				dbg(NHRP_COMMON,
				    "shortcut res_resp: Processing NAT Extension for %pSU", proto);
				while (nhrp_cie_pull(&payload, pp->hdr,
					...
					if (!sockunion_cmp(proto, &cie_proto)) {
						dbg(NHRP_COMMON, "cie_nbma for proto %pSU is %pSU",
						    proto, cie_nbma);
						break;
					}
				}
...
static void nhrp_handle_resolution_req(struct nhrp_packet_parser *pp)
{
	...
	while ((cie = nhrp_cie_pull(&pp->payload, pp->hdr, &cie_nbma,
		...
		dbg(NHRP_COMMON, "shortcut res_rep: parsing CIE with prefixlen=%u", prefix_len);
		...
		if (sockunion_family(&cie_nbma_nat) == AF_UNSPEC) {
			 ...
			 */
			dbg(NHRP_COMMON, "shortcut res_rep: No NAT Extension for %pSU", proto_addr);
		...
		dbg(NHRP_COMMON, "shortcut res_rep: holdtime is %u (if 0, using %u)", holdtime,
		    pp->if_ad->holdtime);
		if (!holdtime)
		...
		dbg(NHRP_COMMON, "shortcut res_rep: updating binding for nmba addr %pSU",
		    nbma_addr);
			       ...
			       struct zbuf *pkt)
{
	...
	if (!(if_ad->flags & NHRP_IFF_REDIRECT)) {
		dbg(NHRP_COMMON, "Send Traffic Indication to %pSU about packet to %pSU ignored",
		    &p->vc->remote.nbma, &dst);
		return;
	}

	dbg(NHRP_COMMON, "Send Traffic Indication to %pSU (online=%d) about packet to %pSU",
...
static void nhrp_handle_error_ind(struct nhrp_packet_parser *pp)
{
	...
	if (!hdr) {
		dbg(NHRP_COMMON, "Truncated Error Indication packet");
		return;
	}

	dbg(NHRP_COMMON, "Error Indication from %pSU about packet to %pSU ignored", &pp->src_proto,
	    &dst_proto);
...
static void nhrp_handle_traffic_ind(struct nhrp_packet_parser *p)
{
	...
	dbg(NHRP_COMMON, "Traffic Indication from %pSU about packet to %pSU: %s", &p->src_proto,
	    &dst, (p->if_ad->flags & NHRP_IFF_SHORTCUT) ? "trying shortcut" : "ignored");
			      ...
			      struct nhrp_packet_parser *pp)
{
	...
	while ((ext = nhrp_ext_pull(&pp->extensions, &extpl)) != NULL) {
		...
		case NHRP_EXTENSION_NAT_ADDRESS:
			...
			if (c) {
				dbg(NHRP_COMMON, "c->cur.remote_nbma_natoa is %pSU",
				...
				if (proto)
					dbg(NHRP_COMMON, "No cache entry for proto %pSU", proto);
...
static void nhrp_packet_debug(struct zbuf *zb, const char *dir)
{
	...
	if (!hdr) {
		dbg(NHRP_COMMON, "%s Truncated packet", dir);
		return;
	}
	if (hdr->type > NHRP_PACKET_MAX) {
		dbg(NHRP_COMMON, "%s Unknown(%u) %pSU -> %pSU", dir, hdr->type, &src_proto,
		    &dst_proto);
		return;
	}
	...
	dbg(NHRP_COMMON, "%s %s(%d) %pSU -> %pSU", dir, name, hdr->type,
	    reply ? &dst_proto : &src_proto, reply ? &src_proto : &dst_proto);
}
...
void nhrp_peer_recv(struct nhrp_peer *p, struct zbuf *zb)
{
	...
	dbg(NHRP_KERNEL, "PACKET: Recv %pSU -> %pSU", &vc->remote.nbma, &vc->local.nbma);
