...
static void nhrp_cache_authorize_binding(struct nhrp_reqid *r, void *arg)
{
	...
	dbg(NHRP_COMMON, "cache: %s %pSU: %s", c->ifp->name, &c->remote_addr, (const char *)arg);
	...
	if (arg && strcmp(arg, "accept") == 0) {
		...
		if (sockunion_family(&c->cur.remote_nbma_natoa) != AF_UNSPEC) {
			dbg(NHRP_COMMON,
			    ...
			    (c->cur.peer ? sockunion2str(&c->cur.peer->vc->remote.nbma, buf[1],
							 sizeof(buf[1]))
					 : "(no peer)"),
			    &c->cur.remote_nbma_natoa);
			      ...
			      union sockunion *nbma_claimed)
{
	...
	if (c->cur.type == type && c->cur.peer == p && c->cur.mtu == mtu) {
		...
		dbg(NHRP_COMMON,
		    ...
		    c->cur.type, type, (c->cur.peer == p) ? "same" : "diff", c->cur.mtu, mtu,
		    buf[0], nbma_oa ? buf[1] : "(NULL)", c->map);
		c->new.type = type;
		c->new.peer = p;
		c->new.mtu = mtu;
