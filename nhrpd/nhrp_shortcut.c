				       ...
				       unsigned long cmd)
{
	...
	case NOTIFY_CACHE_UP:
		if (!s->route_installed) {
			dbg(NHRP_ROUTE, "Shortcut: route install %pFX nh %pSU dev %s", s->p,
					 ...
					 struct nhrp_cache *c, int holding_time)
{
	...
	if (c != s->cache) {
		...
		if (!s->cache || !s->cache->route_installed) {
			dbg(NHRP_ROUTE, "Shortcut: notify cache down because cache?%s or ri?%s",
			    s->cache ? "yes" : "no",
			    s->cache ? (s->cache->route_installed ? "yes" : "no") : "n/a");
			nhrp_shortcut_cache_notify(&s->cache_notifier,
						   NOTIFY_CACHE_DOWN);
		}
	}
					      ...
					      void *arg)
{
	...
	if (pp->hdr->type != NHRP_PACKET_RESOLUTION_REPLY) {
		if (pp->hdr->type == NHRP_PACKET_ERROR_INDICATION
			...
			dbg(NHRP_COMMON, "Shortcut: Resolution: Protocol address unreachable");
	...
	if (!sockunion_same(&cie_proto, &pp->dst_proto)) {
		dbg(NHRP_COMMON, "Shortcut: Warning dst_proto altered from %pSU to %pSU",
	...
	dbg(NHRP_COMMON, "Shortcut: %pFX is at proto %pSU dst_proto %pSU NBMA %pSU cie-holdtime %d",
	    &prefix, proto, &pp->dst_proto, nbma, htons(cie->holding_time));

	if (sockunion_family(nbma)) {
		c = nhrp_cache_get(pp->ifp, proto, 1);
		if (c) {
			dbg(NHRP_COMMON, "Shortcut: cache found, update binding");
			...
			dbg(NHRP_COMMON, "Shortcut: no cache for proto %pSU", proto);
		}
	...
	if (c) {
		...
		dbg(NHRP_COMMON, "NO Shortcut because c NULL?%s or same proto?%s", c ? "no" : "yes",
		    proto && pp && sockunion_same(proto, &pp->dst_proto) ? "yes" : "no");
	}
					      ...
					      bool retry)
{
	...
	dbg(NHRP_COMMON, "Shortcut res_req: set cie ht to %u and mtu to %u. shortcut ht is %u",
...
static void nhrp_shortcut_retry_resolution_req(struct event *t)
{
	...
	event_cancel(&s->t_retry_resolution);
	dbg(NHRP_COMMON, "Shortcut: Retrying Resolution Request");
