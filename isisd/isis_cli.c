		...
		"Flexible Algorithm Number\n")
{
	...
	snprintf(xpath, sizeof(xpath), "%s/flex-algos/flex-algo[flex-algo='%lld']", VTY_CURR_XPATH,
		 algorithm);

	nb_cli_enqueue_change(vty, ".", NB_OP_CREATE, NULL);

	ret = nb_cli_apply_changes(vty, "./flex-algos/flex-algo[flex-algo='%lld']", algorithm);
	if (ret == CMD_SUCCESS)
	   ...
	   "Flexible Algorithm Number\n")
{
	...
	if (!yang_dnode_exists(vty->candidate_config->dnode, xpath)) {
		vty_out(vty, "ISIS flex-algo %lld isn't exist.\n", algorithm);
		return CMD_ERR_NO_MATCH;
	}

	nb_cli_enqueue_change(vty, ".", NB_OP_DESTROY, NULL);
	return nb_cli_apply_changes_clear_pending(vty, "./flex-algos/flex-algo[flex-algo='%lld']",
						  algorithm);
}
