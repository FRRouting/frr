...
struct isis_area *isis_area_create(const char *area_tag, const char *vrf_name)
{
	...
	for (int level = 0; level < 2; level++)
		area->leanking_list[level] = list_new();

	area->leanking_settings = list_new();
...
void isis_area_destroy(struct isis_area *area)
{
	...
	for (int level = 0; level < 2; level++)
		list_delete(&area->leanking_list[level]);

	list_delete(&area->leanking_settings);
...
static int isis_config_write(struct vty *vty)
{
	...
	for (ALL_LIST_ELEMENTS_RO(im->isis, inode, isis)) {
		for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
			...
			write += isis_leanking_config_write(vty, area, AF_INET);
			write += isis_leanking_config_write(vty, area, AF_INET6);
