...
int show_isis_interface_common(struct vty *vty, const char *ifname, char detail)
{
	...
	if (!isis) {
		vty_out(vty, "IS-IS Routing Process not enabled%s",
			VTY_NEWLINE);
		return CMD_SUCCESS;
	}
...
int show_isis_neighbor_common(struct vty *vty, const char *id, char detail)
{
	...
	if (id) {
		if (sysid2buff(sysid, id) == 0) {
			...
			if (dynhn == NULL) {
				vty_out(vty, "Invalid system id %s%s", id,
					VTY_NEWLINE);
				return CMD_SUCCESS;
			}
...
void print_debug(struct vty *vty, int flags, int onoff)
{
	...
	if (flags & DEBUG_ADJ_PACKETS)
		vty_out(vty,
			"IS-IS Adjacency related packets debugging is %s%s",
			onoffs, VTY_NEWLINE);
	...
	if (flags & DEBUG_SNP_PACKETS)
		vty_out(vty, "IS-IS CSNP/PSNP packets debugging is %s%s",
			onoffs, VTY_NEWLINE);
	...
	if (flags & DEBUG_SPF_TRIGGERS)
		vty_out(vty, "IS-IS SPF triggering events debugging is %s%s",
			onoffs, VTY_NEWLINE);
	if (flags & DEBUG_UPDATE_PACKETS)
		vty_out(vty, "IS-IS Update related packet debugging is %s%s",
			onoffs, VTY_NEWLINE);
	...
	if (flags & DEBUG_PACKET_DUMP)
		vty_out(vty, "IS-IS Packet dump debugging is %s%s", onoffs,
			VTY_NEWLINE);
	if (flags & DEBUG_LSP_GEN)
		vty_out(vty, "IS-IS LSP generation debugging is %s%s", onoffs,
			VTY_NEWLINE);
	if (flags & DEBUG_LSP_SCHED)
		vty_out(vty, "IS-IS LSP scheduling debugging is %s%s", onoffs,
			VTY_NEWLINE);
}
	...
	for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		...
		for (int level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			if (area->spf_delay_ietf[level - 1]) {
				vty_out(vty,
					"    Using draft-ietf-rtgwg-backoff-algo-04%s",
					VTY_NEWLINE);
				spf_backoff_show(
					area->spf_delay_ietf[level - 1], vty,
					"    ");
			} else {
				vty_out(vty, "    Using legacy backoff algo%s",
					VTY_NEWLINE);
			}
		}
	}
       ...
       SHOW_STR "IS-IS information\n" "IS-IS summary\n")
{
	...
	for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		...
		for (level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			vty_out(vty, "    IPv4 route computation:%s",
				VTY_NEWLINE);
			...
			vty_out(vty, "    IPv6 route computation:%s",
				VTY_NEWLINE);
...
int isis_config_write(struct vty *vty)
{
	...
	if (isis != NULL) {
		...
		for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
			/* ISIS - Area name */
			vty_out(vty, "router isis %s%s", area->area_tag,
				VTY_NEWLINE);
			write++;
			/* ISIS - Net */
			if (listcount(area->area_addrs) > 0) {
			 ...
			 * false. */
			if (!area->dynhostname) {
				vty_out(vty, " no hostname dynamic%s",
					VTY_NEWLINE);
				write++;
			}
			...
			if (area->newmetric) {
				if (!area->oldmetric)
					vty_out(vty, " metric-style wide%s",
						VTY_NEWLINE);
				else
					vty_out(vty,
						" metric-style transition%s",
						VTY_NEWLINE);
				write++;
			} else {
				vty_out(vty, " metric-style narrow%s",
					VTY_NEWLINE);
				write++;
			}
			/* ISIS - overload-bit */
			if (area->overload_bit) {
				vty_out(vty, " set-overload-bit%s",
					VTY_NEWLINE);
				write++;
			}
			...
			if (area->is_type == IS_LEVEL_1) {
				...
				vty_out(vty, " is-type level-2-only%s",
					VTY_NEWLINE);
				write++;
			}
			...
			if (area->max_lsp_lifetime[0]
				...
				if (area->max_lsp_lifetime[0]
					...
					vty_out(vty, " max-lsp-lifetime %u%s",
			...
			if (area->lsp_mtu != DEFAULT_LSP_MTU) {
				vty_out(vty, " lsp-mtu %u%s", area->lsp_mtu,
					VTY_NEWLINE);
				write++;
			}
