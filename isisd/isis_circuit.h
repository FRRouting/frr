...
void isis_circuit_print_json(struct isis_circuit *circuit, struct json_object *json, char detail);
...
ferr_r isis_circuit_metric_set(struct isis_circuit *circuit, int level, int metric);
...
#ifdef FABRICD
DECLARE_HOOK(isis_circuit_config_write, (struct isis_circuit * circuit, struct vty *vty),
	     (circuit, vty));
#endif
