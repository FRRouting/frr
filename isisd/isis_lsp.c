...
void lsp_print(struct isis_lsp *lsp, struct vty *vty, char dynhost)
{
	...
	vty_out(vty, "%s%s", lsp_bits2string(&lsp->lsp_header->lsp_bits),
		VTY_NEWLINE);
}
...
void lsp_print_detail(struct isis_lsp *lsp, struct vty *vty, char dynhost)
{
	...
	if (lsp->tlv_data.ipv6_reachs)
		for (ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv6_reachs, lnode,
					  ipv6_reach)) {
...
void lsp_te_tlv_fit(struct isis_lsp *lsp, struct list **from, struct list **to,
		    int frag_thold)
{
	int count, size = 0;
	struct listnode *node, *nextnode;
	...
	if (!FRAG_NEEDED(lsp->pdu, frag_thold, size)) {
		...
		elem = (struct te_is_neigh *)listgetdata(
			(struct listnode *)listhead(*from));
		count = count - elem->sub_tlvs_length - IS_NEIGHBOURS_LEN;
		if (count > 0) {
			while (count > 0) {
				listnode_add(
					*to,
					listgetdata((struct listnode *)listhead(
						*from)));
				listnode_delete(
					*from,
					listgetdata((struct listnode *)listhead(
				...
				elem = (struct te_is_neigh *)listgetdata(
					(struct listnode *)listhead(*from));
				count = count - elem->sub_tlvs_length
					- IS_NEIGHBOURS_LEN;
			}
...
static void lsp_build(struct isis_lsp *lsp, struct isis_area *area)
{
				...
				if (area->newmetric) {
					...
					if (!memcmp(te_is_neigh->neigh_id,
						...
						listnode_add(
							...
							te_is_neigh);
						lsp_debug(
							"ISIS (%s): Adding DIS %s.%02x as te-style neighbor",
							area->area_tag,
							sysid_print(
								te_is_neigh
									->neigh_id),
			...
			if (nei && (level & nei->circuit_t)) {
				...
				if (area->newmetric) {
					...
					listnode_add(tlv_data.te_is_neighs,
						     te_is_neigh);
					lsp_debug(
						"ISIS (%s): Adding te-style is reach for %s",
						area->area_tag,
						sysid_print(
	...
	/* FIXME: We pass maximum te_ipv4_reachability length to the
	 * lsp_tlv_fit()
	...
	while (tlv_data.te_ipv4_reachs && listcount(tlv_data.te_ipv4_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.te_ipv4_reachs,
			    &lsp->tlv_data.te_ipv4_reachs, TE_IPV4_REACH_LEN,
	...
	while (tlv_data.ipv6_reachs && listcount(tlv_data.ipv6_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.ipv6_reachs,
			    &lsp->tlv_data.ipv6_reachs, IPV6_REACH_LEN,
