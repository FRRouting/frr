				    ...
				    struct isis_extended_reach *reach)
{
	...
	if (link_admin_group && link_ext_admin_group) {
		...
		if (*link_admin_group != link_ext_admin_group_bitmap0) {
			...
			zlog_warn(
				"ISIS-SPF: LSP from %s neighbor %s. Admin-group 0x%08x differs from ext admin-group 0x%08x.",
				rawlspid_print(lspid_orig),
				rawlspid_print(lspid_neigh), *link_admin_group,
				link_ext_admin_group_bitmap0);
		}
	}
