...
static u_char show_vty_subtlv_llri(struct vty *vty, struct te_subtlv_llri *tlv)
{
	if (vty != NULL) {
		vty_out(vty, "    Link Local  ID: %d%s",
			(u_int32_t)ntohl(tlv->local), VTY_NEWLINE);
		vty_out(vty, "    Link Remote ID: %d%s",
			(u_int32_t)ntohl(tlv->remote), VTY_NEWLINE);
	} else {
...
{
	if (vty != NULL)
		vty_out(vty, "    Local Interface IP Address(es): %s%s",
			inet_ntoa(tlv->value), VTY_NEWLINE);
	else
...
{
	if (vty != NULL)
		vty_out(vty, "    Remote Interface IP Address(es): %s%s",
			inet_ntoa(tlv->value), VTY_NEWLINE);
	else
					...
					struct te_subtlv_te_metric *tlv)
{
	...
	if (vty != NULL)
		vty_out(vty, "    Traffic Engineering Metric: %u%s", te_metric,
			VTY_NEWLINE);
	else
				       ...
				       struct te_subtlv_av_delay *tlv)
{
	...
	if (vty != NULL)
		vty_out(vty, "    %s Average Link Delay: %d (micro-sec)%s",
			A ? "Anomalous" : "Normal", delay, VTY_NEWLINE);
	else
				       ...
				       struct te_subtlv_mm_delay *tlv)
{
	...
	if (vty != NULL)
		vty_out(vty, "    %s Min/Max Link Delay: %d / %d (micro-sec)%s",
			A ? "Anomalous" : "Normal", low, high, VTY_NEWLINE);
	else
				       ...
				       struct te_subtlv_pkt_loss *tlv)
{
	...
	if (vty != NULL)
		vty_out(vty, "    %s Link Packet Loss: %g (%%)%s",
			A ? "Anomalous" : "Normal", fval, VTY_NEWLINE);
	else
       ...
       "Router information\n")
{
	if (IS_MPLS_TE(isisMplsTE)) {
		...
		if (ntohs(isisMplsTE.router_id.s_addr) != 0)
			vty_out(vty, "  Router-Address: %s%s",
				inet_ntoa(isisMplsTE.router_id), VTY_NEWLINE);
		else
			vty_out(vty, "  N/A%s", VTY_NEWLINE);
	} else
		vty_out(vty, "  MPLS-TE is disable on this router%s",
			VTY_NEWLINE);
