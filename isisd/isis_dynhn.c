...
void dyn_cache_init(void)
{
	if (dyn_cache == NULL)
		dyn_cache = list_new();
	THREAD_TIMER_ON(master, isis->t_dync_clean, dyn_cache_cleanup, NULL,
			120);
	return;
}
...
void dynhn_print_all(struct vty *vty)
{
	...
	for (ALL_LIST_ELEMENTS_RO(dyn_cache, node, dyn)) {
		...
		vty_out(vty, "%-15s%-15s%s", sysid_print(dyn->id),
			dyn->name.name, VTY_NEWLINE);
	}

	vty_out(vty, "     * %s %s%s", sysid_print(isis->sysid),
		unix_hostname(), VTY_NEWLINE);
	return;
}
