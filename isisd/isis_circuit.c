...
int isis_circuit_up(struct isis_circuit *circuit)
{
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		if (circuit->is_type & IS_LEVEL_1)
			THREAD_TIMER_ON(master, circuit->u.bc.t_run_dr[0],
					isis_run_dr_l1, circuit,
					2 * circuit->hello_interval[0]);
		if (circuit->is_type & IS_LEVEL_2)
			THREAD_TIMER_ON(master, circuit->u.bc.t_run_dr[1],
					isis_run_dr_l2, circuit,
	...
	if (circuit->is_type & IS_LEVEL_1)
		THREAD_TIMER_ON(
			master, circuit->t_send_psnp[0], send_l1_psnp, circuit,
			isis_jitter(circuit->psnp_interval[0], PSNP_JITTER));

	if (circuit->is_type & IS_LEVEL_2)
		THREAD_TIMER_ON(
			master, circuit->t_send_psnp[1], send_l2_psnp, circuit,
	...
	for (ALL_LIST_ELEMENTS_RO(vrf_iflist(VRF_DEFAULT), node, ifp)) {
		...
		for (ALL_LIST_ELEMENTS_RO(isis->area_list, node2, area)) {
			...
			if (circuit == NULL)
				continue;
			if (circuit->ip_router) {
				vty_out(vty, " ip router isis %s%s",
			...
			if (circuit->circ_type_config == CIRCUIT_T_P2P) {
				vty_out(vty, " isis network point-to-point%s",
					VTY_NEWLINE);
				write++;
			}
			if (circuit->ipv6_router) {
				vty_out(vty, " ipv6 router isis %s%s",
					area->area_tag, VTY_NEWLINE);
				write++;
			}
			...
			/* ISIS - circuit type */
			if (circuit->is_type == IS_LEVEL_1) {
				vty_out(vty, " isis circuit-type level-1%s",
					VTY_NEWLINE);
				write++;
			} else {
				if (circuit->is_type == IS_LEVEL_2) {
			...
			if (circuit->pad_hellos == 0) {
				vty_out(vty, " no isis hello padding%s",
					VTY_NEWLINE);
				write++;
			}
			...
			if (circuit->passwd.type == ISIS_PASSWD_TYPE_HMAC_MD5) {
				vty_out(vty, " isis password md5 %s%s",
					circuit->passwd.passwd, VTY_NEWLINE);
				write++;
			} else if (circuit->passwd.type
				   == ISIS_PASSWD_TYPE_CLEARTXT) {
				vty_out(vty, " isis password clear %s%s",
					circuit->passwd.passwd, VTY_NEWLINE);
				write++;
			}
		}
