				 ...
				 struct isis_srv6_sid *sid)
{
	...
	zclient_send_localsid(zclient, ZEBRA_ROUTE_ADD, &sid->sid, prefixlen, ifp->ifindex, action,
			      &ctx);
}
				   ...
				   struct isis_srv6_sid *sid)
{
	...
	zclient_send_localsid(zclient, ZEBRA_ROUTE_DELETE, &sid->sid, prefixlen, ifp->ifindex,
			      action, NULL);
}
...
void isis_zebra_srv6_adj_sid_install(struct srv6_adjacency *sra)
{
	...
	zclient_send_localsid(zclient, ZEBRA_ROUTE_ADD, &sra->sid, prefixlen, ifp->ifindex, action,
			      &ctx);
}
...
void isis_zebra_srv6_adj_sid_uninstall(struct srv6_adjacency *sra)
{
	...
	zclient_send_localsid(zclient, ZEBRA_ROUTE_DELETE, &sra->sid, prefixlen, ifp->ifindex,
			      action, NULL);
}
