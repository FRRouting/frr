 ...
 */
static void
isis_leaking_apply_match_ip_reach(struct isis_area *area,
				  ...
				  struct isis_extended_ip_reach *ip_reach)
{
	...
	map_ret = route_map_apply(redist->map,
				  (struct prefix *)&(ip_reach->prefix),
				  &area_info);

	if (map_ret == RMAP_PERMITMATCH) {
		struct prefix_leaking *leaking =
			(struct prefix_leaking *)calloc(1, sizeof(*leaking));
		leaking->extended_ip_reach = list_new();
		listnode_add(leaking->extended_ip_reach, ip_reach);
		leaking->metric = area_info.metric;
		listnode_add(area->leaking_list[redist->level_to], leaking);
	}
}

static void
isis_leaking_apply_match_ipv6_reach(struct isis_area *area,
				    struct isis_leaking *redist,
				    struct isis_ipv6_reach *ipv6_reach)
{
	...
	map_ret = route_map_apply(redist->map,
				  (struct prefix *)(&ipv6_reach->prefix),
				  &area_info);
...
static void isis_matching_leaking_prefix(struct isis_area *area,
					 struct isis_lsp *lsp,
					 struct isis_leaking *redist)
{
	if (redist->map_name) {
		if (redist->family == AF_INET)
			for (struct isis_item *ip_reach =
				...
				isis_leaking_apply_match_ip_reach(area, redist,
		...
		if (redist->family == AF_INET6)
			for (struct isis_item *ipv6_reach =
				...
				isis_leaking_apply_match_ipv6_reach(
					area, redist,
					(struct isis_ipv6_reach *)ipv6_reach);
	}
}
			      ...
			      int family)
{
	...
	for (type = 0; type < ZEBRA_ROUTE_MAX; type++) {
		...
		for (ALL_LIST_ELEMENTS_RO(area->leaking_settings, node, redist)) {
			if (!redist->redist)
				continue;
			vty_out(vty, " redistribute %s %s", family_str,
				zebra_route_string(type));
