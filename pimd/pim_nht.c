				    ...
				    bool neighbor_needed, struct interface *ingress_ifp)
{
	...
	for (nh_node = rib->nexthop; nh_node; nh_node = nh_node->next) {
		 ...
		 *
		 * This has to happen before the neighbor is counted below: an
		 * unrepaired gate leaves num_nbrs at 0, and the zero-count check
		 * then returns before the path selection loop is ever reached.
		 */
		if (neighbor_needed && !pim_if_connected_to_source(ifp, src) &&
		    pim_addr_is_any(nhaddr)) {
