...
int pim_msdp_mesh_group_source_modify(struct nb_cb_modify_args *args)
{
	...
	case NB_EV_ABORT:
		break;
	case NB_EV_APPLY:
		mg = nb_running_entry(args->dnode);
		vrf_dnode =
...
int pim_msdp_peer_authentication_type_modify(struct nb_cb_modify_args *args)
{
	...
	case NB_EV_ABORT:
		...
		break;
	case NB_EV_APPLY:
		mp = nb_running_entry(args->dnode);
...
int pim_msdp_mesh_group_members_destroy(struct nb_cb_destroy_args *args)
{
	...
	case NB_EV_ABORT:
		break;
	case NB_EV_APPLY:
		mbr = nb_running_entry(args->dnode);
		mg_dnode =
			yang_dnode_get_parent(args->dnode, "msdp-mesh-groups");
		mg = nb_running_entry(mg_dnode);
	...
	struct nb_cb_destroy_args *args)
{
	...
	case NB_EV_ABORT:
		break;
	case NB_EV_APPLY:
		iif = nb_running_entry(args->dnode);
		pim_iifp = iif->info;
		pim = pim_iifp->pim;
