...
DEFUN_HIDDEN (interface_ip_igmp_join,
...
DEFUN_HIDDEN (interface_no_ip_igmp_join,
...
ALIAS (interface_no_ip_igmp_join,
       interface_no_ip_igmp_join_group_cmd,
       "no ip igmp join-group A.B.C.D [A.B.C.D]",
       NO_STR
       IP_STR
       IFACE_IGMP_STR
       "IGMP join multicast group\n"
...
DEFPY (interface_ip_igmp_static_group,
       interface_ip_igmp_static_group_cmd,
       "ip igmp static-group A.B.C.D$grp [A.B.C.D]$src",
       IP_STR
       IFACE_IGMP_STR
       ...
       "Source address\n")
{
	...
	if (src_str && strcmp(src_str, "0.0.0.0") == 0) {
		vty_out(vty, "Bad source address %s\n", src_str);
		return CMD_WARNING_CONFIG_FAILED;
	}

	snprintf(xpath, sizeof(xpath), FRR_GMP_STATIC_GROUP_XPATH,
		 "frr-routing:ipv4", grp_str, (src_str ? src_str : "0.0.0.0"));
...
DEFPY (interface_no_ip_igmp_static_group,
       interface_no_ip_igmp_static_group_cmd,
...
void pim_cmd_init(void)
{
	...
	install_element(INTERFACE_NODE, &interface_no_ip_igmp_join_group_cmd);
	install_element(INTERFACE_NODE, &interface_ip_igmp_static_group_cmd);
	install_element(INTERFACE_NODE, &interface_no_ip_igmp_static_group_cmd);
