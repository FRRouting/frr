...
void igmp_source_reset_gmi(struct gm_group *group, struct gm_source *source)
{
	...
	if (!pim_filter_match(&pim_ifp->gmp_filter, &sg, group->interface)) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("Timer for %pSG4 on %s not refreshed due to route-map reject",
				   &sg, ifp->name);
		return;
	}
			...
			struct in_addr *sources, int from_igmp_v2_report)
{
	...
	if (!pim_filter_match(&pim_ifp->gmp_filter, &sg, igmp->interface)) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("Rejected ISEX %pSG4 on %s due to route-map", &sg, ifp->name);
		return;
	}
			...
			struct in_addr *sources)
{
	...
	if (!pim_filter_match(&pim_ifp->gmp_filter, &sg, igmp->interface)) {
		if (PIM_DEBUG_GM_TRACE)
			zlog_debug("Rejected TOEX %pSG4 on %s due to route-map", &sg, ifp->name);
		return;
	}
