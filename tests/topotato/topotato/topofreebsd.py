...
class NetworkInstance(ClassHooks):
    ...
    class BaseNS(FreeBSDJail):
        ...
        def routes(self, af = 4, local = False):
            ...
            jsroutes = json.loads(self.check_output(['netstat', '--libxo=json,pretty', '-rn']).decode('US-ASCII'))
            jsroutes = jsroutes['statistics']['route-information']['route-table']['rt-family']
            afstr = 'Internet' if af == 4 else 'Internet6'
            jsroutes = [afroutes for afroutes in jsroutes if afroutes.get('address-family') == afstr][0]['rt-entry']
            ...
            for entry in jsroutes:
                ...
                if dst.startswith('fe80::'):
                    continue
                if entry.get('gateway', '').startswith('link#') and entry['interface-name'] == 'lo0':
                    if not local:
                        continue
                ret[dst] = entry
            return ret
    ...
    class RouterNS(BaseNS):
        ...
        def start(self):
            ...
            '''
            super().start()
            # self.check_call(['/bin/sh', '-c', 'echo 1 > /proc/sys/net/ipv4/ip_forward'])
            # self.check_call(['/bin/sh', '-c', 'echo 1 > /proc/sys/net/ipv6/conf/all/forwarding'])
            # self.check_call(['/bin/sh', '-c', 'echo 1 > /proc/sys/net/ipv6/conf/default/forwarding'])
            # self.check_call(['/bin/sh', '-c', 'echo 0 > /proc/sys/net/ipv6/conf/all/accept_dad'])
            # self.check_call(['/bin/sh', '-c', 'echo 0 > /proc/sys/net/ipv6/conf/default/accept_dad'])
            ...
            for iface in self.instance.network.routers[self.name].ifaces:
                epairname = subprocess.check_output(['ifconfig', 'epair', 'create']).decode('US-ASCII').strip()
                ...
                for ip4 in iface.ip4:
                    self.check_call(['ifconfig', iface.ifname, 'inet', 'alias', str(ip4)])
                for ip6 in iface.ip6:
                    self.check_call(['ifconfig', iface.ifname, 'inet6', 'no_dad', 'alias', str(ip4)])

                subprocess.check_call(['ifconfig', epairother, 'name', ifname(self.name, iface.ifname)])
                subprocess.check_call(['ifconfig', ifname(self.name, iface.ifname), 'vnet', '%d' % self.instance.switch_ns.jid])
        ...
        def link_set(self, iface, state):
            ...
            self.instance.switch_ns.check_call(['ifconfig', ifn, 'up' if state else 'down'])
    ...
    def start(self):
        ...
        for links in self.network.links.values():
            for link in links:
                ...
                brcreated = self.switch_ns.check_output(['ifconfig', 'bridge', 'create']).decode('US-ASCII').strip()
                self.switch_ns.check_call(['ifconfig', brcreated, 'name', brname])
                self.switch_ns.check_call(['ifconfig', ifname(link.a.endpoint.name, link.a.ifname), 'inet6', 'ifdisabled', 'up'])
                self.switch_ns.check_call(['ifconfig', ifname(link.b.endpoint.name, link.b.ifname), 'inet6', 'ifdisabled', 'up'])
                self.switch_ns.check_call(['ifconfig', brname,
                    'addm', ifname(link.a.endpoint.name, link.a.ifname),
                    'addm', ifname(link.b.endpoint.name, link.b.ifname),
                    'up'
                ])
        ...
        for lan in self.network.lans.values():
            ...
            for iface in lan.ifaces:
                self.switch_ns.check_call(['ifconfig', ifname(iface.other.endpoint.name, iface.other.ifname), 'inet6', 'ifdisabled', 'up'])
                addm.extend(['addm', ifname(iface.other.endpoint.name, iface.other.ifname)])
