...
static ssize_t printfrr_st(struct fbuf *buf, struct printfrr_eargs *ea, uintmax_t val)
{
	...
	switch (val) {
		/* clang-format off */
	case OFFLIST:  return ret + bputs(buf, "OFFLIST");
	case ADDING:   return ret + bputs(buf, "ADDING");
	case ONLIST:   return ret + bputs(buf, "ONLIST");
	case REMOVING: return ret + bputs(buf, "REMOVING");
		/* clang-format on */
	default:
...
static void *thread_func(void *arg)
{
	...
	for (i = 0; i < itercount; i++) {
		...
		atomic_store_explicit(&ti->pos, i, memory_order_relaxed);
	...
	atomic_store_explicit(&ti->pos, ~0ULL, memory_order_relaxed);
...
static void mt_tests(long n_threads, size_t levels_init)
{
	...
	do {
		...
		if (atomic_load_explicit(&rcu_thread_info.stopped, memory_order_relaxed))
		...
		for (long i = 0; i < n_threads; i++) {
			size_t pos = atomic_load_explicit(&threads[i].pos, memory_order_relaxed);
			...
			if (atomic_load_explicit(&threads[i].stopped, memory_order_relaxed)) {
	...
	for (long i = 0; i < n_threads; i++) {
		...
		pos = atomic_load_explicit(&threads[i].pos, memory_order_relaxed);
