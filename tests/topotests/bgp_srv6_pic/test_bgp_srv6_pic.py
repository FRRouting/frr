...
def setup_module(mod):
    ...
    for rname, router in tgen.routers().items():
        router.load_config(
            TopoRouter.RD_ZEBRA, os.path.join(CWD, "{}/zebra.conf".format(rname)), '-p'
        )
        router.load_config(
...
def test_bgp_route_presence():
    ...
    logger.info(
        "Check that 192.168.2.0/24 vpnv4 entry has 4 entry with remote-sid"
    )
    check_rib("r1", "show bgp ipv4 vpn 192.168.2.0/24 json", "json/r1_vpnv4_route.json")

    logger.info(
        "Check that 192.168.2.0/24 unicast entry has 4 entry with remote-sid"
    )

    check_rib("r1", "show bgp vrf vrf10 ipv4 unicast 192.168.2.0/24 json", "json/r1_bgp_vrf10_route.json")

    logger.info(
        "Check that 192.168.2.0/24 unicast entry in fib has 4 entry with remote-sid"
    )

    check_rib("r1", "show ip route vrf vrf10 192.168.2.0/24 json", "json/r1_ip_vrf10_route.json")

    r1 = tgen.gears["r1"]
    json_ip_route = json.loads(r1.vtysh_cmd("show ip route vrf vrf10 192.168.2.0/24 json"))
    ...
    installedNexthopGroupId = json_ip_route.get('192.168.2.0/24')[0].get('installedNexthopGroupId')
    installedPicNexthopGroupId = json_ip_route.get('192.168.2.0/24')[0].get('installedPicNexthopGroupId')
    ...
    def _check_pic_nexthop():
        ...
        output = json.loads(
            tgen.gears["r1"].vtysh_cmd(nh_cmd)
        )
