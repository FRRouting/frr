...
def test_link_failure_upa_origination():
    ...
    _, result = topotest.run_and_expect(_leaf1_has_blackhole, None, count=30, wait=1)
...
def test_link_recovery_upa_withdrawal():
    ...
    _, result = topotest.run_and_expect(_leaf1_no_blackhole, None, count=30, wait=1)
...
def test_multi_as_extcom_aggregation():
    ...
    _, result = topotest.run_and_expect(
        partial(_leaf_has_blackhole, leaf1, "192.168.100.0/25"), None, count=30, wait=1
    )
    assert result is None, f"Leaf1 blackhole not installed for .0/25: {result}"

    # Check Leaf2 blackholes
    _, result = topotest.run_and_expect(
        partial(_leaf_has_blackhole, leaf2, "192.168.100.0/25"), None, count=30, wait=1
    )
...
def test_partial_recovery_extcom_cleanup():
    ...
    _, result = topotest.run_and_expect(_check_spine, None, count=30, wait=1)
