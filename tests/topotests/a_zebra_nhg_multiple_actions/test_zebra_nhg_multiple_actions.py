...
def test_sharp_install_routes():
    "Test installing 500 unique routes on each leaf using SHARP"
    tgen = get_topogen()
    ...
    # First, install 500 routes on each leaf
    step("Installing 500 routes on each leaf")
    # For each leaf, install 500 routes in its own range
    # Leaf1: 1.0.0.0/32 - 1.0.1.243/32
    # Leaf2: 2.0.0.0/32 - 2.0.1.243/32
    # etc...
    for leaf_num in range(1, 9):
        leaf = tgen.gears[f"leaf{leaf_num}"]
        base_first_octet = leaf_num  # 1-8 for leaves 1-8

        step(f"Installing 500 routes on leaf{leaf_num}")

        # Install 500 /32 routes
        cmd = f"sharp install route {base_first_octet}.0.0.0 nexthop 10.1.{leaf_num}.1 500"
        leaf.vtysh_cmd(cmd)
...
def verify_leaf_route_reception(tgen):
    ...
    for leaf_num in range(1, 9):
        ...
        def _check_route_count():
            ...
            if (
                route_count != 4000
            ):  # 7 other leaves × 500 routes each + 500 routes from own leaf
                return f"Route count mismatch: got {route_count}, expected 4000"
            return None
        ...
        def _check_rib_routes():
            ...
            if routes_in_rib != 3500:
                return f"RIB route count mismatch: got {routes_in_rib}, expected 3500"
            return None
...
def test_leaf_route_source_nhg_consistency():
    ...
    for leaf_num in range(1, 9):
        ...
        def _check_source_nhg_consistency():
            ...
            # Verify each source has all 500 routes
            incomplete_sources = []
