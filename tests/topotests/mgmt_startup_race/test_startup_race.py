...
# Routes to add via CLI during race window
CLI_ROUTES = [
    "192.168.1.0/24",
    "192.168.2.0/24",
    "192.168.3.0/24",
...
def check_route_present(router, prefix):
    """Check if a route is present in zebra."""
...
def test_startup_race(tgen):
    ...
    for prefix in CLI_ROUTES:
        r1.net.cmd_nostatus(f"vtysh -c 'config t' -c 'ip route {prefix} 101.0.0.2'")
    ...
    step("Verifying CLI-added routes are present")
    for prefix in CLI_ROUTES:
        check_route_present(r1, prefix)
    logging.info("All %d CLI routes present", len(CLI_ROUTES))
