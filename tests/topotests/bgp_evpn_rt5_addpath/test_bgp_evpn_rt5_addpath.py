...
def test_bgp_evpn_rt5_addpath_tx_bestpath():
    ...
    f = _converge_fn(r2, "show bgp l2vpn evpn route detail type prefix json", expected)
    _, result = topotest.run_and_expect(f, None, count=60, wait=1)
    assert result is None, "Both paths are still best, they should still be sent"

    logger.info("Make path through C2 better")
    r1: TopoRouter = tgen.gears["r1"]
    r1.vtysh_cmd("""
        conf
        route-map set-pref permit 10
    ...
    f = _converge_fn(r1, "show bgp l2vpn evpn route detail type prefix json", expected)
    _, result = topotest.run_and_expect(f, None, count=60, wait=1)
    assert result is None, "Both paths should be present"
    ...
    f = _converge_fn(r2, "show bgp l2vpn evpn route detail type prefix json", expected)
    _, result = topotest.run_and_expect(f, None, count=60, wait=1)
    assert result is None, "Only the best path through C2 should be sent"
