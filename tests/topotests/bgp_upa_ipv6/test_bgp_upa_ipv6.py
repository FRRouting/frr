...
# Copyright (c) 2026 by
# Patrice Brissette <pbrisset@example.com>
...
def test_bgp_convergence():
    ...
    success, _ = topotest.run_and_expect(_converged, True, count=30, wait=1)
...
def test_upa_extcom_received():
    ...
    success, _ = topotest.run_and_expect(_routes_received, True, count=10, wait=1)
...
def test_aggregate_upa_origination():
    ...
    success, _ = topotest.run_and_expect(_routes_present, True, count=10, wait=0.5)
    ...
    success, _ = topotest.run_and_expect(_upa_originated, True, count=10, wait=0.5)
...
def test_aggregate_upa_withdrawal():
    ...
    success, _ = topotest.run_and_expect(_upa_present, True, count=10, wait=0.5)
    ...
    success, _ = topotest.run_and_expect(_upa_withdrawn, True, count=10, wait=0.5)
...
def test_global_upa_originate_all():
    ...
    success, _ = topotest.run_and_expect(_global_upa_originated, True, count=10, wait=0.5)
...
def test_upa_vs_reachable_precedence():
    ...
    success, _ = topotest.run_and_expect(_upa_is_best, True, count=10, wait=0.5)
    ...
    success, _ = topotest.run_and_expect(_reachable_is_best, True, count=10, wait=0.5)
...
def test_received_upa_dbit_zebra():
    ...
    success, _ = topotest.run_and_expect(_blackhole_installed, True, count=10, wait=1)
