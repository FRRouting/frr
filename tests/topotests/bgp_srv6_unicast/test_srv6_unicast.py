...
def get_unicast_sid(afi):
    ...
    output = tgen.net["r1"].cmd(
        f"vtysh -c 'show bgp segment-routing srv6' | grep -Po 'unicast_sid\\[{afi}\\]: \\K.*'").strip()

    if output is None or "null" in output:
        return "R1 sid[%s] is null" % afi

    r1_unicast_sid = output

    output = tgen.net["r3"].cmd(
        f"vtysh -c 'show bgp segment-routing srv6' | grep -Po 'unicast_sid\\[{afi}\\]: \\K.*'").strip()

    if output is None or "null" in output:
        return "R3 sid[%s] is null" % afi

    r3_unicast_sid = output
...
def test_bgp_srv6_encap():
    ...
    - check srv6 encap R1 <-> R2
    - check srv6 encap-strict R1 <-> R3
