...
#
# pim_vrf_fhr_rp.py is the modified version of test_pim_vrf.py
...
"""
pim_vrf_fhr_rp.py: Test PIM with VRFs and FHR as RP.
"""

# XXX clean up in later commit to avoid conflict on rebase
# pylint: disable=C0413

# Tests PIM with VRF and FHR as RP
#
# R1 is split into 2 VRF: Blue and Red, the others are normal
# routers and Hosts. Here R1 is a RP and FHR connected to Hosts.
...
| receive |------------------|  VRF Blue  |---------+--------|         |
|IGMP JOIN| .10           .1 |  PIM RP    | 192.168.101.0/24 |         |  
+---------+                  |            |                  +---------+
                            =| = = R1 = = |=
+---------+                  |            |                  +---------+
| Host H3 | 192.168.100.0/24 |            | 192.168.101.0/24 | R12     |
| receive |------------------|  VRF Red   |---------+--------|         |
|IGMP JOIN| .20           .1 |  PIM RP    | .1      |    .12 |         |
+---------+                  +------------+         |        +---------+
                                                 .4 | 
                                               +----------+
                                               |  Host H4 |
...
def build_topo(tgen):
    ...
    # Create the main router as RP
    tgen.add_router("r1")

    # Create the PIM NON RP routers
...
def test_vrf_pimreg_interfaces():
    ...
    r1 = tgen.gears["r1"]
    r1.vtysh_cmd("conf\ninterface blue\nip pim")
    r1.vtysh_cmd("conf\nvrf blue\nip pim rp 192.168.0.1 239.100.0.1/32\nexit-vrf")
...
def check_mcast_entry(mcastaddr, pimrp, receiver, sender, vrf):
    ...
    with McastTesterHelper(tgen) as helper:
        ...
        test_func = functools.partial(
            topotest.router_json_cmp, router, "show ip pim vrf {} join json".format(vrf), expected
        )
        _, res = topotest.run_and_expect(test_func, None, count=10, wait=2)
        assertmsg = (
...
def test_mcast_vrf_blue():
    ...
    check_mcast_entry("239.100.0.1", "r1", "h1", "h2", "blue")
...
def test_mcast_vrf_red():
    ...
    check_mcast_entry("239.100.0.1", "r1", "h3", "h4", "red")
