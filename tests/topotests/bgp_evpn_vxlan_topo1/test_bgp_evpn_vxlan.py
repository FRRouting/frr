...
def test_evpn_mac_vni_all_json_nummacs():
    """Verify numMacs stays at the VNI level in MAC JSON output."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
...
def _restore_pe2_vni_state(pe2):
    ...
    pe2.vtysh_cmd("configure terminal\nrouter bgp 65000\n no bgp graceful-restart")


def test_remote_vtep_soft_imet_withdraw(request):
    """
    RT-3 withdraw should soft-disable the remote VTEP while remote MACs
    and neighs remain.
    """
    tgen = get_topogen()
    if tgen.routers_have_failure():
    ...
    pe2 = tgen.gears["PE2"]
    host2 = tgen.gears["host2"]
    request.addfinalizer(lambda: _restore_pe2_vni_state(pe2))
    vni = 101
    ...
    assert result is None, f"PE1 missing expected MACs before test: {result}"
    ...
    _, result = topotest.run_and_expect(test_fn, None, count=10, wait=1)
    assert result is None, f"PE1 lost remote MACs after IMET withdraw: {result}"

    test_fn = partial(evpn_verify_hrep_absent, pe1, vni, remote_vtep)
    _, result = topotest.run_and_expect(test_fn, None, count=10, wait=2)
    assert result is None, f"PE1 still has HREP after IMET withdraw: {result}"
    ...
    def _remote_neigh_withdrawn():
        out = pe1.vtysh_cmd(
            f"show evpn arp-cache vni {vni} ip {host2_ip} json", isjson=True
        )
        if not out:
            return None
        return f"remote neigh {host2_ip} still present on PE1"

    _, result = topotest.run_and_expect(_remote_neigh_withdrawn, None, count=30, wait=2)
    assert result is None, f"PE1 remote neigh not withdrawn: {result}"
...
def test_remote_vtep_orphan_sweep(request):
    """
...
def test_remote_vtep_gr_reconnect(request):
    """BGP GR reconnect must retain EVPN state during stale cleanup."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    request.addfinalizer(
        lambda: pe1.vtysh_cmd("configure terminal\nrouter bgp 65000\n no bgp graceful-restart")
    )

    pe1.vtysh_cmd("configure terminal\nrouter bgp 65000\n bgp graceful-restart")
    pe2.vtysh_cmd("configure terminal\nrouter bgp 65000\n bgp graceful-restart")
    host2.run("ping -c 1 10.10.1.3")

    test_fn = partial(
        evpn_verify_vni_remote_vtep_flood, pe1, 101, "10.30.30.30", "HER"
    )
    _, result = topotest.run_and_expect(test_fn, None, count=20, wait=2)
    assert result is None, f"PE1 baseline remote VTEP flood HER missing: {result}"
    ...
    _, result = topotest.run_and_expect(test_fn, None, count=20, wait=2)
    assert result is None, f"PE1 lost VTEP during GR restart: {result}"
    ...
    _, result = topotest.run_and_expect(test_fn, None, count=30, wait=2)
    assert result is None, f"PE1 VTEP did not recover after GR reconnect: {result}"


def test_advertise_all_vni_toggle(request):
    """Disabling and re-enabling advertise-all-vni preserves remote state."""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    test_fn = partial(
        evpn_verify_vni_remote_vtep_flood, pe1, 101, "10.30.30.30", "-"
    )
    _, result = topotest.run_and_expect(test_fn, None, count=20, wait=2)
    assert result is None, f"PE1 VTEP was removed when advertise-all-vni was disabled: {result}"
    ...
    _, result = topotest.run_and_expect(test_fn, None, count=30, wait=2)
    assert result is None, f"PE1 VTEP did not recover after advertise-all-vni was re-enabled: {result}"
