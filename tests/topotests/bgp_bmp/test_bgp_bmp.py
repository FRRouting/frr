...
def setup_module(mod):
    ...
    # # for debug
    # tgen.gears["r1"].run("rm /tmp/bmp.cap")
    # tgen.gears["r1"].run("tcpdump -nni r1-eth0 -s 0 -w /tmp/bmp.cap &", stdout=None)
...
def _test_prefixes(policy, vrf=None, step=0):
    ...
    for type in ("update", "withdraw"):
        ...
        if policy == LOC_RIB and type == "withdraw":
            # withdraw in loc-rib is not yet supported
            return
