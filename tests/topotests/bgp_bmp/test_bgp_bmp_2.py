...
def test_bgp_routerid_changed():
    ...
    logger.info("checking for BMP peers LOC-RIB message with router-id set to 192.168.1.77.")
    test_func = partial(
        bmp_check_for_peer_message,
        peers,
        "peer up",
    ...
    success, _ = topotest.run_and_expect(test_func, True, count=30, wait=1)
    assert success, "Checking the BMP peer up LOC-RIB message with router-id set to 192.168.1.77 failed !."
