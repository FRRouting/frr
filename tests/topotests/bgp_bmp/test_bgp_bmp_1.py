...
def test_addpath_id():
    ...
    success, ret1 = topotest.run_and_expect_type(test_func, dict, count=30, wait=1)
    assert success, "bmp addpath rib-out-pre-policy failed for : %s" % ret1
    ...
    success, ret1 = topotest.run_and_expect_type(test_func, dict, count=30, wait=1)
    assert success, "bmp addpath rib-out-post-policy failed for %s" % ret1
    ...
    logger.info(
        "*** [R2, R3ecmp] -> R1 -> R4, check bmp withdraw rib-in-pre-policy logging ***"
    )
    test_func = partial(
        bmp_check_for_addpath,
        ["7.7.7.7/32", "7:7:7::7/128"],
    ...
    assert ret2 == ret, "bmp addpath rib-in-pre-policy failed for %s" % ret2

    logger.info(
        "*** [R2, R3ecmp] -> R1 -> R4, check bmp withdraw rib-in-post-policy logging ***"
    )
    test_func = partial(
        bmp_check_for_addpath,
        ["7.7.7.7/32", "7:7:7::7/128"],
    ...
    success, ret2 = topotest.run_and_expect_type(test_func, dict, count=30, wait=1)
    assert ret2 == ret, "bmp addpath rib-in-post-policy failed for %s" % ret2

    logger.info("*** [R2, R3ecmp] -> R1 -> R4, check bmp withdraw loc-rib logging ***")
    test_func = partial(
        bmp_check_for_addpath,
        ["7.7.7.7/32", "7:7:7::7/128"],
        "loc-rib",
    ...
    success, ret2 = topotest.run_and_expect_type(test_func, dict, count=30, wait=1)
    assert ret2 == ret, "bmp addpath loc-rib failed for %s" % ret2

    logger.info(
        "*** [R2, R3ecmp] -> R1 -> R4, check bmp withdraw rib-out-pre-policy logging ***"
    )
    test_func = partial(
        bmp_check_for_addpath,
        ["7.7.7.7/32", "7:7:7::7/128"],
    ...
    logger.info(
        "*** [R2, R3ecmp] -> R1 -> R4, check bmp withdraw rib-out-post-policy logging ***"
    )
    test_func = partial(
        bmp_check_for_addpath,
        ["7.7.7.7/32", "7:7:7::7/128"],
    ...
    success, ret2 = topotest.run_and_expect_type(test_func, dict, count=30, wait=1)
    assert ret2 == ret1, "bmp addpath rib-out-post-policy failed for %s" % ret2
