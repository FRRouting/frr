...
def setup_module(mod):
    ...
    for rname, router in router_list.items():
        print(f"starting dplane server for {rname}")
...
def test_fpmsyncd():
    ...
    r1 = tgen.gears["r1"]
    ref_file = '{}/{}/ref.json'.format(CWD, r1.name)
    expected = json.loads(open(ref_file).read())
    test_func = partial(topotest.router_json_cmp,
                        r1, 'show ip route json', expected)
    _, res = topotest.run_and_expect(test_func, None, count=50, wait=1)
    assertmsg = '{}: ecmp route do not converge'.format(r1.name)
    assert res is None, assertmsg
    
    route_num = 0
    ecmp_route_num =0
    record_json_path = f"{r1.logdir}/r1/output.json"
        ...
        for route in routes:
            if route["protocol"] == "bgp":
                if len(route["nexthops"]) == 1:
                    route_num += 1
                elif len(route["nexthops"]) == 2:
                    ecmp_route_num += 1

    assertmsg = '{}: dplane record route do not match real route'.format(r1.name)
    assert ROUTE_NUM == route_num and ROUTE_NUM == ecmp_route_num,assertmsg
