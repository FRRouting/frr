...
The r2/r3 tenant VRFs enable skip-evpn-imported, so fixed builds still
...
DIRECT_IMPORT_PREFIX = "192.168.70.1/32"
TOGGLE_PREFIX = "192.168.80.1/32"
...
def _configure_skip_evpn_imported(router, vrf, enabled):
    command = "advertise ipv4 unicast skip-evpn-imported"
    if not enabled:
        command = "no {0}".format(command)

    output = router.vtysh_cmd(
        "configure terminal\n"
        "router bgp 65000 vrf {0}\n"
...
def _evpn_type5_prefix_count(router, prefix):
    address, prefixlen = prefix.split("/")
    evpn_prefix = "[5]:[0]:[{0}]:[{1}]".format(prefixlen, address)
    return router.vtysh_cmd("show bgp l2vpn evpn route type 5").count(evpn_prefix)


def _wait_evpn_type5_prefix_count(router, prefix, expected, count=60, wait=1):
    def _check_count():
        return _evpn_type5_prefix_count(router, prefix)

    _, result = topotest.run_and_expect(
        _check_count, expected, count=count, wait=wait
    )
    assert result == expected, (
        "{0} EVPN type-5 {1} count expected={2} last_seen={3}\n{4}".format(
            router.name,
            prefix,
            expected,
            result,
...
def _wait_evpn_type5_prefix_count_at_least(router, prefix, expected, count=60, wait=1):
    def _check_count():
        return _evpn_type5_prefix_count(router, prefix) >= expected

    _, result = topotest.run_and_expect(
        _check_count, True, count=count, wait=wait
    )
    assert result is True, (
        "{0} EVPN type-5 {1} count expected at least {2}\n{3}".format(
            router.name,
            prefix,
            expected,
            router.vtysh_cmd("show bgp l2vpn evpn route type 5"),
...
def test_direct_evpn_import_is_not_reexported_without_skip():
    "Verify the pre-existing direct EVPN-import suppression remains intact."
    ...
    try:
        step("Disable skip-evpn-imported only in GREEN on both PEs")
        for router in (r2, r3):
            _configure_skip_evpn_imported(router, "GREEN", False)

        step("Inject a new Type-5 origin in r1 GREEN")
        _configure_static_route(r1, "GREEN", DIRECT_IMPORT_PREFIX, True)

        step("Verify both PEs directly import the r1 Type-5 route into GREEN")
        _wait_bgp_route(r2, "GREEN", DIRECT_IMPORT_PREFIX, True)
        _wait_bgp_route(r3, "GREEN", DIRECT_IMPORT_PREFIX, True)

        step("Verify direct imports are not re-exported without the new option")
        for router in (r1, r2, r3):
            _wait_evpn_type5_prefix_count(router, DIRECT_IMPORT_PREFIX, 1)
    finally:
        _configure_static_route(r1, "GREEN", DIRECT_IMPORT_PREFIX, False)
        for router in (r2, r3):
            _configure_skip_evpn_imported(router, "GREEN", True)


def test_skip_evpn_imported_running_config():
    "Verify the option is serialized once for every configured tenant VRF."

    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)

    for router in (tgen.gears["r2"], tgen.gears["r3"]):
        running_config = router.vtysh_cmd("show running-config bgpd")
        logger.info(
            "%s running-config with skip-evpn-imported:\n%s",
            router.name,
            running_config,
        )
        actual = running_config.count("advertise ipv4 unicast skip-evpn-imported")
        assert actual == len(PE_VRFS), (
            "{0} expected {1} skip-evpn-imported running-config lines, "
            "found {2}\n{3}".format(
                router.name,
                len(PE_VRFS),
                actual,
...
def test_no_skip_evpn_imported_reexports_and_reenabling_cleans_up():
    "Verify no/enable transitions refresh Type-5 export eligibility."
    ...
    try:
        step("Inject a Type-5 origin while skip-evpn-imported is enabled")
        _configure_static_route(r1, "GREEN", TOGGLE_PREFIX, True)

        step("Verify the original Type-5 route is the only EVPN route")
        for router in (r1, r2, r3):
            _wait_evpn_type5_prefix_count(router, TOGGLE_PREFIX, 1)

        step("Remove skip-evpn-imported from all PE tenant VRFs")
        for router in pe_routers:
            for vrf in PE_VRFS:
                _configure_skip_evpn_imported(router, vrf, False)

        step("Verify removing the option restores EVPN-origin re-export")
        _wait_evpn_type5_prefix_count_at_least(r1, TOGGLE_PREFIX, 2)

        step("Re-enable skip-evpn-imported on all PE tenant VRFs")
        for router in pe_routers:
            for vrf in PE_VRFS:
                _configure_skip_evpn_imported(router, vrf, True)

        step("Verify re-enabling the option withdraws re-exported copies")
        for router in (r1, r2, r3):
            _wait_evpn_type5_prefix_count(router, TOGGLE_PREFIX, 1)
    finally:
        _configure_static_route(r1, "GREEN", TOGGLE_PREFIX, False)
        for router in pe_routers:
            for vrf in PE_VRFS:
                _configure_skip_evpn_imported(router, vrf, True)
