...
def test_rtadv_wheel_remove_after_ifindex_change(tgen):
    ...
    # RTM_DELLINK runs if_delete_update(), which must drain the RA wheel
    # before ifp->ifindex is set to IFINDEX_INTERNAL.  Keep the configured
    # interface until zebra has processed the kernel delete, then remove it.
    ...
    # A stale wheel entry would use-after-free on the next tick; wait one
    # period so process_rtadv() would crash if the drain was missed.
