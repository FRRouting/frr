...
def _wait_ra_armed(r1, ifname):
    ...
    assert ok, "interface {} not listed in ra-interfaces".format(ifname)
...
def _wait_ra_stopped(r1, ifname):
    ...
    assert ok, "interface {} still listed in ra-interfaces".format(ifname)


# Matches RTADV_TIMER_WHEEL_PERIOD_MS in zebra/zebra_router.h.
_RA_WHEEL_PERIOD_SEC = 1


def _wait_zebra_survives_ra_wheel(r1):
    """Wait one RA wheel period, then assert zebra survived the tick."""
    deadline = time.monotonic() + _RA_WHEEL_PERIOD_SEC

    def _zebra_alive_past_wheel_period():
        err = r1.check_router_running()
        if err:
            return "zebra exited: {}".format(err)

        remaining = deadline - time.monotonic()
        if remaining > 0:
            return "waiting {:.1f}s for RA wheel period".format(remaining)

        return None

    ok, result = topotest.run_and_expect(
        _zebra_alive_past_wheel_period, None, count=30, wait=1
    )
    assert ok and result is None, result
