...
This test uses a raw BGP speaker (bgp_sender.py) to send 20000 routes
...
15-ASN AS_PATH, making the total data (~1.4 MB) well exceed the
...
reads and spins on every queue-full event, producing tens of thousands
of wasted bgp_process_reads invocations.
...
peer1 announces 20000 /32 routes, each in a separate UPDATE message.
...
# so the call count stays roughly proportional to ROUTE_COUNT (~18K observed).
...
def test_bgp_io_thread_not_spinning():
    ...
    the input queue is full. With input-queue-limit 100 and 20000
    individual UPDATE messages sent at maximum speed, the total data
    (~1.4 MB) exceeds the ibuf_work ring buffer (~96 KB). This means
    the socket remains readable while the queue is full, causing
    bgp_process_reads to be invoked tens of thousands of times in
    tight spin loops with no useful work.
