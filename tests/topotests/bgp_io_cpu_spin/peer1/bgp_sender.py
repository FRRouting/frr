...
def main():
    ...
    # Wait for FRR to start listening
    for attempt in range(30):
        ...
        print("ERROR: Could not connect to {}:179".format(peer_ip))
        sys.exit(1)

    # Disable Nagle's algorithm for faster individual packet sending
    ...
    # Receive OPEN from peer
    msg_type, _ = recv_msg(sock)
    assert msg_type == BGP_OPEN, "Expected OPEN, got type {}".format(msg_type)
    ...
    msg_type, _ = recv_msg(sock)
    assert msg_type == BGP_KEEPALIVE, "Expected KEEPALIVE, got type {}".format(msg_type)
