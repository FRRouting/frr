...
def setup_module(module):
    ...
    # Load unified FRR configurations (frr.conf for R1, R2, R3, Spine)
...
def test_ipv4_unicast_prefix_filtering():
    ...
    # Wait for the underlay route to be advertised (route exchange may lag
    # slightly behind session establishment).
    spine = tgen.gears["spine"]

    def _underlay_received():
        output = spine.vtysh_cmd(
            "show bgp ipv4 unicast neighbors 192.168.10.1 routes json", isjson=True
        )
        return "10.0.1.1/32" in output.get("routes", {})

    _, result = topotest.run_and_expect(_underlay_received, True, count=60, wait=1)
    assert result, "Spine should receive 10.0.1.1/32 from R1"
