...
#
# 
# Test OSPF RFC4222 Recommendation 2: dead-timer reset on non-Hello packets
# - "ip ospf dead-timer-reset any-control" enables RFC4222 inactivity timer behavior
#
# Verify:
#   - The non-hello dead timer reset counter increments when LSU/DD/LSR/ACK packets
#     are received from a neighbor while dead-timer-reset any-control is active
#
import sys
import pytest
import json
...
def test_ospf_inactive_basic (tgen):
...
def test_ospf_dead_timer(tgen):
    ...
    # Ensure adjacency is up before playing with Inactivity timer packets
...
"""
    ...
    assert (
        resets > 0
    ), "Expected non-hello dead timer resets > 0 after LSU flood, got {}".format(resets)
