...
def setup_module(mod):
    ...
    cmds_vlan = [
        "ip link add link r2-eth2 name r21-eth2 type vlan id 41",
        "ip link set dev r21-eth2 up",
        "ip link add link r2-eth2 name r24-eth2 type vlan id 44",
        "ip link set dev r24-eth2 up",
        "ip link set dev r24-eth2 master r2-vrf-141",
        "ip link set dev r24-eth2 up",
    ]
    for cmd in cmds_vlan:
        logger.info("cmd to r2: " + cmd)
    ...
    cmds_vlan = [
        "ip link add link r4-eth0 name r41-eth0 type vlan id 41",
        "ip link set dev r41-eth0 up",
        "ip link add link r4-eth0 name r44-eth0 type vlan id 44",
        "ip link set dev r44-eth0 up",
        "ip link set dev r44-eth0 master r4-vrf-141",
        "ip link set dev r44-eth0 up",
    ]
    for cmd in cmds_vlan:
        logger.info("cmd to r4: " + cmd)
        output = router.cmd_raises(cmd)
        logger.info("result: " + output)
