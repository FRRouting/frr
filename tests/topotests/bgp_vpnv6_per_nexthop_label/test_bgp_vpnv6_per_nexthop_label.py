...
def test_network_command():
    ...
    for p in ["172:31::14/128", "172:31::15/128"]:
        test_func = functools.partial(
            check_show_bgp_vpn_prefix_not_found, tgen.gears["r2"], "ipv6", p,
            "444:1"
        )
        success, _ = topotest.run_and_expect(test_func, None, count=10, wait=3)
    ...
    logger.info("Checking BGP VPNv6 labels on r2")
    test_func = functools.partial(
        check_show_bgp_vpn_prefix_not_found, tgen.gears["r2"], "ipv6",
        "172:31::14/128", "444:1"
    )
    success, _ = topotest.run_and_expect(test_func, None, count=10, wait=3)
    assert success, "network %s should not present on r2" % p
    ...
    logger.info("Use network command for connected host network 192:168::255:0/112 in vrf")
    tgen.gears["r1"].vtysh_cmd(
        "configure terminal\nrouter bgp 65500 vrf vrf1\naddress-family ipv6 unicast\n"
    ...
    test_func = functools.partial(
        ...
        tgen.gears["r1"], tgen.gears["r2"], "vrf1", network="192:168::255:0/112",
    )
    success, _ = topotest.run_and_expect(test_func, None, count=10, wait=3)
    assert success, "192:168::255:0/112 does not fallback to vrf"

    logger.info("Use network command for statically routed network 192:168::3:0/112 in vrf")
    ...
    test_func = functools.partial(
        ...
        tgen.gears["r1"], tgen.gears["r2"], "192:168::3:0/112", "192:2::11"
    )
    success, _ = topotest.run_and_expect(test_func, None, count=10, wait=3)
