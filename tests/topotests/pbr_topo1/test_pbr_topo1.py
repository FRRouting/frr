...
def test_pbr_nhg_with_vrf_disable_enable():
        ...
        if isinstance(nhg_data, list):
            for item in nhg_data:
                if isinstance(item, dict) and item.get("name") == "C":
                    nhg = item
                    break
        elif isinstance(nhg_data, dict):
            for key, value in nhg_data.items():
                if isinstance(value, dict) and value.get("name") == "C":
                    nhg = value
                    break
    ...
    # Allow time for VRF restoration to propagate
    topotest.sleep(2, "Waiting for VRF restoration to propagate")
