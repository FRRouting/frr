...
def test_bgp_suppress_duplicates():
    ...
    configure terminal
        router bgp
         address-family ipv4 unicast
          no network 10.5.5.5/32 route-map rmap-network
    """
    )

    # For route-map to take effect
    test_func = functools.partial(_bgp_check_route_non_exist, r1, "10.5.5.5/32")
    _, result = topotest.run_and_expect(test_func, None, count=20, wait=1)
    assert result is None, "bgp route not removed"
    ...
    configure terminal
        router bgp
         address-family ipv4 unicast
          network 10.5.5.5/32 route-map rmap-network
    """
    )
