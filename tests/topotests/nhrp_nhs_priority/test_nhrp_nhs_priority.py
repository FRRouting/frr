...
"""
test_nhrp_nhs_priority.py: Test NHS priority for NHRP
"""
...
def test_protocols_convergence():
    ...
    for rname, router in router_list.items():
        if "nh" not in rname:
            continue
        step("Check {} NHRP cache".format(rname))
    ...
    for rname, router in router_list.items():
        ...
        step("Check {} NHRP routes".format(rname))
    ...
    # Check NHRP NHS configuration on clients
    logger.info("Checking NHS configuration")
    for rname, router in router_list.items():
        if "nhc" not in rname:
            continue

        step("Check {} NHS configuration".format(rname))
        json_file = "{}/{}/nhrp_nhs.json".format(CWD, router.name)
        expected = json.loads(open(json_file).read())
        test_func = partial(
            topotest.router_json_cmp, router, "show ip nhrp nhs json", expected
        )
        _, result = topotest.run_and_expect(test_func, None, count=40, wait=0.5)

        output = router.vtysh_cmd("show ip nhrp nhs json")
        logger.info(output)
...
def test_shortcut():
    ...
    logger.info("Testing NHRP shortcuts with NHS priorities")

    # Verify nhc1 nhrp routes before shortcut creation
    step("Verify nhc1 NHRP routes before shortcut creation")
    nhc1 = tgen.gears["nhc1"]
    ...
    step("Initiate shortcut by pinging between clients")
    host = tgen.gears["host"]
    ping_test(host, "10.5.5.5", 1000, "nhc2 via shortcut")

    # Now check that NHRP shortcut route installed
    step("Now check that NHRP shortcut route installed")
    ...
    step("check the shortcut disappears because of no traffic")
...
def test_shortcut_backup():
    ...
    step("Bringing down nhs1, primary NHRP server.")
    ...
    step("Verify shortcut is still active")
    host = tgen.gears["host"]
    ping_test(host, "10.5.5.5", 1000, "nhc2 via shortcut")
...
def test_nhs_recovery():
    ...
    step("Bring up primary NHS GRE interface")
    logger.info("Bringing up nhs1, primary NHRP server.")
    shutdown_bringup_interface(tgen, "nhs1", "nhs1-gre0", True)
    step("Re-test protocol convergence with NHS1 up")
    test_protocols_convergence()
    step("Re-test shortcut with NHS1 up")
    test_shortcut()
