...
def test_frr_reload_interface_section_deletion():
    ...
    def test_deleted_interface_converge():
        output = json.loads(r1.vtysh_cmd(f"show interface veth0 json"))
        expected = {
            "veth0": {
                "ipAddresses": None,
    ...
    r1.cmd_raises("vtysh -c 'write terminal no-header' > frr-with-interface.conf")
    ...
    _, result = topotest.run_and_expect(test_deleted_interface_converge, None, count=30, wait=1)
    assert (
        result is None
    ), "Has ipAddresses field interface deletion and reload to clean"
