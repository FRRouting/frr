...
# Tracker timeout r1 boots with (see r1/frr.conf).  A test that changes the
# timer must restore this, otherwise every later test inherits it: a tracker
# that cannot saturate waits the full timer before flushing.
STARTUP_TRACKER_TIMEOUT = 20
...
def test_nhg_tracker_no_form():
    ...
    logger.info(
        "Verifying 'show zebra' on r1 reports NHG Tracker Timeout back to default 10 seconds"
    )

    def check_show_zebra_tracker_default():
        output = r1.vtysh_cmd("show zebra")
        if not re.search(r"NHG Tracker Timeout\s+10 seconds", output):
            return "NHG Tracker Timeout 10 seconds not found in show zebra"
        return None
...
def test_nhg_tracker_route_map():
    ...
    # and put the tracker timer back to the value r1 booted with so the
    # 60s set above does not slow down whatever runs next.
    # ------------------------------------------------------------------
    ...
    r1.vtysh_cmd(
        "\n".join(
            [
                ...
                "zebra nexthop-group tracker {}".format(STARTUP_TRACKER_TIMEOUT),
                "end",
            ]
        )
    )
...
def test_nhg_tracker_ecmp_2way_1way_transition():
    """
    Verify a 2 -> 1 -> 2 ECMP width change is handled as an ordinary
    width change and not as a singleton<->group shape change.

    Steps:
      0. Scrub any leftover sharp routes for 48.1.1.1..10/32.
      1. Install 10 routes with 2-way ECMP and record the group.
      2. Shrink to 1-way; check the tracker fired, took a reuse path,
         did not record a shape skip, and converged.
      3. Grow back to 2-way; same checks.
      4. Cleanup.
    """
    tgen = get_topogen()
    if tgen.routers_have_failure():
    ...
    prefixes = ["48.1.1.{}/32".format(i) for i in range(1, 11)]
    two_way_nhs = ["10.0.1.2", "10.0.2.3"]
    one_way_nhs = ["10.0.1.2"]

    # Pin the timer instead of inheriting whatever a previous test left.
    # On the 1-way leg the group also carries routes reachable only via
    # r2, which do not change, so the tracker cannot saturate and always
    # waits the full timer before flushing.
    ...
    def install(nhs):
        r1.vtysh_cmd(
            "sharp install routes 48.1.1.1 nexthops {} 10".format(",".join(nhs))
        )

    def converged_nhg_id():
        """Single NHG id shared by all prefixes, or (None, err)."""
        nhg_id, err = _get_route_installed_nhg_id(r1, prefixes[0], "sharp")
        if err:
            return None, err
        if not nhg_id or nhg_id <= 0:
            return None, "{} has no installed NHG id".format(prefixes[0])
        err = _check_routes_use_nhg(r1, prefixes, nhg_id)
        if err:
            return None, err
        return nhg_id, None

    # --- Step 0: scrub any leftover sharp routes for this prefix range
    logger.info("Step 0: scrub leftover sharp routes for 48.1.1.1..10/32")
    r1.vtysh_cmd("sharp remove routes 48.1.1.1 10")
    _, result = topotest.run_and_expect(check_sharp_clean, None, count=10, wait=1)
    assert result is None, "leftover sharp routes not cleared: {}".format(result)

    # --- Step 1: install the 2-way baseline
    logger.info("Step 1: sharp install 10 routes with 2-way ECMP %s", two_way_nhs)
    install(two_way_nhs)

    def check_baseline():
        nhg_id, err = converged_nhg_id()
        if err:
            return err
        return _check_nhg_nexthops(r1, nhg_id, two_way_nhs)

    _, result = topotest.run_and_expect(check_baseline, None, count=30, wait=1)
    assert result is None, "2-way install did not converge: {}".format(result)

    base_nhg_id, err = converged_nhg_id()
    assert err is None, "could not read baseline NHG id: {}".format(err)

    _, result = topotest.run_and_expect(
        lambda: _check_kernel_state(r1, base_nhg_id, two_way_nhs, prefixes),
        None,
        count=20,
        wait=1,
    )
    assert result is None, "baseline kernel state mismatch: {}".format(result)
    logger.info("Step 1: baseline NHG id %s carries %s", base_nhg_id, two_way_nhs)

    def transition(label, nhs):
        """Re-issue the routes with `nhs` and verify the tracker reused."""
        before = _get_tracker_global_counters(r1)
        assert before is not None, "{}: could not snapshot tracker counters".format(
            label
        )

        logger.info("%s: re-issue 48.1.1.1..10/32 with nexthops %s", label, nhs)
        install(nhs)

        def check_tracker_reused():
            cur = _get_tracker_global_counters(r1)
            if cur is None:
                return "could not snapshot tracker counters"

            def delta(key):
                return cur.get(key, 0) - before.get(key, 0)

            # The tracker must arm and complete.  If the width change is
            # misread as a shape change, rib_link_track_ecmp_change never
            # creates one and these stay flat.
            if delta("trackersAllocated") < 1 or delta("trackersFreed") < 1:
                return (
                    "tracker did not fire: allocated +{}, freed +{} "
                    "(a width change must create a tracker)".format(
                        delta("trackersAllocated"), delta("trackersFreed")
                    )
                )
            # The winner must take a reuse path rather than the
            # shape-mismatch skip.
            if delta("reuseSkip") != 0:
                return (
                    "reuseSkip advanced by {}: the width change was treated "
                    "as a singleton<->group shape change and NHG reuse was "
                    "abandoned".format(delta("reuseSkip"))
                )
            reused = delta("reuseCopy") + delta("reuseFastpath")
            if reused < 1:
                return (
                    "no winner took a reuse path (reuseCopy +{}, "
                    "reuseFastpath +{})".format(
                        delta("reuseCopy"), delta("reuseFastpath")
                    )
                )
            return None

        _, result = topotest.run_and_expect(
            check_tracker_reused, None, count=60, wait=2
        )
        assert result is None, "{}: {}".format(label, result)

        def check_converged():
            nhg_id, err = converged_nhg_id()
            if err:
                return err
            return _check_nhg_nexthops(r1, nhg_id, nhs)

        _, result = topotest.run_and_expect(check_converged, None, count=30, wait=1)
        assert result is None, (
            "{}: routes did not settle on one group carrying {}: {}".format(
                label, nhs, result
            )
        )

        nhg_id, err = converged_nhg_id()
        assert err is None, "{}: could not read NHG id: {}".format(label, err)

        _, result = topotest.run_and_expect(
            lambda: _check_kernel_state(r1, nhg_id, nhs, prefixes),
            None,
            count=20,
            wait=1,
        )
        assert result is None, "{}: kernel state mismatch: {}".format(label, result)

        logger.info(
            "%s: PASS - tracker reused, all routes on NHG %s carrying %s",
            label,
            nhg_id,
            nhs,
        )
    ...
    # --- Step 2: shrink 2-way -> 1-way
    transition("Step 2 (2-way -> 1-way)", one_way_nhs)

    # --- Step 3: grow 1-way -> 2-way
    transition("Step 3 (1-way -> 2-way)", two_way_nhs)

    # --- Step 4: cleanup
    logger.info("Step 4: cleanup sharp routes 48.1.1.1..10/32")
    r1.vtysh_cmd("sharp remove routes 48.1.1.1 10")
    r1.vtysh_cmd(
        "configure terminal\nzebra nexthop-group tracker {}\nend".format(
            STARTUP_TRACKER_TIMEOUT
        )
    )
    _, result = topotest.run_and_expect(check_sharp_clean, None, count=20, wait=1)
    assert result is None, "cleanup did not remove sharp routes: {}".format(result)
