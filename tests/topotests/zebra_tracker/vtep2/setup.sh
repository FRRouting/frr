#!/bin/bash
# Linux plumbing for vtep2's two L3VNIs -- identical to vtep1/setup.sh with
# this VTEP's tunnel source (10.121.0.2) and SVI addresses.
...
ip link add vxlan4001 type vxlan id 4001 dstport 4789 local 10.121.0.2 nolearning
...
ip addr add 10.201.0.12/24 dev vlan101
...
ip link add vxlan4002 type vxlan id 4002 dstport 4789 local 10.121.0.2 nolearning
...
ip addr add 10.202.0.12/24 dev vlan102
