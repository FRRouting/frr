...
#
# SRv6 L2 EVPN topotest - VLAN Bundle service type.
#
...
# Each C-VLAN gets its own RFC 5737 documentation /24 (per FRR topotest policy).
VLAN_NET = {11: "192.0.2", 12: "198.51.100", 13: "203.0.113"}
...
def setup_module(mod):
    ...
    for rname in ("r1", "r2"):
        r = tgen.gears[rname]
        tgen.gears[rname].load_frr_config(os.path.join(CWD, "%s/frr.conf" % rname))
    ...
    # locator only after the initial frr.conf load; same race the vlan-based
    # test handles).
    ...
    for r, loc, pfx in (("r1", "LOC1", "fcbb:bbbb:1::/48"), ("r2", "LOC1", "fcbb:bbbb:2::/48")):
    ...
    def _converged():
        try:
            ...
            if not ("fcbb:bbbb:2:" in r1r and "fcbb:bbbb:1:" in r2r):
...
def test_per_evi_locator_sid_advertised():
    ...
    for r, peer_block in (("r1", "fcbb:bbbb:2:"), ("r2", "fcbb:bbbb:1:")):
...
def test_bundle_single_srl2():
    ...
    for vid in BUNDLE_VLANS:
        tgen.gears["r1"].run("ping -c1 -W1 -I %s.1 %s.2" % (VLAN_NET[vid], VLAN_NET[vid]))
...
def test_bundle_transparent_ping():
    ...
    for vid in BUNDLE_VLANS:
        check_ping("r1", "%s.2" % VLAN_NET[vid], True, 20, 1, source_addr="%s.1" % VLAN_NET[vid])
