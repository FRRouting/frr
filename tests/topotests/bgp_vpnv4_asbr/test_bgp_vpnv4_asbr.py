...
def bgp_vpnv4_prefix_check(router, rd, prefix, label, nexthop):
    ...
    * 'nexthop': The nexthop expected associated with the ('rd','prefix') tuple
    """
    logger.info("Checking BGP IPv4 VPN update on {}".format(router.name))
    dump = router.vtysh_cmd("show bgp ipv4 vpn {} json".format(prefix), isjson=True)
    assert dump, "{0}, {1}, route distinguisher {2} not present".format(
    ...
    for dumped_rd, pathes in dump.items():
        ...
        for path in pathes["paths"]:
            ...
            ), "{0}, {1}, rd {2}, remoteLabel not present".format(
                router.name, prefix, rd
            )
            ...
            assert (
                "nexthops" in path.keys()
            ), "{0}, {1}, rd {2}, no nexthops present".format(router.name, prefix, rd)
            for nh in path["nexthops"]:
                assert (
                    "ip" in nh.keys()
                ), "{0}, {1}, rd {2}, no ipv4 nexthop available".format(
                    router.name, prefix, rd
                )
                logger.info(
                    "{0}, {1}, rd {2}, remoteLabel {3}, nexthop {4}".format(
                        router.name, prefix, rd, path["remoteLabel"], nh["ip"]
                    )
                )
                if nh["ip"] != nexthop:
                    continue
                logger.info(
                    "{0}, {1}, rd {2}, remoteLabel {3}, nexthop {4} found".format(
                        router.name, prefix, rd, path["remoteLabel"], nh["ip"]
                    )
                )
                return None
    assert 0, "{}, show bgp ipv4 vpn {}, rd {}, label {} nexthop {} not found".format(
    ...
    for prefix, rname_to_test in vpnv4_entries.items():
        ...
        for rd, pathes in dump.items():
            for path in pathes["paths"]:
                ...
                assert (
                    "nexthops" in path.keys()
                ), "{0}, {1}, no nexthop available".format(router.name, prefix)
                for nh in path["nexthops"]:
                    assert (
                        "ip" in nh.keys()
                    ), "{0}, {1}, no ipv4 nexthop available".format(router.name, prefix)
                    logger.info(
                        "{0}, {1}, label value is {2}, nexthop is {3}".format(
                            router.name, prefix, path["remoteLabel"], nh["ip"]
                        )
                    )
                    in_label = mpls_table_check_entry(
                        router, path["remoteLabel"], vpnv4_nht[nh["ip"]]
                    )
