...
def test_comm_attri_sent_to_nbr_includes_large(request):
    ...
    assert "large" in comm_attri.lower(), (
        ...
        "even though send-community large is enabled by default.".format(
            tc_name, comm_attri
        )
    )
    ...
    assert "standard" in comm_attri.lower(), (
        "Testcase {} : Failed \n"
        "commAttriSentToNbr should include 'standard' but got: '{}'".format(
            tc_name, comm_attri
        )
    )
    assert "extended" in comm_attri.lower(), (
        "Testcase {} : Failed \n"
        "commAttriSentToNbr should include 'extended' but got: '{}'".format(
            tc_name, comm_attri
        )
    )
