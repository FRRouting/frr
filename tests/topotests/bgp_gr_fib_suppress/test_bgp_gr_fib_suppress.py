...
def verify_graceful_restart_json(router, neighbor_ip, expected_local_mode="Restart*", expected_remote_mode="Helper"):
    ...
    try:
        ...
        # Check local GR mode
        local_mode = gr_info.get("localGrMode", "Unknown")
        remote_mode = gr_info.get("remoteGrMode", "Unknown")
...
def verify_f_bit_json(router, neighbor_ip, expected=True):
    ...
    try:
        ...
        # F-bit might be in address family specific section
        ipv4_unicast = gr_info.get("ipv4Unicast", {})
