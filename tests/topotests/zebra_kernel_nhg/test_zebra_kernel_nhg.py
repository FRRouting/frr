...
def _get_frr_owned_nhg_ids(r1, prefix):
    """Return (group_id, singleton_id) for an FRR-owned static route.

    Zebra installs a group NHG on the route that depends on a singleton
    carrying the gateway. Kernel replace/delete of the group id as a
    `via` object is rejected (Invalid argument), so tests that mutate
    the kernel object must target the singleton.
    """
    group_id = _get_route_nhg_id(r1, prefix)
    if group_id is None:
    ...
    if not nhg:
        logger.info("FRR-owned NHG %s not found", group_id)
        return None
    ...
    depends = nhg.get("depends") or []
    if len(depends) != 1:
        logger.info(
            "FRR-owned NHG %s expected a single singleton depend, got %s",
            group_id,
            depends,
        )
        return None

    singleton_id = depends[0]
    singleton_json = r1.vtysh_cmd(
        f"show nexthop-group rib {singleton_id} json", isjson=True
    )
    singleton = singleton_json.get(str(singleton_id))
    if not singleton or singleton.get("type") != "zebra":
        logger.info(
            "FRR-owned singleton %s missing or wrong type: %s",
            singleton_id,
            singleton,
        )
        return None

    return (group_id, singleton_id)
...
def test_frr_owned_nhg_replace_is_reverted():
    ...
    def _get_frr_owned_nhg():
        return _get_frr_owned_nhg_ids(r1, prefix)

    step("Wait for FRR-owned NHG to be installed")
    success, nhg_ids = topotest.run_and_expect_type(
        _get_frr_owned_nhg, tuple, count=20, wait=1
    )
    assert success, f"FRR-owned NHG for route {prefix} was not installed"
    group_id, singleton_id = nhg_ids

    step("Externally replace FRR-owned kernel singleton nexthop")
    r1.cmd_raises(
        f"ip nexthop replace id {singleton_id} via {replacement_ip} dev r1-eth0"
    )

    def _check_frr_owned_replace_reverted():
        nhg_json = r1.vtysh_cmd(
            f"show nexthop-group rib {singleton_id} json", isjson=True
        )
        nhg = nhg_json.get(str(singleton_id))
        if not nhg:
            logger.info(
                "FRR-owned singleton %s disappeared after replace", singleton_id
            )
            return False

        if nhg.get("type") != "zebra":
            logger.info(
                "FRR-owned singleton %s type mismatch after replace: %s",
                singleton_id,
                nhg.get("type"),
            )
            return False
        ...
        if len(nexthops) != 1 or nexthops[0].get("ip") != original_ip:
            logger.info(
                "FRR-owned singleton %s not restored to %s: %s",
                singleton_id,
                original_ip,
                nexthops,
            )
        ...
        kernel_nh = r1.cmd(f"ip nexthop show id {singleton_id}")
        if original_ip not in kernel_nh or replacement_ip in kernel_nh:
            logger.info(
                "Kernel singleton %s not restored, output: %s",
                singleton_id,
                kernel_nh,
            )
            return False

        group_json = r1.vtysh_cmd(f"show nexthop-group rib {group_id} json", isjson=True)
        group = group_json.get(str(group_id))
        if not group or group.get("depends") != [singleton_id]:
            logger.info(
                "FRR-owned group %s missing or lost singleton depend %s: %s",
                group_id,
                singleton_id,
                group,
            )
...
def test_frr_owned_nhg_delete_is_recreated():
    ...
    step("Externally delete FRR-owned kernel singleton nexthop")
    r1.cmd_raises(f"ip nexthop del id {singleton_id}")

    def _check_frr_owned_delete_recreated():
        route = _get_route_entry(r1, prefix)
        if not route or route.get("nexthopGroupId") != group_id:
            logger.info(
        ...
        if not nhg or nhg.get("type") != "zebra":
            logger.info(
                "FRR-owned singleton %s missing or wrong type after delete: %s",
                singleton_id,
                nhg,
            )
            return False
        ...
        if gateway not in kernel_nh:
            logger.info(
                "Kernel singleton %s was not recreated, output: %s",
                singleton_id,
                kernel_nh,
            )
            return False
