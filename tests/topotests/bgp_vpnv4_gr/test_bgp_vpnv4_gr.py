...
def check_show_bgp_vpnv4_prefix(router, rd, prefix, stale=True, presence=True):
    ...
    * 'stale': the stale flag is set if True, otherwise the used flag should be set
    * 'presence': if True, the stale and used flag are checked, otherwise, the prefix should not be present
    """
    ...
    elif not stale and presence:
        expected = {rd: {"prefix": prefix, "paths": [{"stale": True, "valid": True}]}}
        # 1. stale must not be present
        ret = topotest.json_cmp(output, expected)
        if ret is None:
            return "not good"
        # 2. prefix must be present
        expected = {rd: {"prefix": prefix, "paths": [{"valid": True}]}}
        return topotest.json_cmp(output, expected)
    elif not presence:
...
def test_cut_link():
    ...
    router.run("nft add rule ip filter INPUT tcp sport 179 reject with tcp reset")
    router.run("nft add rule ip filter INPUT tcp dport 179 reject with tcp reset")

    router = tgen.gears["r1"]
    check_stale_routes(router)

    # Check BGP VPNv4 routing tables are in stale routes
    logger.info("Checking BGP IPv4 VPN routes are removed".format(router.name))
    for prefix in ("172.31.0.10/32", "172.31.1.10/32"):
        test_func = partial(
    ...
    logger.info("Checking BGP IPv4 routes are removed".format(router.name))
    for prefix in ("172.31.0.10/32", "172.31.1.10/32"):
        test_func = partial(
            check_show_bgp_ipv4_prefix, router, "vrf1", prefix, presence=False
        )
...
def test_uncut_link():
    ...
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    router = tgen.gears["r2"]
    router.run("nft flush chain filter INPUT")

    router = tgen.gears["r1"]
    # Check BGP VPNv4 routing tables are in stale routes
    logger.info("Checking BGP IPv4 VPN routes are no more staled".format(router.name))
    for prefix in ("172.31.0.10/32", "172.31.1.10/32"):
        test_func = partial(
            check_show_bgp_vpnv4_prefix,
            router,
...
def test_cut_link_remove_prefix_and_uncut_link():
    ...
    router_1 = tgen.gears["r1"]
    router_2 = tgen.gears["r2"]
    prefix_1 = "172.31.0.10/32"
    prefix_2 = "172.31.1.10/32"

    logger.info("{}, cutting link between r1 and r2".format(router_2.name))
    router_2.run("nft add rule ip filter INPUT tcp sport 179 reject with tcp reset")
    router_2.run("nft add rule ip filter INPUT tcp dport 179 reject with tcp reset")

    # Check BGP VPNv4 routing tables are in stale routes
    # Check BGP IPv4 route from VRF are still present
    check_stale_routes(router_1)

    logger.info("{}, removing {} network".format(router_2.name, prefix_1))
    router_2.vtysh_cmd(
    ...
    logger.info("{}, uncutting link between r1 and r2".format(router_2.name))
    router_2.run("nft flush chain filter INPUT")
    logger.info(
        "{}, checking BGP IPv4 VPN route {} is removed".format(router_1.name, prefix_1)
    )
    test_func = partial(
        check_show_bgp_vpnv4_prefix,
        router_1,
    ...
    _, result = topotest.run_and_expect(test_func, None, count=40, wait=2)
    assertmsg = '"{}" JSON output mismatches'.format(router_1.name)
    assert result is None, assertmsg

    logger.info(
        "{}, checking BGP IPv4 VPN route {} is present".format(router_1.name, prefix_2)
    )
    test_func = partial(
        check_show_bgp_vpnv4_prefix,
        router_1,
    ...
    logger.info(
        "{}, checking BGP IPv4 route {} is removed".format(router_1.name, prefix_1)
    )
    test_func = partial(
        check_show_bgp_ipv4_prefix, router_1, "vrf1", prefix_1, presence=False
    )
    _, result = topotest.run_and_expect(test_func, None, count=40, wait=2)
    assertmsg = '"{}" JSON output mismatches'.format(router_1.name)
    assert result is None, assertmsg
    ...
    logger.info(
        "{}, checking BGP IPv4 route {} is present".format(router_1.name, prefix_2)
    )
    test_func = partial(
        check_show_bgp_ipv4_prefix, router_1, "vrf1", prefix_2, presence=True
    )
    _, result = topotest.run_and_expect(test_func, None, count=40, wait=2)
    assertmsg = '"{}" JSON output mismatches'.format(router_1.name)
    assert result is None, assertmsg
