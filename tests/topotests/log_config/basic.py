...
def do_test_log(unet, topotest_started=False):
    ...
    step("Testing new daemon logfile staticd.log")

    r1.cmd_raises('vtysh -c "conf\nlog daemon staticd file staticd2.log"')
    common_log["staticd"] = WatchLog(r1.rundir / "staticd2.log")
    s = f"LOGFILE-5-{our_pid}-MARK"
    _do_test_log(r1, s, common_log)

    step("Testing other logs not included in new daemon logfile staticd.log")

    new_logs = dict(daemon_logs.items())
    del new_logs["staticd"]
    s = f"LOGFILE-6-{our_pid}-MARK"
    _do_test_log_notfound(r1, s, common_log)
