...
def wait_for_bgp_convergence(router, expected_state, count=60, wait=5):
...
# Topology Class
class TemplateTopo(object):
    def __init__(self):
        """Create a test topology."""
        self.topo = {
            "routers": {
                "r1": {"links": {"r2": {"ipv4": "172.16.1.1/24"}}},
                "r2": {
                    "links": {
                        "r1": {"ipv4": "172.16.1.2/24"},
                        "r3": {"ipv4": "172.16.2.1/24"},
                    }
                },
                "r3": {"links": {"r2": {"ipv4": "172.16.2.2/24"}}},
            }
        }
...
def build_topo(tgen):
    ...
    # Initialize TemplateTopo
    template = TemplateTopo()
    logger.info("Topology created with following parameters:")
    logger.info("Routers: %s", template.topo['routers'])
...
def test_peer_group_graceful_shutdown_move_peer():
    ...
    success, result = topotest.run_and_expect(_check_peer_group_config, True, count=60, wait=5)
