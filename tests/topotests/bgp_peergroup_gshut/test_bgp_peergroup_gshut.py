...
def test_peer_group_graceful_shutdown_session_stability():
    ...
    success, result = topotest.run_and_expect(
        lambda: check_no_session_flap(r2, "172.16.1.1", r1_timing['bgpTimerUpMsec'], r1_timing['bgpTimerUpEstablishedEpoch']),
        True, count=12, wait=5
    )
    assert success, "R1 session flapped after enabling GSHUT"

    # Check R3 session stability
    success, result = topotest.run_and_expect(
        lambda: check_no_session_flap(r2, "172.16.2.2", r3_timing['bgpTimerUpMsec'], r3_timing['bgpTimerUpEstablishedEpoch']),
        True, count=12, wait=5
    )
    assert success, "R3 session flapped after enabling GSHUT"
    ...
    assert success, "R1 session flapped after disabling GSHUT"
    ...
    assert success, "R3 session flapped after disabling GSHUT"
