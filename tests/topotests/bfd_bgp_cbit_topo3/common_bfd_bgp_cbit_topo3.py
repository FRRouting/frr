...
#
# common_bfd_bgp_cbit_topo3.py
#
# Copyright (c) 2019 6WIND
#

"""
common_bfd_bgp_cbit_topo3.py: Test the FRR BFD daemon with multihop and BGP
unnumbered.
"""
...
def common_setup_module(bgpd_tweak, _mod):
    "Sets up the pytest environment"
    topodef = {
        "s1": ("r1", "r2"),
        "s2": ("r2", "r3"),
    }
    tgen = Topogen(topodef, "test_bfd_bgp_cbit_topo3")
    tgen.start_topology()
    ...
    for rname, router in router_list.items():
        ...
        router.load_config(
            TopoRouter.RD_BGP,
            os.path.join(
                CWD, "{}/{}".format(rname, bgpd_tweak.get(rname, "bgpd.conf"))
            ),
        )
...
def common_teardown_module(_mod):
    "Teardown the pytest environment"
    tgen = get_topogen()
    tgen.stop_topology()


def common_protocols_convergence():
    """
...
def common_bfd_loss_intermediate(peers_tweak):
    """
    ...
    for router in tgen.routers().values():
        if router.name == "r2":
            continue
        json_file = "{}/{}/{}".format(
            CWD, router.name, peers_tweak.get(router.name, "peers_down.json")
        )
        expected = json.loads(open(json_file).read())
...
def common_bfd_comes_back_again():
    """
...
if __name__ == "__main__":
    print("This module cannot run standalone")
    sys.exit(1)
