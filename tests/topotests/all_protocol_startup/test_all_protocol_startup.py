...
def test_nexthop_groups():
    ...
    net["r1"].cmd('ip link set r1-eth1 down;ip link set r1-eth2 down;ip link set r1-eth3 down;ip link set r1-eth4 down')
    sleep(1)
    net["r1"].cmd('ip link set r1-eth1 up;ip link set r1-eth2 up;ip link set r1-eth3 up;ip link set r1-eth4 up')
    sleep(5)
    ...
    post_total_nhs = len((re.search(r'group ([\d/]+)', post_nh_show)).group(1).split('/'))

    assert post_total_nhs == pre_total_nhs, "Expected same nexthops(pre-{}: post-{}) in NHG (pre-{}:post-{}) after few Interface flaps".format(pre_total_nhs, post_total_nhs, pre_nhg.group(1), post_nhg.group(1))
