...
{
	struct prefix p = {.family = AF_UNSPEC};
	uint8_t node_id[7];
	...
	vertices[vertex_count] =
		isis_vertex_new(node_id, VTYPE_NONPSEUDO_TE_IS);
	vertices[vertex_count]->d_N = 15;
	vertex_count++;
...
static void test_ordered(void)
{
	...
	for (size_t i = 0; i < vertex_count; i++) {
		assert(isis_find_vertex(&q, vertices[i]->N.id,
					vertices[i]->type)
		       == vertices[i]);
	}

	assert(isis_vertex_queue_pop(&q) == vertices[2]);
	assert(isis_find_vertex(&q, vertices[2]->N.id, vertices[2]->type)
	       == NULL);

	assert(isis_vertex_queue_pop(&q) == vertices[3]);
	assert(isis_find_vertex(&q, vertices[3]->N.id, vertices[3]->type)
	       == NULL);

	assert(isis_vertex_queue_pop(&q) == vertices[0]);
	assert(isis_find_vertex(&q, vertices[0]->N.id, vertices[0]->type)
	       == NULL);

	assert(isis_vertex_queue_pop(&q) == vertices[1]);
	assert(isis_find_vertex(&q, vertices[1]->N.id, vertices[1]->type)
	       == NULL);

	isis_vertex_queue_delete(&q, vertices[4]);
	assert(isis_find_vertex(&q, vertices[4]->N.id, vertices[4]->type)
	       == NULL);
