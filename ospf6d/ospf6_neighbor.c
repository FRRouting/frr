	...
	*/
	...
	if (on->request_list->count == 0)
		...
		if (on->thread_send_lsreq == NULL)
			on->thread_send_lsreq = thread_add_event(
				master, ospf6_lsreq_send, on, 0);
	}
...
static void ospf6_neighbor_show(struct vty *vty, struct ospf6_neighbor *on)
{
	...
	*/
	...
	vty_out(vty, "%-15s %3d %11s %8s/%-12s %11s %s[%s]%s", router_id,
					 ...
					 struct ospf6_neighbor *on)
{
	...
	*/
	...
	vty_out(vty, "%-15s %8s/%-11s %-15s %-15s %s[%s]%s", router_id,
				       ...
				       struct ospf6_neighbor *on)
{
	...
	vty_out(vty, "    Area %s via interface %s (ifindex %d)%s",
		...
		on->ospf6_if->interface->ifindex, VNL);
	vty_out(vty, "    His IfIndex: %d Link-local address: %s%s",
		on->ifindex, linklocal_addr, VNL);
	vty_out(vty, "    State %s for a duration of %s%s",
		ospf6_neighbor_state_str[on->state], duration, VNL);
	vty_out(vty, "    His choice of DR/BDR %s/%s, Priority %d%s", drouter,
		bdrouter, on->priority, VNL);
	vty_out(vty, "    DbDesc status: %s%s%s SeqNum: %#lx%s",
		...
		(CHECK_FLAG(on->dbdesc_bits, OSPF6_DBDESC_MSBIT) ? "Master"
								 : "Slave"),
		(u_long)ntohl(on->dbdesc_seqnum), VNL);
	...
	vty_out(vty, "    %d Pending LSAs for DbDesc in Time %s [thread %s]%s",
		...
		(on->thread_send_dbdesc ? "on" : "off"), VNL);
	...
	vty_out(vty, "    %d Pending LSAs for LSReq in Time %s [thread %s]%s",
		...
		(on->thread_send_lsreq ? "on" : "off"), VNL);
	for (lsa = ospf6_lsdb_head(on->request_list); lsa;
	     lsa = ospf6_lsdb_next(lsa))
		vty_out(vty, "      %s%s", lsa->name, VNL);
	...
	vty_out(vty,
		...
		(on->thread_send_lsupdate ? "on" : "off"), VNL);
	...
	vty_out(vty, "    %d Pending LSAs for LSAck in Time %s [thread %s]%s",
		...
		(on->thread_send_lsack ? "on" : "off"), VNL);
       ...
       "Display DR choices\n")
{
	...
	if (showfunc == ospf6_neighbor_show)
		vty_out(vty, "%-15s %3s %11s %8s/%-12s %11s %s[%s]%s",
			...
			"Duration", "I/F", "State", VNL);
	else if (showfunc == ospf6_neighbor_show_drchoice)
		vty_out(vty, "%-15s %8s/%-11s %-15s %-15s %s[%s]%s", "RouterID",
			"State", "Duration", "DR", "BDR", "I/F", "State", VNL);
      ...
      )
{
	...
	if ((inet_pton(AF_INET, argv[idx_ipv4]->arg, &router_id)) != 1) {
		vty_out(vty, "Router-ID is not parsable: %s%s",
