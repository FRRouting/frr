       ...
       V4NOTATION_STR)
{
	...
	if (ret == 0) {
		vty_out(vty, "malformed OSPF Router-ID: %s%s",
       ...
       "Delay in milliseconds\n")
{
	...
	if (argc == 5) {
		VTY_GET_INTEGER("LSA min-arrival", minarrival,
				argv[idx_number]->arg);
...
static void ospf6_show(struct vty *vty, struct ospf6 *o)
{
	...
	vty_out(vty,
		...
		o->spf_delay, VNL, o->spf_holdtime, VNL, o->spf_max_holdtime,
		VNL, o->spf_hold_multiplier, VNL);
	...
	if (o->ts_spf.tv_sec || o->ts_spf.tv_usec) {
		...
		vty_out(vty, "last executed %s ago, reason %s%s", buf, rbuf,
			VNL);
	...
	vty_out(vty, " SPF timer %s%s%s", (o->t_spf_calc ? "due in " : "is "),
		buf, VNL);
	...
	if (CHECK_FLAG(o->config_flags, OSPF6_LOG_ADJACENCY_CHANGES)) {
		if (CHECK_FLAG(o->config_flags, OSPF6_LOG_ADJACENCY_DETAIL))
			vty_out(vty, " All adjacency changes are logged%s",
				VTY_NEWLINE);
		else
...
static int ospf6_distance_config_write(struct vty *vty)
{
	...
	if (ospf6->distance_all)
		vty_out(vty, " distance %u%s", ospf6->distance_all,
			VTY_NEWLINE);
