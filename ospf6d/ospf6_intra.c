...
static int ospf6_router_lsa_show(struct vty *vty, struct ospf6_lsa *lsa)
{
	...
	for (current = start;
		...
		vty_out(vty, "    Type: %s Metric: %d%s", name,
			ntohs(lsdesc->metric), VNL);
...
static int ospf6_link_lsa_show(struct vty *vty, struct ospf6_lsa *lsa)
{
	...
	vty_out(vty, "     Priority: %d Options: %s%s", link_lsa->priority,
		options, VNL);
	...
	for (current = start; current < end;
		...
		vty_out(vty, "     Prefix: %s/%d%s", buf, prefix->prefix_length,
			VNL);
	}
