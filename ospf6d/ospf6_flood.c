...
void ospf6_install_lsa(struct ospf6_lsa *lsa)
{
	...
	if (!OSPF6_LSA_IS_MAXAGE(lsa))
		lsa->expire = thread_add_timer(
			master, ospf6_lsa_expire, lsa,
				  ...
				  struct ospf6_interface *oi)
{
	...
	for (ALL_LIST_ELEMENTS(oi->neighbor_list, node, nnode, on)) {
		...
		if (on->thread_send_lsupdate == NULL)
			on->thread_send_lsupdate = thread_add_timer(
				master, ospf6_lsupdate_send_neighbor, on,
	...
	if ((oi->type == OSPF_IFTYPE_BROADCAST)
		...
		if (oi->thread_send_lsupdate == NULL)
			oi->thread_send_lsupdate = thread_add_event(
				master, ospf6_lsupdate_send_interface, oi, 0);
	} else {
		/* reschedule retransmissions to all neighbors */
		for (ALL_LIST_ELEMENTS(oi->neighbor_list, node, nnode, on)) {
			THREAD_OFF(on->thread_send_lsupdate);
			on->thread_send_lsupdate = thread_add_event(
				master, ospf6_lsupdate_send_neighbor, on, 0);
		}
	}
}
					   ...
					   struct ospf6_neighbor *from)
{
	...
	if (ismore_recent < 0) {
		if (oi->drouter == from->router_id) {
			...
			if (oi->thread_send_lsack == NULL)
				oi->thread_send_lsack = thread_add_timer(
					master, ospf6_lsack_send_interface, oi,
					3);
		} else {
			if (is_debug)
				zlog_debug(
	...
	if (CHECK_FLAG(lsa->flag, OSPF6_LSA_DUPLICATE)
		...
		if (from->thread_send_lsack == NULL)
			from->thread_send_lsack = thread_add_event(
				master, ospf6_lsack_send_neighbor, from, 0);
		return;
	}
					   ...
					   struct ospf6_neighbor *from)
{
	...
	if (ismore_recent < 0) {
		...
		if (oi->thread_send_lsack == NULL)
			oi->thread_send_lsack = thread_add_timer(
				master, ospf6_lsack_send_interface, oi, 3);
		return;
	}
		       ...
		       struct ospf6_lsa_header *lsa_header)
{
	...
	if (old == NULL || ismore_recent < 0) {
		...
		if (new->header->adv_router
			...
			new->refresh = thread_add_event(
				master, ospf6_lsa_refresh, new, 0);
		}
	...
	/* (8) previous database copy is more recent */
	{
		...
		/* Otherwise, Send database copy of this LSA to this neighbor */
		{
			...
			if (from->thread_send_lsupdate == NULL)
				from->thread_send_lsupdate = thread_add_event(
