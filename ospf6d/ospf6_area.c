...
void ospf6_area_show(struct vty *vty, struct ospf6_area *oa)
{
	...
	if (!IS_AREA_STUB(oa))
		...
		if (oa->no_summary) {
			vty_out(vty, " Area %s[Stub, No Summary]%s", oa->name,
				VNL);
		} else {
			vty_out(vty, " Area %s[Stub]%s", oa->name, VNL);
		}
	}
	vty_out(vty, "     Number of Area scoped LSAs is %u%s", oa->lsdb->count,
		VNL);
	...
	if (oa->ts_spf.tv_sec || oa->ts_spf.tv_usec) {
		...
		if (result / TIMER_SECOND_MICRO > 0) {
			...
			vty_out(vty, "SPF last executed %ldus ago%s", result,
				VTY_NEWLINE);
		}
	} else
       ...
       "Advertised metric for this range\n")
{
	...
	if (ret != 1 || prefix.family != AF_INET6) {
		vty_out(vty, "Malformed argument: %s%s",
			argv[idx_ipv6_prefixlen]->arg, VNL);
		return CMD_SUCCESS;
	}
	...
	if (argc > idx_type) {
		if (strmatch(argv[idx_type]->text, "not-advertise")) {
			...
			VTY_GET_INTEGER_RANGE("cost", cost, argv[5]->arg, 0,
					      OSPF_LS_INFINITY);
			UNSET_FLAG(range->flag, OSPF6_ROUTE_DO_NOT_ADVERTISE);
		}
	}
       ...
       "Advertised metric for this range\n")
{
	...
	if (ret != 1 || prefix.family != AF_INET6) {
		vty_out(vty, "Malformed argument: %s%s", argv[idx_ipv6]->arg,
			VNL);
		return CMD_SUCCESS;
	}
	...
	range = ospf6_route_lookup(&prefix, oa->range_table);
	if (range == NULL) {
		vty_out(vty, "Range %s does not exists.%s", argv[idx_ipv6]->arg,
			VNL);
		return CMD_SUCCESS;
	}
...
void ospf6_area_config_write(struct vty *vty)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa)) {
		...
		if (IS_AREA_STUB(oa)) {
			if (oa->no_summary)
				vty_out(vty, " area %s stub no-summary%s",
					oa->name, VNL);
			else
				vty_out(vty, " area %s stub%s", oa->name, VNL);
		}
		...
		if (IMPORT_NAME(oa))
			vty_out(vty, " area %s import-list %s%s", oa->name,
				IMPORT_NAME(oa), VNL);
		if (EXPORT_NAME(oa))
			vty_out(vty, " area %s export-list %s%s", oa->name,
				EXPORT_NAME(oa), VNL);
	}
}
       ...
       "Show SPF tree\n")
{
	...
	if (inet_pton(AF_INET, argv[idx_ipv4]->arg, &area_id) != 1) {
		vty_out(vty, "Malformed Area-ID: %s%s", argv[idx_ipv4]->arg,
			VNL);
		return CMD_SUCCESS;
	}
       ...
       OSPF6_AREA_ID_STR)
{
	...
	if (inet_pton(AF_INET, argv[idx_ipv4_2]->arg, &area_id) != 1) {
		vty_out(vty, "Malformed Area-ID: %s%s", argv[idx_ipv4_2]->arg,
			VNL);
		return CMD_SUCCESS;
	}
	oa = ospf6_area_lookup(area_id, ospf6);
