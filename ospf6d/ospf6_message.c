				     ...
				     struct ospf6_neighbor *on)
{
	...
	if (!CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MBIT)
		...
		on->thread_send_dbdesc = thread_add_event(
			master, ospf6_dbdesc_send_newone, on, 0);
...
int ospf6_hello_send(struct thread *thread)
{
	...
	/* set next thread */
	oi->thread_send_hello = thread_add_timer(master, ospf6_hello_send, oi,
						 oi->hello_interval);
...
int ospf6_lsack_send_neighbor(struct thread *thread)
{
	...
	for (lsa = ospf6_lsdb_head(on->lsack_list); lsa;
		...
		if (p - sendbuf + sizeof(struct ospf6_lsa_header)
			...
			on->thread_send_lsack = thread_add_event(
				master, ospf6_lsack_send_neighbor, on, 0);
...
int ospf6_lsack_send_interface(struct thread *thread)
{
	...
	for (lsa = ospf6_lsdb_head(oi->lsack_list); lsa;
		...
		if (p - sendbuf + sizeof(struct ospf6_lsa_header)
			...
			oi->thread_send_lsack = thread_add_event(
				master, ospf6_lsack_send_interface, oi, 0);
