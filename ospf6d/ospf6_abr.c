...
bool ospf6_check_and_set_router_abr(struct ospf6 *o)
{
	...
	for (ALL_LIST_ELEMENTS_RO(o->area_list, node, oa)) {
		zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "%s, area_id %pI4",
				__func__, &oa->area_id);
	...
	if ((area_count > 1) && (is_backbone)) {
		zlog_debug_cond(IS_OSPF6_DEBUG_ABR,
					...
					struct ospf6_area *area)
{
	...
	zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "%s : finish area %s", __func__,
			area->name);
...
void ospf6_abr_range_update(struct ospf6_route *range, struct ospf6 *ospf6)
{
	...
	zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "%s: range %pFX, cost %d", __func__,
			&range->prefix, cost);
	...
	zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "%s: range %pFX update", __func__,
			&range->prefix);
	...
	if (CHECK_FLAG(range->flag, OSPF6_ROUTE_ACTIVE_SUMMARY)
		...
		if (!CHECK_FLAG(range->flag, OSPF6_ROUTE_BLACKHOLE_ADDED)) {
			zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "Add discard route");
...
void ospf6_abr_old_route_remove(struct ospf6_lsa *lsa, struct ospf6_route *old,
				struct ospf6_route_table *table)
{
	zlog_debug_cond(IS_OSPF6_DEBUG_ABR, "%s: route %pFX, paths %d",
			__func__, &old->prefix, listcount(old->paths));
...
void ospf6_abr_examin_summary(struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
	...
	if (route) {
		...
		zlog_debug_cond(is_debug, "%s: route %pFX, paths %d", __func__,
	...
	if (cost == OSPF_LS_INFINITY) {
		zlog_debug_cond(is_debug, "cost is LS_INFINITY, ignore");
		if (old)
			ospf6_abr_old_route_remove(lsa, old, table);
		return;
	}
	if (OSPF6_LSA_IS_MAXAGE(lsa)) {
		zlog_debug_cond(is_debug, "%s: LSA %s is MaxAge, ignore",
				__func__, lsa->name);
		if (old)
			ospf6_abr_old_route_remove(lsa, old, table);
		return;
	...
	if (lsa->header->adv_router == oa->ospf6->router_id) {
		zlog_debug_cond(is_debug, "LSA %s is self-originated, ignore",
				lsa->name);
		if (old)
			ospf6_route_remove(old, table);
		return;
	...
	*/
	...
	zlog_debug_cond(old && is_debug, "%s: found old route %pFX, paths %d",
...
void ospf6_abr_prefix_resummarize(struct ospf6 *o)
{
	...
	zlog_debug_cond(IS_OSPF6_DEBUG_ABR,
			"Re-examining Inter-Prefix Summaries");
