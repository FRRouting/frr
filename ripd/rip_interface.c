	...
	if (ri->enable_network || ri->enable_interface) {
		{
			...
			if (!ri->t_wakeup)
				ri->t_wakeup = thread_add_timer(
					master, rip_interface_wakeup, ifp, 1);
       ...
       "Interface name\n")
{
	...
	if (ret < 0) {
		vty_out(vty, "There is a same network configuration %s%s",
			argv[idx_ipv4_word]->arg, VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Interface name\n")
{
	...
	if (ret < 0) {
		vty_out(vty, "Can't find network configuration %s%s",
			argv[idx_ipv4_word]->arg, VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Old ripd compatible\n")
{
	...
	if (argc > 5) {
		if (auth_type != RIP_AUTH_MD5) {
			vty_out(vty,
				"auth length argument only valid for md5%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
