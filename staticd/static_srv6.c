...
>>>>>>> d763f236a (staticd: Move SID interface dependency check to separate function)
			if (is_up) {
...
=======
		...
		if (is_up && !CHECK_FLAG(sid->flags, STATIC_FLAG_SRV6_SID_VALID)) {
			...
			static_zebra_srv6_sid_uninstall(sid);
			UNSET_FLAG(sid->flags, STATIC_FLAG_SRV6_SID_SENT_TO_ZEBRA);
>>>>>>> 20b07b8b2 (staticd: Refactor and add comments to SID interface dependency logic)
		}
	}
}
