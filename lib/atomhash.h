 ...
 */
/* Note this header file is under ISC license while the actual atomhash code
 * is GPL licensed.  This is to make clear that other source code may freely
 * _include_ this header, and GPL starts applying when the atomhash code is
 * _linked_.)
 */
...
struct atomhash_head {
	...
	/* disable grow & shrink.
	 *
	 * This is primarily used for testing.  It can theoretically be used in
	 * production code, but it'd be better to improve the common overall
	 * typesafe API instead.  (Or implement atomhash_pop_all() with it.)
	 * Either way, work for another time.
	 */
	bool freeze_size;
...
/* _init requires the caller to be exclusive owner of the struct, obviously */
extern void atomhash_init(struct atomhash_head *head);

/* WARNING: this does NOT complete immediately, it schedules an RCU operation!
 * that means the head:
 * - CANNOT be immediately reused with atomhash_init
 * - CANNOT be freed with XFREE, which also applies to the containing struct
 *   if it's embedded(!)
 *
 * rcu_free() is the correct thing to do.
 */
