...
int prefix_same(const struct prefix *p1, const struct prefix *p2)
{
        ...
        if (p1->family == p2->family && p1->prefixlen == p2->prefixlen) {
                if (p1->family == AF_INET)
                        if (IPV4_ADDR_SAME
                            (&p1->u.prefix4.s_addr, &p2->u.prefix4.s_addr))
                                return 1;
                if (p1->family == AF_INET6)
                        if (IPV6_ADDR_SAME
                            (&p1->u.prefix6.s6_addr,
                             &p2->u.prefix6.s6_addr))
                                return 1;
                if (p1->family == AF_ETHERNET)
                        if (!memcmp
                            (&p1->u.prefix_evpn, &p2->u.prefix_evpn,
                             sizeof(struct evpn_addr)))
                                return 1;
        }
        return 0;
                                    ...
                                    struct prefix *prefix)
{
        if (su->sa.sa_family == AF_INET) {
                ...
                p = prefix ? (struct prefix_ipv4 *) prefix :
                    prefix_ipv4_new();
                p->family = AF_INET;
        ...
        if (su->sa.sa_family == AF_INET6) {
                ...
                p = prefix ? (struct prefix_ipv6 *) prefix :
                    prefix_ipv6_new();
                p->family = AF_INET6;
                p->prefixlen = IPV6_MAX_BITLEN;
...
const char *prefix2str(union prefixconstptr pu, char *str, int size)
{
        ...
        case AF_ETHERNET:
                if (p->u.prefix_evpn.route_type == 5) {
                        ...
                        family =
                            (p->u.prefix_evpn.
                             flags & (IP_ADDR_V4 | IP_PREFIX_V4)) ? AF_INET
                            : AF_INET6;
                        snprintf(str, size, "[%d]:[%u][%s]/%d",
                                 p->u.prefix_evpn.route_type,
                                 p->u.prefix_evpn.eth_tag,
                                 inet_ntop(family,
                                           &p->u.prefix_evpn.ip.addr, buf,
                                           PREFIX2STR_BUFFER),
                                 p->prefixlen);
                } else {
...
char *prefix_mac2str(const struct ethaddr *mac, char *buf, int size)
{
        ...
        snprintf(ptr, (ETHER_ADDR_STRLEN),
                 ...
                 (uint8_t) mac->octet[1], (uint8_t) mac->octet[2],
                 (uint8_t) mac->octet[3], (uint8_t) mac->octet[4],
                 (uint8_t) mac->octet[5]);
        return ptr;
}
