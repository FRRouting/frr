...
static bool grpc_is_running(void)
{
	...
	running = grpc_state == GRPC_STATE_RUNNING;
...
static bool grpc_is_shutting_down(void)
{
	...
	pthread_mutex_lock(&s_server_lock);
	shutting_down = grpc_state == GRPC_STATE_SHUTDOWN;
	pthread_mutex_unlock(&s_server_lock);
...
static struct lyd_node *select_config_get_result(struct lyd_node *dnode, const std::string &path)
{
...
class CommitRpcState : public RpcStateBase {
	...
	CallState run_mainthread(struct event *event) override
	{
		...
		ret = nb_config_commit_dispatch_async(candidate->config, nb_phase,
						      request.comment().c_str(), async_done, this,
						      errmsg, sizeof(errmsg));
		if (!ret) {
			async_pending = true;
			return MORE;
