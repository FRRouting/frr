...
static int zapi_nhg_child_encode(struct stream *s, int cmd, struct zapi_nhg_group *api_nhg_group)
{
	int i;

	if (cmd != ZEBRA_NHG_CHILD_ADD) {
		flog_err(EC_LIB_ZAPI_ENCODE, "%s: Specified zapi NHG command (%d) doesn't exist",
			 __func__, cmd);
		return -1;
	}
...
static int zapi_nhg_encode(struct stream *s, int cmd, struct zapi_nhg *api_nhg)
{
	...
	if (cmd == ZEBRA_NHG_ADD) {
		...
		for (i = 0; i < api_nhg->nexthop_num; i++)
			zapi_nexthop_encode(s, &api_nhg->nexthops[i], 0, api_nhg->message);

		/* Backup nexthops */
		stream_putw(s, api_nhg->backup_nexthop_num);

		for (i = 0; i < api_nhg->backup_nexthop_num; i++)
			zapi_nexthop_encode(s, &api_nhg->backup_nexthops[i], 0, api_nhg->message);
	}
...
enum zclient_send_status zclient_nhg_child_send(struct zclient *zclient, int cmd,
