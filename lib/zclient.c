 ...
 */
int zapi_srv6_locator_static_sids_encode(struct stream *s, struct srv6_locator *loc,
 ...
 */
int zapi_srv6_locator_static_sids_decode(struct stream *s, struct srv6_locator *loc,
					 struct list *static_sids_list)
{
	struct srv6_sid *static_sid = NULL;
	uint16_t len = 0;
...
int srv6_manager_get_locator_static_sids(struct zclient *zclient, const char *locator_name)
{
	...
	if (zclient->sock < 0) {
		flog_err(EC_LIB_ZAPI_SOCKET, "%s: invalid zclient socket", __func__);
		return -1;
	}
	...
	s = zclient->obuf;
	stream_reset(s);
	zclient_create_header(s, ZEBRA_SRV6_MANAGER_GET_LOCATOR_STATIC_SIDS, VRF_DEFAULT);
