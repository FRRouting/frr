...
static struct prefix_list_entry *prefix_list_entry_new(void)
{
        ...
        new =
            XCALLOC(MTYPE_PREFIX_LIST_ENTRY,
...
static struct prefix_list *prefix_list_get(afi_t afi, int orf,
                                           const char *name)
{
        struct prefix_list *plist;
...
static struct prefix_list_entry *prefix_list_entry_make(struct prefix
                                                        *prefix,
                                                        enum
                                                        prefix_list_type
                                                        type, int seq,
...
static struct prefix_list_entry *prefix_seq_check(struct prefix_list
                                                  *plist, int seq)
{
        struct prefix_list_entry *pentry;
...
static struct prefix_list_entry *prefix_list_entry_lookup(struct
                                                          prefix_list
                                                          *plist,
                                                          struct prefix
                                                          *prefix,
...
static void
trie_walk_affected(size_t validbits, struct pltrie_table *table,
...
static int trie_table_empty(struct pltrie_table *table)
{
        size_t i;
        for (i = 0; i < PLC_LEN; i++)
                if (table->entries[i].next_table
                    || table->entries[i].up_chain)
                        return 0;
        return 1;
}
                     ...
                     struct prefix_list_entry *pentry)
{
        ...
        table = plist->trie;
        for (depth = 0; validbits > PLC_BITS && depth < maxdepth - 1;
             depth++) {
                     ...
                     struct prefix_list_entry *pentry)
{
        ...
        trie_walk_affected(validbits, table, *bytes, pentry,
                           trie_install_fn);
}
...
static struct prefix_list_entry *prefix_entry_dup_check(struct prefix_list
                                                        *plist,
                                                        struct
                                                        prefix_list_entry
                                                        *new)
                        ...
                        const char *prefix, const char *ge, const char *le)
{
        ...
        if (strncmp("permit", typestr, 1) == 0)
                ...
                vty_out(vty, "%% prefix type must be permit or deny%s",
                        VTY_NEWLINE);
                return CMD_WARNING;
        }
        ...
        case AFI_L2VPN:
        default:
                vty_out(vty, "%% Unrecognized AFI (%d)%s", afi,
                        VTY_NEWLINE);
                return CMD_WARNING;
                break;
        }
        ...
        /* Make prefix entry. */
        pentry =
            prefix_list_entry_make(&p, type, seqnum, lenum, genum, any);
                          ...
                          const char *le)
{
        ...
        if (!plist) {
                vty_out(vty, "%% Can't find specified prefix-list%s",
                        VTY_NEWLINE);
                return CMD_WARNING;
        }
        ...
        if ((typestr == NULL) || (prefix == NULL)) {
                vty_out(vty, "%% Both prefix and type required%s",
                        VTY_NEWLINE);
                return CMD_WARNING;
        }
        ...
        /* Lookup prefix entry. */
        pentry =
            prefix_list_entry_lookup(plist, &p, type, seqnum, lenum,
                                     genum);
...
static void
vty_show_prefix_entry(struct vty *vty, afi_t afi,
                      struct prefix_list *plist,
                      struct prefix_master *master,
                      enum display_type dtype, int seqnum)
{
        ...
        if (dtype == normal_display) {
                ...
                if (plist->desc)
                        vty_out(vty, "   Description: %s%s", plist->desc,
                                VTY_NEWLINE);
                ...
                vty_out(vty,
                        ...
                        plist->tail ? plist->tail->seq : 0, VTY_NEWLINE);
        }
                     ...
                     const char *seq, enum display_type dtype)
{
        ...
        if (name) {
                ...
                if (!plist) {
                        vty_out(vty,
                                "%% Can't find specified prefix-list%s",
                                VTY_NEWLINE);
                        return CMD_WARNING;
                }
                ...
                for (plist = master->num.head; plist; plist = plist->next)
                        vty_show_prefix_entry(vty, afi, plist, master,
                                              dtype, seqnum);
                      ...
                      const char *prefix)
{
        ...
        if (name == NULL && prefix == NULL) {
                ...
                if (prefix) {
                        ret = str2prefix(prefix, &p);
                        if (ret <= 0) {
                                vty_out(vty, "%% prefix is malformed%s",
                                        VTY_NEWLINE);
                                return CMD_WARNING;
                        }
                }
      ...
      "Any prefix match. Same as \"0.0.0.0/0 le 32\"\n")
{
        ...
        int idx_ipv4_any = 4;
        return vty_prefix_list_install(vty, AFI_IP, argv[idx_word]->arg,
                                       NULL, argv[idx_permit_deny]->arg,
                                       argv[idx_ipv4_any]->arg, NULL,
                                       NULL);
}
...
{
        int idx_word = 3;
        return vty_prefix_list_uninstall(vty, AFI_IP, argv[idx_word]->arg,
...
{
        int idx_word = 3;
        return vty_prefix_list_desc_unset(vty, AFI_IP,
                                          argv[idx_word]->arg);
}
...
DEFUN(show_ip_prefix_list,
      ...
      "show ip prefix-list", SHOW_STR IP_STR PREFIX_LIST_STR)
{
        return vty_show_prefix_list(vty, AFI_IP, NULL, NULL,
                                    normal_display);
}
...
{
        int idx_word = 3;
        return vty_show_prefix_list(vty, AFI_IP, argv[idx_word]->arg, NULL,
                                    normal_display);
}
      ...
      "sequence number of an entry\n" "Sequence number\n")
{
        int idx_word = 3;
        int idx_number = 5;
        return vty_show_prefix_list(vty, AFI_IP, argv[idx_word]->arg,
                                    argv[idx_number]->arg,
                                    sequential_display);
}
      ...
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n")
{
        int idx_word = 3;
        int idx_ipv4_prefixlen = 4;
        return vty_show_prefix_list_prefix(vty, AFI_IP,
                                           argv[idx_word]->arg,
                                           argv[idx_ipv4_prefixlen]->arg,
                                           normal_display);
}
...
DEFUN(clear_ip_prefix_list,
      ...
      "clear ip prefix-list", CLEAR_STR IP_STR PREFIX_LIST_STR)
{
...
{
        int idx_word = 3;
        return vty_clear_prefix_list(vty, AFI_IP, argv[idx_word]->arg,
                                     NULL);
}
      ...
      "Any prefix match.  Same as \"::0/0 le 128\"\n")
{
        ...
        int idx_permit_deny = 3;
        int idx_ipv6_any = 4;
        return vty_prefix_list_install(vty, AFI_IP6, argv[idx_word]->arg,
                                       NULL, argv[idx_permit_deny]->arg,
                                       argv[idx_ipv6_any]->arg, NULL,
                                       NULL);
}
      ...
      "Maximum prefix length to be matched\n" "Maximum prefix length\n")
{
        ...
        return vty_prefix_list_install(vty, AFI_IP6, argv[idx_word]->arg,
                                       NULL, argv[idx_permit_deny]->arg,
                                       argv[idx_ipv6_prefixlen]->arg, NULL,
                                       argv[idx_number]->arg);
}
...
{
        int idx_word = 3;
        return vty_prefix_list_uninstall(vty, AFI_IP6, argv[idx_word]->arg,
                                         NULL, NULL, NULL, NULL, NULL);
}
...
{
        int idx_word = 3;
        return vty_prefix_list_desc_unset(vty, AFI_IP6,
                                          argv[idx_word]->arg);
}
...
DEFUN(show_ipv6_prefix_list,
      ...
      "show ipv6 prefix-list", SHOW_STR IPV6_STR PREFIX_LIST_STR)
{
        return vty_show_prefix_list(vty, AFI_IP6, NULL, NULL,
                                    normal_display);
}
...
{
        int idx_word = 3;
        return vty_show_prefix_list(vty, AFI_IP6, argv[idx_word]->arg,
                                    NULL, normal_display);
}
      ...
      "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")
{
        int idx_word = 3;
        int idx_ipv6_prefixlen = 4;
        return vty_show_prefix_list_prefix(vty, AFI_IP6,
                                           argv[idx_word]->arg,
                                           argv[idx_ipv6_prefixlen]->arg,
                                           normal_display);
}
...
DEFUN(clear_ipv6_prefix_list,
      ...
      "clear ipv6 prefix-list", CLEAR_STR IPV6_STR PREFIX_LIST_STR)
{
...
{
        int idx_word = 3;
        return vty_clear_prefix_list(vty, AFI_IP6, argv[idx_word]->arg,
                                     NULL);
}
...
struct stream *prefix_bgp_orf_entry(struct stream *s,
                                    struct prefix_list *plist,
                   ...
                   int permit, int set)
{
        ...
        if (set) {
                pentry = prefix_list_entry_make(&orfp->p,
                                                (permit ? PREFIX_PERMIT :
                                                 PREFIX_DENY), orfp->seq,
                                                orfp->le, orfp->ge, 0);
                            ...
                            u_char use_json)
{
        ...
        if (use_json) {
                ...
                for (pentry = plist->head; pentry; pentry = pentry->next) {
                        ...
                        json_object_string_add(json_list,
                                               "seqPrefixListType",
                                               prefix_list_type_str
                                               (pentry));
                        ...
                        json_object_object_add(json_prefix, buf_a,
                                               json_list);
                }
                if (afi == AFI_IP)
