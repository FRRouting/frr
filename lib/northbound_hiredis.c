...
static const char *hiredis_sub_channels[][MAXIMUM_CHANNEL_NUM] = { { "BABEL", "frr-babel", NULL },
								   { "BFD", "frr-bfd", NULL },
								   { "BGP", "frr-bgp", NULL },
...
const char *hi_change_oper_str[] = { "create", "modify", "delete", "move", "select", "unknow" };
...
static void hiredis_edit_diff_get_origin(const struct lyd_node *node, char **origin,
					 int *origin_own);
...
static bool frr_hiredis_get_path(const char *xpath, LYD_FORMAT lyd_format, bool with_defaults);

/* cuda operation api */
static LY_ERR hiredis_data_tree_from_dnode(const struct lyd_node *dnode, LYD_FORMAT lyd_format,
					   bool with_defaults);
...
static bool frr_hiredis_get_path(const char *xpath, LYD_FORMAT lyd_format, bool with_defaults)
{
	...
	if (lyd_merge_siblings(&dnode_state, dnode_config, LYD_MERGE_DESTRUCT) != LY_SUCCESS) {
	...
	LY_ERR err = lyd_validate_all(&dnode_final, ly_native_ctx, 0, NULL);

	if (err) {
		flog_err(EC_LIB_HIREDIS_INIT, "%s validate lyd_validate_all() failed: %s", __func__,
		...
		if (parent->schema) {
			...
			LY_LIST_FOR (((struct lyd_node_opaq *)parent)->attr, a) {
				...
				if (!strcmp(a->name.name, "origin")) {
					...
					if (!lyrc) {
						if (!strcmp(attr_meta->annotation->module->name,
							    "ietf-origin")) {
							meta = attr_meta;
							break;
						} else {
...
store_value:
		...
		case LY_TYPE_BINARY:
			...
			sr_val->data.binary_val =
				strdup(lyd_value_get_canonical(LYD_CTX(node), val));
		...
		case LY_TYPE_DEC64:
			...
			sr_val->data.decimal64_val =
				strtod(lyd_value_get_canonical(LYD_CTX(node), val), &ptr);
			if (ptr[0]) {
				goto error;
			}
			break;
		...
		case LY_TYPE_IDENT:
			...
			sr_val->data.identityref_val =
				strdup(lyd_value_get_canonical(LYD_CTX(node), val));
			break;
		case LY_TYPE_INST:
			sr_val->type = HI_INSTANCEID_T;
			sr_val->data.instanceid_val =
				strdup(lyd_value_get_canonical(LYD_CTX(node), val));
			break;
		case LY_TYPE_INT8:
	...
	case LYS_ANYDATA:
		...
		case LYD_ANYDATA_LYB:
			...
			if (lyd_parse_data_mem(LYD_CTX(node), any->value.mem, LYD_LYB,
					       LYD_PARSE_STRICT, 0, &tree)) {
				      ...
				      hi_val_t *sr_old_val, hi_val_t *sr_new_val)
{
	...
	DEBUGD(&nb_dbg_client_hiredis, "hiredis: processing change [xpath %s]", xpath);
	...
	case HI_OP_MOVED:
		...
		break;
	default:
		flog_err(EC_LIB_DEVELOPMENT, "%s: unexpected operation %u [xpath %s]", __func__,
			 sr_op, xpath);
		return NB_ERR;
	}
...
static bool frr_hiredis_change_apply(struct nb_config *candidate)
{
	...
	ret = nb_candidate_commit_prepare(context, candidate, NULL, &transaction, false, false,
	...
	if (transaction) {
		...
		nb_candidate_commit_apply(transaction, true, NULL, errmsg, sizeof(errmsg));
		nb_config_free(can);
	}
...
static void hiredis_match_event(struct event *event)
{
	...
	if (hiredis_get_operation(str, &operation)) {
		hiredis_config_change(XSTRDUP(MTYPE_HIREDIS,
					      str + strlen(hi_change_oper_str[operation]) + 1),
				      LYD_JSON, operation);
...
static int frr_hiredis_init(void)
{
	...
	if (frr_pthread_run(fpt, NULL) < 0) {
		flog_err(EC_LIB_SYSTEM_CALL, "%s: error creating pthread: %s", __FUNCTION__,
			 safe_strerror(errno));
		return ret;
	}
...
static void frr_hiredis_module_very_late_init(struct event *thread)
{
	...
	if (args) {
		...
		if (port < 1024 || port > UINT16_MAX) {
			flog_err(EC_LIB_HIREDIS_INIT, "%s: port number must be between 1025 and %d",
				 __func__, UINT16_MAX);
			goto error;
		}
	}
...
FRR_MODULE_SETUP(.name = "frr_hiredis", .version = FRR_VERSION,
		 .description = "FRR hiredis northbound module", .init = frr_hiredis_module_init, );
