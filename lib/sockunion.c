...
const char *inet_sutop(const union sockunion *su, char *str)
{
        ...
        case AF_INET:
                inet_ntop(AF_INET, &su->sin.sin_addr, str,
                          INET_ADDRSTRLEN);
                break;
        case AF_INET6:
                inet_ntop(AF_INET6, &su->sin6.sin6_addr, str,
                          INET6_ADDRSTRLEN);
                break;
        }
        return str;
...
static void sockunion_normalise_mapped(union sockunion *su)
{
        ...
        if (su->sa.sa_family == AF_INET6
            && IN6_IS_ADDR_V4MAPPED(&su->sin6.sin6_addr)) {
                ...
                memcpy(&sin.sin_addr, ((char *) &su->sin6.sin6_addr) + 12,
                       4);
                                 ...
                                 size_t len)
{
        ...
        case AF_INET6:
                return inet_ntop(AF_INET6, &(su->sin6.sin6_addr), buf,
                                 len);
                break;
...
enum connect_result
sockunion_connect(int fd, const union sockunion *peersu,
                  unsigned short port, ifindex_t ifindex)
{
        ...
        case AF_INET6:
                ...
                if (IN6_IS_ADDR_LINKLOCAL(&su.sin6.sin6_addr) && ifindex) {
                        ...
                        SET_IN6_LINKLOCAL_IFINDEX(su.sin6.sin6_addr,
                                                  ifindex);
                }
#endif                          /* KAME */
                break;
...
int sockopt_reuseaddr(int sock)
{
        ...
        ret = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR,
                         (void *) &on, sizeof(on));
        if (ret < 0) {
                zlog_warn("can't set sockopt SO_REUSEADDR to socket %d",
                          sock);
                return -1;
        }
        return 0;
...
int sockopt_reuseport(int sock)
{
        ...
        ret = setsockopt(sock, SOL_SOCKET, SO_REUSEPORT,
                         (void *) &on, sizeof(on));
        if (ret < 0) {
                zlog_warn("can't set sockopt SO_REUSEPORT to socket %d",
                          sock);
                return -1;
        }
        return 0;
...
{
#ifdef TCP_CORK
        return setsockopt(sock, IPPROTO_TCP, TCP_CORK, &onoff,
                          sizeof(onoff));
#else
        return 0;
#endif
...
int sockopt_minttl(int family, int sock, int minttl)
{
        ...
        if (family == AF_INET) {
                int ret =
                    setsockopt(sock, IPPROTO_IP, IP_MINTTL, &minttl,
                               sizeof(minttl));
                if (ret < 0)
                        zlog_warn
        ...
        if (family == AF_INET6) {
                int ret =
                    setsockopt(sock, IPPROTO_IPV6, IPV6_MINHOPCOUNT,
...
int sockopt_v6only(int family, int sock)
{
        ...
        if (family == AF_INET6) {
                ret = setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY,
                                 (void *) &on, sizeof(int));
                if (ret < 0) {
                        zlog_warn("can't set sockopt IPV6_V6ONLY "
                                  "to socket %d", sock);
                        return -1;
                }
                return 0;
...
int sockunion_same(const union sockunion *su1, const union sockunion *su2)
{
        ...
        case AF_INET6:
                ret = memcmp(&su1->sin6.sin6_addr, &su2->sin6.sin6_addr,
                             sizeof(struct in6_addr));
                if ((ret == 0)
                    && IN6_IS_ADDR_LINKLOCAL(&su1->sin6.sin6_addr)) {
                        /* compare interface indices */
                        if (su1->sin6.sin6_scope_id
                            && su2->sin6.sin6_scope_id)
                                ret =
                                    (su1->sin6.sin6_scope_id ==
                                     su2->sin6.sin6_scope_id) ? 0 : 1;
                }
                break;
        }
        if (ret == 0)
...
unsigned int sockunion_hash(const union sockunion *su)
{
        ...
        case AF_INET6:
                return jhash2(su->sin6.sin6_addr.s6_addr32,
                              ZEBRA_NUM_OF(su->sin6.sin6_addr.s6_addr32),
                              0);
        }
        return 0;
}
...
int sockunion_cmp(const union sockunion *su1, const union sockunion *su2)
{
        ...
        if (su1->sa.sa_family == AF_INET6)
                return in6addr_cmp(&su1->sin6.sin6_addr,
                                   &su2->sin6.sin6_addr);
        return 0;
}
