				    ...
				    const struct prefix *dst)
{
	...
	if (filter->extended) {
		...
		for (i = 0; i < sizeof(dst_m) / 4; i++)
			dst_m.s6_addr32[i] &= ~filter->sadr6.dst_mask.s6_addr32[i];

		if (memcmp(&src_m, &filter->sadr6.src, sizeof(src_m)) == 0 &&
		    memcmp(&dst_m, &filter->sadr6.dst, sizeof(dst_m)) == 0)
			return 1;
	} else {
		struct in6_addr check_addr = {};
		...
		if (memcmp(&check_addr, &filter->sadr6.src, sizeof(check_addr)) == 0)
			return 1;
	}
