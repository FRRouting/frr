	...
	"Virtual Private LAN Service\n")
{
	...
	snprintf(xpath, sizeof(xpath), "/frr-l2vpn:l2vpn/l2vpn-instance[name='%s']", l2vpn_name);
	...
	"Interface's name\n")
{
	...
	snprintf(xpath_index, sizeof(xpath_index), "%s/member-interface[interface='%s']",
		 VTY_CURR_XPATH, ifname);
...
static int l2vpn_config_write(struct vty *vty)
{
	const struct lyd_node *dnode;
	int written = 0;

	dnode = yang_dnode_get(running_config->dnode, "/frr-l2vpn:l2vpn/l2vpn-instance");
	if (dnode) {
...
struct cmd_node l2vpn_node = {
	...
	.config_write = l2vpn_config_write,
};
...
static void l2vpn_init_new(bool in_backend)
{
	if (!in_backend) {
