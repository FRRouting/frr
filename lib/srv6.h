...
struct seg6local_context {
	...
	uint32_t table;
	ifindex_t ifindex;
	/* output interface for End.DX2/DT2U/DT2M  */
	ifindex_t oif;
...
enum srv6_endpoint_behavior_codepoint {
	...
	SRV6_ENDPOINT_BEHAVIOR_UDX2 = 0x0041,
	/* 0x0042 is uDX2V (IANA); not used here, do not reuse for another behavior */
	SRV6_ENDPOINT_BEHAVIOR_UDT2U = 0x0043,
	SRV6_ENDPOINT_BEHAVIOR_UDT2M = 0x0044,
					   ...
					   const struct srv6_sid_ctx *ctx)
{
	...
	case ZEBRA_SEG6_LOCAL_ACTION_END_DX2:
		snprintfrr(str + len, size - len, " oif_idx %u", ctx->oif);
		break;

	case ZEBRA_SEG6_LOCAL_ACTION_END_DT2U:
		snprintfrr(str + len, size - len, " oif_idx %u vni %u", ctx->oif, ctx->dt2_vni);
		break;

	case ZEBRA_SEG6_LOCAL_ACTION_END_DT2M:
		snprintfrr(str + len, size - len, " oif_idx %u vni %u (BUM)", ctx->oif,
			   ctx->dt2_vni);
		break;
