 ...
 */
void nexthop_group_append_nexthops(struct nexthop_group *to, const struct nexthop_group *from)
{
...
static struct nexthop_group_cmd *nhgc_get(const char *name)
{
	...
	if (!nhgc) {
		...
		nhgc->nhg_child_group_list->cmp = (int (*)(void *, void *))nhgl_child_group_cmp;
		nhgc->nhg_child_group_list->del = (void (*)(void *))nhgl_child_group_delete;
 ...
 */
static bool nexthop_group_unsave_child_group(struct nexthop_group_cmd *nhgc, const char *group_name)
{
	char *child_group_name;
	struct listnode *node, *nnode;

	for (ALL_LIST_ELEMENTS(nhgc->nhg_child_group_list, node, nnode, child_group_name)) {
...
static bool nexthop_group_save_child_group(struct nexthop_group_cmd *nhgc, const char *group_name)
{
	char *child_group_name;
      ...
      "The name of the group\n")
{
	...
	if (listcount(nhgc->nhg_list)) {
		vty_out(vty, "%% child nexthop group not possible when 'nexthop' is configured\n");
...
{
	if (enable == !!CHECK_FLAG(nhgc->nhg.message, NEXTHOP_GROUP_MESSAGE_SRTE))
		return;
...
static void nexthop_group_save_nhop(struct nexthop_group_cmd *nhgc, const char *nhvrf_name,
				    const union sockunion *addr, const char *intf, bool onlink,
 ...
 */
static bool nexthop_group_parse_nexthop(struct nexthop *nhop, const union sockunion *addr,
					const char *intf, bool onlink, const char *name,
					const char *labels, vni_t vni, int *lbl_ret,
					uint32_t weight, const char *backup_str, uint32_t color)
{
	int ret = 0;
	struct vrf *vrf;
	int num;
...
{
	return (nexthop_group_parse_nexthop(nhop, nhh->addr, nhh->intf, nhh->onlink,
					    nhh->nhvrf_name, nhh->labels, nhh->vni, NULL,
					    nhh->weight, nhh->backup_str, nhh->color));
}
      ...
      SRTE_COLOR_STR)
{
	...
	if (listcount(nhgc->nhg_child_group_list)) {
		vty_out(vty, "%% nexthop not possible when 'child-group' is configured\n");
		return CMD_WARNING_CONFIG_FAILED;
	}
	...
	legal = nexthop_group_parse_nexthop(&nhop, addr, intf, !!onlink, vrf_name, label, vni,
					    &lbl_ret, weight, backup_idx, color);
	...
	if (yes) {
		...
		/* Save config always */
		nexthop_group_save_nhop(nhgc, vrf_name, addr, intf, !!onlink, label, weight,
					backup_idx, color);
...
static int nexthop_group_write(struct vty *vty)
{
	...
	RB_FOREACH (nhgc, nhgc_entry_head, &nhgc_entries) {
		...
		for (ALL_LIST_ELEMENTS_RO(nhgc->nhg_child_group_list, node, child_group))
...
void nexthop_group_disable_vrf(struct vrf *vrf)
{
	...
	RB_FOREACH (nhgc, nhgc_entry_head, &nhgc_entries) {
		...
		for (ALL_LIST_ELEMENTS(nhgc->nhg_child_group_list, node, nnode, child_group)) {
			if (nhg_hooks.del_nexthop_or_child_group)
				nhg_hooks.del_nexthop_or_child_group(nhgc, NULL);
...
void nexthop_group_child_group_match(const char *nhgc_name,
				     void (*cb_func)(const struct nexthop_group_cmd *nhgc))
{
	...
	RB_FOREACH (nhgc_tmp, nhgc_entry_head, &nhgc_entries) {
		for (ALL_LIST_ELEMENTS_RO(nhgc_tmp->nhg_child_group_list, node, child_group)) {
			...
			void (*delete)(const char *name))
{
	...
	if (add_nexthop_or_child_group)
		nhg_hooks.add_nexthop_or_child_group = add_nexthop_or_child_group;
	if (del_nexthop_or_child_group)
		nhg_hooks.del_nexthop_or_child_group = del_nexthop_or_child_group;
	if (delete)
