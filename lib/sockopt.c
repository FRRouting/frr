...
int setsockopt_ipv6_pktinfo(int sock, int val)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_RECVPKTINFO, &val,
                       sizeof(val));
        if (ret < 0)
...
#else                           /*RFC2292 */
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_PKTINFO, &val,
                       sizeof(val));
        if (ret < 0)
...
#ifdef GNU_LINUX
        ret =
            setsockopt(sock, IPPROTO_RAW, IPV6_CHECKSUM, &val,
                       sizeof(val));
#else
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_CHECKSUM, &val,
                       sizeof(val));
#endif                          /* GNU_LINUX */
        if (ret < 0)
...
int setsockopt_ipv6_multicast_hops(int sock, int val)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &val,
                       sizeof(val));
        if (ret < 0)
...
int setsockopt_ipv6_unicast_hops(int sock, int val)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &val,
                       sizeof(val));
        if (ret < 0)
...
int setsockopt_ipv6_hoplimit(int sock, int val)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_RECVHOPLIMIT, &val,
                       sizeof(val));
        if (ret < 0)
                zlog_warn("can't setsockopt IPV6_RECVHOPLIMIT");
#else                           /*RFC2292 */
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_HOPLIMIT, &val,
                       sizeof(val));
        if (ret < 0)
...
int setsockopt_ipv6_tclass(int sock, int tclass)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IPV6, IPV6_TCLASS, &tclass,
                       sizeof(tclass));
        if (ret < 0)
                zlog_warn
                          ...
                          unsigned int mcast_addr, ifindex_t ifindex)
{
        ...
        if ((ret < 0) && (optname == IP_ADD_MEMBERSHIP)
            && (errno == EADDRINUSE)) {
                setsockopt(sock, IPPROTO_IP, MCAST_LEAVE_GROUP,
                           (void *) &gr, sizeof(gr));
                ret =
                    setsockopt(sock, IPPROTO_IP, MCAST_JOIN_GROUP,
                               (void *) &gr, sizeof(gr));
        }
        return ret;
        ...
        assert(optname == IP_ADD_MEMBERSHIP
            ...
            && (errno == EADDRINUSE)) {
                ...
                zlog_info
                    ("setsockopt_ipv4_multicast attempting to drop and "
                     "re-add (fd %d, mcast %s, ifindex %u)", sock,
                     inet_ntop(AF_INET, &mreqn.imr_multiaddr, buf[0],
                               sizeof(buf[0])), ifindex);
        ...
        ret =
            setsockopt(sock, IPPROTO_IP, optname, (void *) &mreq,
                       sizeof(mreq));
        if ((ret < 0) && (optname == IP_ADD_MEMBERSHIP)
            && (errno == EADDRINUSE)) {
                /* see above: handle possible problem when interface comes back up */
...
int setsockopt_ipv4_multicast_loop(int sock, u_char val)
{
        ...
        ret =
            setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP, (void *) &val,
                       sizeof(val));
        if (ret < 0)
...
static int setsockopt_ipv4_ifindex(int sock, ifindex_t val)
{
        ...
        if ((ret =
             setsockopt(sock, IPPROTO_IP, IP_PKTINFO, &val,
                        sizeof(val))) < 0)
                zlog_warn
        ...
        if ((ret =
             setsockopt(sock, IPPROTO_IP, IP_RECVIF, &val,
                        sizeof(val))) < 0)
...
/* Linux pktinfo based ifindex retrieval */
        ...
        pktinfo =
            (struct in_pktinfo *) getsockopt_cmsg_data(msgh, IPPROTO_IP,
                                                       IP_PKTINFO);
        ...
        /* BSD */
        sdl =
            (struct sockaddr_dl *) getsockopt_cmsg_data(msgh, IPPROTO_IP,
                                                        IP_RECVIF);
        if (sdl != NULL)
         ...
         */
        ifindex_p =
            (uint_t *) getsockopt_cmsg_data(msgh, IPPROTO_IP, IP_RECVIF);
        if (ifindex_p != NULL)
                ifindex = *ifindex_p;
        else
                ifindex = 0;
...
#define TCP_MD5_AUTH_DEL 2
        ...
        cmd.command =
            (password != NULL ? TCP_MD5_AUTH_ADD : TCP_MD5_AUTH_DEL);
        cmd.address = addr->s_addr;
        cmd.keylen = (password != NULL ? strlen(password) : 0);
        cmd.key = password;

        return setsockopt(sock, IPPROTO_TCP, TCP_MD5_AUTH, &cmd,
                          sizeof cmd);
...
#endif                          /* GNU_LINUX */
        if ((ret =
             setsockopt(sock, IPPROTO_TCP, TCP_MD5SIG, &md5sig,
                        sizeof md5sig)) < 0) {
