			 ...
			 route_map_event_t type)
{
	...
	if (ret) {
		...
		case RMAP_RULE_MISSING:
			...
			break;
		case RMAP_COMPILE_ERROR:
			vty_out(vty, "%% BGP Argument is malformed.%s",
				VTY_NEWLINE);
			break;
		}
		if (dep_name)
...
static void vty_show_route_map_entry(struct vty *vty, struct route_map *map)
{
	...
	for (index = map->head; index; index = index->next) {
		vty_out(vty, "route-map %s, %s, sequence %d%s", map->name,
		...
		for (rule = index->match_list.head; rule; rule = rule->next)
			vty_out(vty, "    %s %s%s", rule->cmd->str,
				rule->rule_str, VTY_NEWLINE);
		...
		if (index->nextrm)
			vty_out(vty, "    Call %s%s", index->nextrm,
				VTY_NEWLINE);

		/* Exit Policy */
		vty_out(vty, "  Action:%s", VTY_NEWLINE);
		if (index->exitpolicy == RMAP_GOTO)
			vty_out(vty, "    Goto %d%s", index->nextpref,
				VTY_NEWLINE);
		else if (index->exitpolicy == RMAP_NEXT)
			vty_out(vty, "    Continue to next entry%s",
				VTY_NEWLINE);
	...
	if (name) {
		...
		if (map) {
			...
			if (zlog_default && zlog_default->instance)
				vty_out(vty, " %d", zlog_default->instance);
			vty_out(vty, ": 'route-map %s' not found%s", name,
				VTY_NEWLINE);
			return CMD_SUCCESS;
		}
	} else {
       ...
       "IPv6 address of next hop\n")
{
	...
	if (!IN6_IS_ADDR_LINKLOCAL(&addr)) {
		vty_out(vty, "%% Invalid link-local nexthop address%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Route map tag\n")
{
	...
	if (map == NULL) {
		vty_out(vty, "%% Could not find route-map %s%s", mapname,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
       ...
       "Number\n")
{
	...
	if (index) {
		...
		if (num)
			VTY_GET_INTEGER_RANGE("route-map index", d, num, 1,
					      65535);
		else
			d = index->pref + 1;
...
static int route_map_config_write(struct vty *vty)
{
	...
	for (map = route_map_master.head; map; map = map->next)
		for (index = map->head; index; index = index->next) {
			...
			vty_out(vty, "route-map %s %s %d%s", map->name,
				route_map_type_str(index->type), index->pref,
				VTY_NEWLINE);
			...
			if (index->nextrm)
				vty_out(vty, " call %s%s", index->nextrm,
					VTY_NEWLINE);
			if (index->exitpolicy == RMAP_GOTO)
				vty_out(vty, " on-match goto %d%s",
					index->nextpref, VTY_NEWLINE);
...
void route_map_init(void)
{
	...
	route_map_master_hash =
		hash_create(route_map_hash_key_make, route_map_hash_cmp);
