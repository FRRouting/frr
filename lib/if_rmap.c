...
static struct if_rmap *if_rmap_set(const char *ifname,
              ...
              const char *routemap_name)
{
        ...
        if (type == IF_RMAP_IN) {
                if (!if_rmap->routemap[IF_RMAP_IN])
                        return 0;
                if (strcmp(if_rmap->routemap[IF_RMAP_IN], routemap_name) !=
                    0)
                        return 0;
        ...
        if (type == IF_RMAP_OUT) {
                if (!if_rmap->routemap[IF_RMAP_OUT])
                        return 0;
                if (strcmp(if_rmap->routemap[IF_RMAP_OUT], routemap_name)
                    != 0)
                        return 0;
      ...
      "Route map for output filtering\n" "Route map interface name\n")
{
        ...
        ret =
            if_rmap_unset(argv[idx_ifname]->arg, type,
