...
static int zfpm_conn_up_thread_cb(struct thread *thread)
{
	...
	while ((rnode = zfpm_rnodes_iter_next(iter))) {
		...
		zfpm_g->t_conn_up = thread_add_background(
			zfpm_g->master, zfpm_conn_up_thread_cb, 0, 0);
		return 0;
	}
...
static int zfpm_conn_down_thread_cb(struct thread *thread)
{
	...
	while ((rnode = zfpm_rnodes_iter_next(iter))) {
		...
		zfpm_rnodes_iter_pause(iter);
		zfpm_g->t_conn_down = thread_add_background(
			zfpm_g->master, zfpm_conn_down_thread_cb, 0, 0);
		return 0;
	}
 ...
 */
static inline int zfpm_encode_route(rib_dest_t *dest, struct rib *rib,
	...
	*msg_type = FPM_MSG_TYPE_NONE;
	...
	case ZFPM_MSG_FORMAT_NETLINK:
		...
		*msg_type = FPM_MSG_TYPE_NETLINK;
		...
		len = zfpm_netlink_encode_route(cmd, dest, rib, in_buf,
						in_buf_len);
...
static void zfpm_build_updates(void)
{
	...
	do {
		...
		if (write_msg) {
			data_len = zfpm_encode_route(dest, rib, (char *)data,
...
#define ZFPM_SHOW_STAT(counter)                                                \
	do {                                                                   \
		vty_out(vty, "%-40s %10lu %16lu%s", #counter,                  \
...
static void zfpm_show_stats(struct vty *vty)
{
	...
	vty_out(vty, "%s%-40s %10s     Last %2d secs%s%s", VTY_NEWLINE,
		"Counter", "Total", ZFPM_STATS_IVL_SECS, VTY_NEWLINE,
		VTY_NEWLINE);
...
static int fpm_remote_srv_write(struct vty *vty)
{
	...
	if (zfpm_g->fpm_server != FPM_DEFAULT_IP
	    || zfpm_g->fpm_port != FPM_DEFAULT_PORT)
		vty_out(vty, "fpm connection ip %s port %d%s", inet_ntoa(in),
			zfpm_g->fpm_port, VTY_NEWLINE);
