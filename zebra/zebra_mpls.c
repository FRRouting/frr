		       ...
		       struct route_node *rn, struct route_entry *re)
{
	...
	while (nexthop) {
		if (!CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
			nexthop = nexthop_next_resolution(
				nexthop, zvrf_nexthop_resolution);
			continue;
		}
		...
		if (nhlfe) {
			...
			if (IS_ZEBRA_DEBUG_MPLS) {
				...
				nhlfe2str(nhlfe, buf, BUFSIZ);
				zlog_debug(
					"Add LSP in-label %u type %d nexthop %s out-label %s",
					lsp->ile.in_label, lsp_type, buf,
					mpls_label2str(
						nexthop->nh_label->num_labels,
						nexthop->nh_label->label,
						label_str, sizeof(label_str),
					       ...
					       struct zebra_vrf *zvrf)
{
	...
	if (zvrf->zebra_mpls_fec_nexthop_resolution == status)
		return;
				   ...
				   struct mpls_label_stack *nh_label)
{
	...
	if (nh_label->num_labels > nhlfe->nexthop->nh_label->num_labels) {
		nh_label_tmp = XREALLOC(
			MTYPE_NH_LABEL, nhlfe->nexthop->nh_label,
			sizeof(struct mpls_label_stack) +
				nh_label->num_labels * sizeof(mpls_label_t));
		if (nh_label_tmp) {
