	...
	"SRLG value\n")
{
	...
	snprintfrr(xpath, sizeof(xpath), "./srlg[.='%s']", srlg_str);
	nb_cli_enqueue_change(vty, xpath, no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);

	return nb_cli_apply_changes(vty, NULL);
}

static void lib_interface_zebra_link_params_srlg_cli_write(struct vty *vty,
							   const struct lyd_node *dnode,
							   bool show_defaults)
{
	vty_out(vty, "  srlg %u\n", yang_dnode_get_uint32(dnode, NULL));
...
DEFPY_YANG (ip_import_vrf,
	    ip_import_vrf_cmd,
	    "[no] ip import-vrf WORD$src_vrf [distance (1-255)$distance] [route-map RMAP_NAME$rmap]",
	    NO_STR
	    IP_STR
	    ...
	    "Route map name\n")
{
	...
	if (no)
		...
		const char *dist = distance_str;

		if (!dist)
			dist = STRINGIFY(ZEBRA_VRF_IMPORT_DISTANCE_DEFAULT);

		nb_cli_enqueue_change(vty, xpath, NB_OP_CREATE, NULL);
		snprintfrr(xpath_value, sizeof(xpath_value), "%s/distance", xpath);
		nb_cli_enqueue_change(vty, xpath_value, NB_OP_MODIFY, dist);
...
DEFPY_YANG (ipv6_import_vrf,
	    ipv6_import_vrf_cmd,
	    "[no] ipv6 import-vrf WORD$src_vrf [distance (1-255)$distance] [route-map RMAP_NAME$rmap]",
	    NO_STR
	    IPV6_STR
	    "Import routes from another VRF\n"
	    "Source VRF name\n"
	    ...
	    "Route map name\n")
{
	...
	snprintfrr(xpath, sizeof(xpath),
		   "./frr-zebra:zebra/import-vrf[afi-safi='%s'][source-vrf='%s']", afi_safi,
		   src_vrf);
...
static void lib_vrf_zebra_import_vrf_cli_write(struct vty *vty, const struct lyd_node *dnode,
					       bool show_defaults)
{
	const char *afi_safi = yang_dnode_get_string(dnode, "afi-safi");
	const char *src = yang_dnode_get_string(dnode, "source-vrf");
	uint8_t distance = yang_dnode_get_uint8(dnode, "distance");
	const char *rmap = NULL;
	afi_t afi;
	safi_t safi;
	...
	if (distance != ZEBRA_VRF_IMPORT_DISTANCE_DEFAULT)
		vty_out(vty, " distance %u", distance);
	if (rmap)
		vty_out(vty, " route-map %s", rmap);
	vty_out(vty, "\n");
...
const struct frr_yang_module_info frr_zebra_cli_info = {
	...
	.nodes = {
		...
		},
		{
			...
			.cbs.cli_show = lib_vrf_zebra_import_vrf_cli_write,
		},
		{
...
void zebra_cli_init(void)
{
	...
	install_element(VRF_NODE, &ip_import_vrf_cmd);
	install_element(VRF_NODE, &ipv6_import_vrf_cmd);
