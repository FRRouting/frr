...
static void zebra_dplane_queue_limit_cli_write(struct vty *vty,
					       const struct lyd_node *dnode,
					       bool show_defaults)
{
...
static void zebra_zapi_packets_cli_write(struct vty *vty,
					 const struct lyd_node *dnode,
					 bool show_defaults)
{
...
static void zebra_workqueue_hold_timer_cli_write(struct vty *vty,
						 const struct lyd_node *dnode,
						 bool show_defaults)
{
...
static void zebra_import_kernel_table_xpath(char *xpath, size_t xpath_len,
...
static void zebra_import_kernel_table_cli_write(struct vty *vty,
						const struct lyd_node *dnode,
						bool show_defaults)
{
...
{
	if (!no)
		nb_cli_enqueue_change(vty, "/frr-zebra:zebra/dplane-queue-limit",
...
{
	if (!no)
		nb_cli_enqueue_change(vty, "/frr-zebra:zebra/zapi-packets",
				      NB_OP_MODIFY, packets_str);
	else
		nb_cli_enqueue_change(vty, "/frr-zebra:zebra/zapi-packets",
				      NB_OP_DESTROY, NULL);
...
{
	if (!no)
		nb_cli_enqueue_change(vty, "/frr-zebra:zebra/workqueue-hold-timer",
	    ...
	    "route-map name\n")
{
	...
	zebra_import_kernel_table_xpath(xpath, sizeof(xpath), AFI_IP6, safi,
					 table_id_str);
