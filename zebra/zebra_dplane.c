...
struct zebra_dplane_ctx {
	...
	enum zebra_dplane_read_reason zd_read_reason;
	uint32_t zd_read_seq;
	uint32_t zd_read_flags;
...
bool dplane_ctx_get_neigh_read_mac_valid(const struct zebra_dplane_ctx *ctx)
{
	...
	return ctx->u.neigh_read.mac_valid;
}
							...
							uint32_t read_seq, uint32_t read_flags)
{
	...
	ctx->zd_status = ZEBRA_DPLANE_REQUEST_SUCCESS;
	ctx->zd_read_reason = reason;
	ctx->zd_read_seq = read_seq;
	ctx->zd_read_flags = read_flags;
...
static enum zebra_dplane_result dplane_neigh_read_enqueue(struct zebra_ns *zns, ifindex_t ifindex,
							  const struct ipaddr *ip, vni_t vni,
							  const struct ethaddr *mac,
							  enum zebra_dplane_read_reason reason,
							  uint32_t read_seq, uint32_t read_flags)
{
	...
	ctx->u.neigh_read.vni = vni;
	if (ip)
		ctx->u.neigh_read.ip = *ip;
	if (mac) {
		ctx->u.neigh_read.mac = *mac;
		ctx->u.neigh_read.mac_valid = true;
	}
...
{
	return dplane_fdb_read_enqueue(zns, 0, 0, 0, 0, NULL, false, false, ZEBRA_DPLANE_READ_NONE,
				       0, 0);
}

enum zebra_dplane_result dplane_neigh_read(struct zebra_ns *zns)
{
	return dplane_neigh_read_enqueue(zns, 0, NULL, 0, NULL, ZEBRA_DPLANE_READ_NONE, 0, 0);
}
						      ...
						      uint32_t read_seq, uint32_t read_flags)
{
	...
	return dplane_fdb_read_enqueue(zns, 0, br_ifp->ifindex, vid, vni, mac,
				       IS_ZEBRA_IF_BRIDGE_VLAN_AWARE(br_zif), false, reason,
				       read_seq, read_flags);
}
...
{
	return dplane_neigh_read_enqueue(zns, vlan_ifp->ifindex, NULL, 0, NULL,
					 ZEBRA_DPLANE_READ_NONE, 0, 0);
}
...
enum zebra_dplane_result dplane_neigh_read_specific_ip(struct zebra_ns *zns,
						       ...
						       uint32_t read_seq, uint32_t read_flags)
{
	return dplane_neigh_read_enqueue(zns, vlan_ifp->ifindex, ip, vni, mac, reason, read_seq,
					 read_flags);
}
