...
void rib_route_dplane_update(struct zebra_dplane_ctx *ctx)
{
	...
	if (!startup && selfroute && op == DPLANE_OP_ROUTE_INSTALL && !zrouter.asic_offloaded) {
	...
	if (op == DPLANE_OP_ROUTE_INSTALL) {
		...
		if (!tb[RTA_MULTIPATH]) {
			...
			if (!nhe_id) {
				nh = parse_nexthop_unicast(ns_id, rtm->rtm_flags, tb, bh_type,
							   index, prefsrc, gate, afi, vrf_id);
			...
			if (!nhe_id) {
				 ...
				 */
				nhop_num = parse_multipath_nexthops_unicast(ns_id, ng,
									    rtm->rtm_family, rtnh,
									    tb, prefsrc, vrf_id);
