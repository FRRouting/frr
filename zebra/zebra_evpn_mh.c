...
static int zebra_evpn_es_send_add_to_client(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("send add local es %s %pIA active %u df_pref %u%s to %s", es->esi_str,
			   &zmh_info->es_originator_ip, oper_up, es->df_pref,
			   bypass ? " bypass" : "", zebra_route_string(client->proto));
...
int zebra_evpn_remote_es_del(const esi_t *esi, struct ipaddr *vtep_ip)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("remote es %s vtep %pIA del", esi_to_str(esi, buf, sizeof(buf)),
			   vtep_ip);
