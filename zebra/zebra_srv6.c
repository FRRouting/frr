...
DEFINE_HOOK(srv6_manager_get_locator_static_sids,
	    (struct srv6_locator **loc, struct list *static_sids_list, struct zserv *client,
...
void srv6_manager_get_locator_static_sids_call(struct srv6_locator **loc,
					       struct list *static_sids_list, struct zserv *client,
					       const char *locator_name, vrf_id_t vrf_id)
{
	hook_call(srv6_manager_get_locator_static_sids, loc, static_sids_list, client, locator_name,
		  vrf_id);
}
							 ...
							 const char *locator_name, vrf_id_t vrf_id)
{
	*loc = zebra_srv6_locator_lookup(locator_name);
	if (!*loc) {
		zlog_err("Unable to find the requested locator %s to %s instance %u", locator_name,
			 zebra_route_string(client->proto), client->instance);
		return -1;
	} else if (IS_ZEBRA_DEBUG_PACKET)
		zlog_info("Find request locator %s to %s instance %u", locator_name,
			  zebra_route_string(client->proto), client->instance);

	return zsend_srv6_manager_get_locator_static_sids_response(*loc, static_sids_list, client,
								   vrf_id);
}
