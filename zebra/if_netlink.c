				...
				struct interface *slave)
{
	...
	if (!nl_attr_put32(&req.n, sizeof(req), IFLA_MASTER, master->ifindex)) {
		zlog_err("%s: failed to encode IFLA_MASTER attribute in netlink message", __func__);
		return -1;
	}
	if (!nl_attr_put32(&req.n, sizeof(req), IFLA_LINK, slave->ifindex)) {
		zlog_err("%s: failed to encode IFLA_LINK attribute in netlink message", __func__);
		return -1;
	}
...
static int netlink_request_vlan(struct zebra_ns *zns, int family, int type)
{
	...
	if (!nl_attr_put32(&req.n, sizeof(req), BRIDGE_VLANDB_DUMP_FLAGS,
		...
		zlog_err("%s: Failed to put BRIDGE_VLANDB_DUMP_FLAGS attr to netlink message",
			 __func__);
		return -1;
	}
