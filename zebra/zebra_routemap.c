...
static enum route_map_cmd_result_t
route_match_next_hop_seg6_prefix_list(void *rule, const struct prefix *prefix,
				      void *object)
{
	struct zebra_rmap_obj *rm_data;
	struct prefix_list *plist;
	...
	return (prefix_list_apply(plist, &seg) == PREFIX_DENY ? RMAP_NOMATCH
							      : RMAP_MATCH);
}

static enum route_map_cmd_result_t
route_match_ip_next_hop_seg6_prefix_list(void *rule,
					 const struct prefix *prefix,
					 void *object)
{
	if (prefix->family == AF_INET)
		return route_match_next_hop_seg6_prefix_list(rule, prefix,
							     object);
...
static enum route_map_cmd_result_t
route_match_ipv6_next_hop_seg6_prefix_list(void *rule,
					   const struct prefix *prefix,
					   void *object)
{
	if (prefix->family == AF_INET6)
...
static const struct route_map_rule_cmd
		route_match_ip_next_hop_seg6_prefix_list_cmd = {
	"ip next-hop seg6 prefix-list",
	route_match_ip_next_hop_seg6_prefix_list,
	route_match_ip_next_hop_seg6_prefix_list_compile,
	route_match_ip_next_hop_seg6_prefix_list_free
};

static const struct route_map_rule_cmd
		route_match_ipv6_next_hop_seg6_prefix_list_cmd = {
	"ipv6 next-hop seg6 prefix-list",
	route_match_ipv6_next_hop_seg6_prefix_list,
...
static enum route_map_cmd_result_t
route_set_srv6_encap_source(void *rule, const struct prefix *prefix,
			    void *object)
{
	struct zebra_rmap_obj *rm_data;
...
static void *route_set_srv6_encap_source_compile(const char *arg)
{
	...
	if (inet_pton(AF_INET6, arg, &v6addr) == 1) {
		encap_source = XMALLOC(MTYPE_ROUTE_MAP_COMPILED,
