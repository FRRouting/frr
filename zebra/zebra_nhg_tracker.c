...
void tracker_winner_pre_remove(struct route_node *rn, struct route_entry *re)
{
	...
	if (!parent_nhe) {
#ifdef NHG_TRK_VERBOSE_LOG
		zlog_info("%s: re %p prefix %pRN type %s vrf %u status 0x%x re->nhe NHG %u: WINNER set but tagged parent NHG %u is gone",
	...
	if (parent_nhe->tracker_pending_winners == 0) {
		zlog_info("%s: NHG %u: all winners drained (last via pre-remove of re %p prefix %pRN)",
