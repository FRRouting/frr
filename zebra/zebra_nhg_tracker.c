...
void tracker_winner_pre_remove(struct route_node *rn, struct route_entry *re)
{
	...
	if (!parent_nhe) {
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s: re %p prefix %pRN type %s vrf %u status 0x%x re->nhe NHG %u: WINNER set but no installed peer on rn",
	...
	if (parent_nhe->tracker_pending_winners == 0 &&
		...
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s: NHG %u: all winners drained (last via pre-remove of re %p prefix %pRN), scheduling consolidation",
