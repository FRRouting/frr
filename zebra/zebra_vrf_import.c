						       ...
						       vrf_id_t dst_vrf_id, afi_t afi)
{
	...
	/*
	 * Walk the top-level group only, not ALL_NEXTHOPS: the resolved chain is the
	 * source VRF's resolution result, and importing it would turn one protocol
	 * nexthop into several ECMP members. The destination VRF re-resolves.
	 */
	for (src_nh = src_re->nhe->nhg.nexthop; src_nh; src_nh = src_nh->next)
							    ...
							    const struct prefix *p)
{
	struct route_table *table;
	struct route_node *rn;
	struct route_entry *re, *match = NULL;
...
/* Whether re already is exactly what importing src_re would produce. */
static bool zebra_vrf_import_re_unchanged(const struct route_entry *re,
					  const struct route_entry *src_re, uint8_t distance,
					  uint32_t flags, const struct nexthop_group *ng)
{
	/*
	 * Flags the RIB and the dataplane own on an installed entry. They say
	 * nothing about what was imported, so they are masked out.
	 */
	uint32_t rib_owned = ZEBRA_FLAG_SELECTED | ZEBRA_FLAG_TRAPPED | ZEBRA_FLAG_OFFLOADED |
			     ZEBRA_FLAG_OFFLOAD_FAILED;

	if ((re->flags & ~rib_owned) != (flags & ~rib_owned) || re->metric != src_re->metric ||
	    re->mtu != src_re->mtu || re->tag != src_re->tag || re->distance != distance)
		return false;

	/*
	 * nhe holds the resolution result in the destination VRF, while
	 * nhe_received holds the group that was handed to rib_add_multipath(),
	 * which is what gets rebuilt here.
	 */
	return re->nhe_received && nexthop_group_equal_no_recurse(&re->nhe_received->nhg, ng);
}
				      ...
				      struct route_entry *src_re)
{
	...
	/*
	 * Do not copy the source flags. They belong to the source route: the copy
	 * never had the dataplane state they carry, and SELFROUTE or FIB_OVERRIDE
	 * would change how it is selected and installed here.
	 *
	 * The copy does need recursion. Its nexthops lost the source VRF ifindex
	 * and are resolved again in the destination VRF, where the gateway is
	 * usually not on a connected subnet.
	 */
	import_flags = ZEBRA_FLAG_ALLOW_RECURSION;

	/*
	 * Selected-route updates also fire for changes the copy does not care
	 * about, such as a resolution change below an unchanged protocol nexthop.
	 * Reinstalling an identical entry would churn the RIB and the FIB for
	 * nothing.
	 */
	oldre = zebra_vrf_import_lookup_imported(dst_zvrf, import->afi, import->safi,
						 zvrf_id(src_zvrf), &src_rn->p);
	if (oldre &&
	    zebra_vrf_import_re_unchanged(oldre, src_re, import->distance, import_flags, ng)) {
		nexthop_group_delete(&ng);
		return 0;
	}

	/*
	 * The source distance is not carried over. It ranks the source protocol
	 * against the other protocols in the source VRF, which says nothing
	 * about how a copy should rank in the destination VRF, and a kernel or
	 * connected route would arrive at distance zero and outrank everything
	 * there. The import distance is configured per import instead.
	 */
	newre = zebra_rib_route_entry_new(zvrf_id(dst_zvrf), ZEBRA_ROUTE_VRF_IMPORT, 0,
					  import_flags, 0, dst_zvrf->table_id, src_re->metric,
					  src_re->mtu, import->distance, src_re->tag);
	newre->vrf_import_src_vrf_id = zvrf_id(src_zvrf);

	/*
	 * rib_add_multipath() replaces a previous copy of this source prefix in
	 * place, so the route survives the update in the FIB. Only the paths that
	 * leave nothing importable delete it outright.
	 */
			 ...
			 const char *src_vrf_name, uint8_t distance, const char *rmap_name)
{
	...
	if (!dst_zvrf || !src_vrf_name || afi >= AFI_MAX || safi >= SAFI_MAX || distance == 0)
		return -1;
