...
/*
 * XPath:
 * /frr-route-map:lib/route-map/entry/set-action/rmap-set-action/frr-zebra-route-map:initcwnd
 */
int lib_route_map_entry_set_action_rmap_set_action_initcwnd_modify(
	struct nb_cb_modify_args *args)
{
	struct routemap_hook_context *rhc;
	const char *wnd;
	...
	/* Add configuration. */
	rhc = nb_running_get_entry(args->dnode, NULL, true);
	wnd = yang_dnode_get_string(args->dnode, NULL);

	/* Set destroy information. */
	rhc->rhc_shook = generic_set_delete;
	rhc->rhc_rule = "initcwnd";

	rv = generic_set_add(rhc->rhc_rmi, "initcwnd", wnd, args->errmsg,
			     args->errmsg_len);
	if (rv != CMD_SUCCESS) {
		rhc->rhc_shook = NULL;
		return NB_ERR_INCONSISTENCY;
...
int lib_route_map_entry_set_action_rmap_set_action_initcwnd_destroy(
	struct nb_cb_destroy_args *args)
{
	switch (args->event) {
	case NB_EV_VALIDATE:
...
/*
 * XPath:
 * /frr-route-map:lib/route-map/entry/set-action/rmap-set-action/frr-zebra-route-map:initrwnd
 */
int lib_route_map_entry_set_action_rmap_set_action_initrwnd_modify(
	struct nb_cb_modify_args *args)
{
	struct routemap_hook_context *rhc;
	const char *wnd;
	...
	/* Set destroy information. */
	rhc->rhc_shook = generic_set_delete;
	rhc->rhc_rule = "initrwnd";

	rv = generic_set_add(rhc->rhc_rmi, "initrwnd", wnd, args->errmsg,
			     args->errmsg_len);
	if (rv != CMD_SUCCESS) {
		rhc->rhc_shook = NULL;
		return NB_ERR_INCONSISTENCY;
...
int lib_route_map_entry_set_action_rmap_set_action_initrwnd_destroy(
	struct nb_cb_destroy_args *args)
{
	switch (args->event) {
	case NB_EV_VALIDATE:
