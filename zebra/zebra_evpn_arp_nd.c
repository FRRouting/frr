					  ...
					  int len)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ARP_ND_PKT)
		zlog_debug(
			"evpn arp_nd of len %u redirect to vni %d %pI4 with vxh(0x%x 0x%x)",
			len + (int)sizeof(struct vxlanhdr), zevpn->vni, &vtep_ip,
...
static int zebra_evpn_arp_nd_sock_create(struct zebra_if *zif)
{
	...
	if (setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog))
	    < 0) {
		flog_err(EC_LIB_SOCKET,
			 "evpn arp_nd sock SO_ATTACH_FILTER set: fd %d errno %s",
			 fd, safe_strerror(errno));
		close(fd);
		return -1;
	}
...
#else
/* ARP redirection is only supported on linux */
void zebra_evpn_arp_nd_if_update(struct zebra_if *zif, bool enable)
{
}
...
{
	if (IS_ZEBRA_DEBUG_EVPN_MH_ARP_ND_EVT)
		zlog_debug("Enable arp_nd failover is only supported on linux");
}
