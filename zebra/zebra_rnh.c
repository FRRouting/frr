...
/* Hash key function for rnh - uses client pointer as key */
static uint32_t rnh_hash_key(const struct rnh *rnh)
{
	/* Use protocol type for deterministic ordering.
	 * For pseudowires (client == PSEUDOWIRE_CLIENT), this will hash
	 * consistently as well.
	 */
	if (rnh->client == PSEUDOWIRE_CLIENT)
		return (uint32_t)UINT8_MAX; /* Use max value for pseudowires */
	return (uint32_t)rnh->client->proto;
}

/* Hash comparison function for rnh */
static int rnh_hash_cmp(const struct rnh *rnh1, const struct rnh *rnh2)
{
...
DECLARE_HASH(rnh_hash, struct rnh, rnh_hash_item, rnh_hash_cmp, rnh_hash_key);
			  ...
			  bool *exists)
{
	...
	rnh = rnh_hash_find(&rnhc->rnh_hash, &lookup);
	...
	if (rnh) {
		...
		*exists = false;
		...
		/* Add to hash */
		rnh_hash_add(&rnhc->rnh_hash, rnh);
...
struct rnh *zebra_lookup_rnh(struct prefix *p, vrf_id_t vrfid, safi_t safi, struct zserv *client)
{
	...
	lookup.client = lookup_client;
	return rnh_hash_find(&rnhc->rnh_hash, &lookup);
}
...
static void zebra_delete_rnh(struct rnh *rnh)
{
	...
	rnhc = rn->info;
	if (rnhc && rnh->client)
		rnh_hash_del(&rnhc->rnh_hash, rnh);

	zebra_free_rnh(rnh);

	/* If hash is now empty, free the container */
	if (rnhc && rnh_hash_count(&rnhc->rnh_hash) == 0) {
		rnh_hash_fini(&rnhc->rnh_hash);
			     ...
			     void *ctx)
{
	...
	frr_each (rnh_hash, &rnhc->rnh_hash, rnh) {
		count++;
		if (cb(rnh, ctx) != 0)
			break;
	}
				     ...
				     struct route_node *nrn)
{
	...
	rnh = rnh_hash_first(&rnhc->rnh_hash);
	if (!rnh)
		return;
...
void zebra_rnh_node_cleanup(struct route_node *node)
{
	...
	frr_each_safe (rnh_hash, &rnhc->rnh_hash, rnh) {
		rnh_hash_del(&rnhc->rnh_hash, rnh);
...
static void print_rnh(struct route_node *rn, struct vty *vty, json_object *json)
{
	...
	frr_each (rnh_hash, &rnhc->rnh_hash, rnh) {
		...
		frr_each (rnh_hash, &rnhc->rnh_hash, compare_rnh) {
			if (compare_rnh == rnh)
				break;
