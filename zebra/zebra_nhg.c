...
/*
 * Will this NHE's current content be programmed as a kernel group
 * (NHA_GROUP) rather than a bare NHA_GATEWAY/NHA_OIF singleton?
 *
 * Mirrors the case selection in zebra_nhe_find(): multi-nexthop and
 * recursive entries are always groups, and a single non-recursive nexthop
 * is a leaf only when the entry carries a specific AFI -- parent NHEs live
 * at AFI_UNSPEC and get wrapped in a group of one.  The dplane keys the
 * encoding off depends-emptiness, so anything rebuilding a depends tree has
 * to agree with this.
 *
...
bool zebra_nhg_content_is_group(const struct nhg_hash_entry *nhe)
{
	...
	if (nh->next || CHECK_FLAG(nh->flags, NEXTHOP_FLAG_RECURSIVE))
		return true;

	return nhe->afi == AFI_UNSPEC;
}
 ...
 * and rebuilds them from the new nexthop list.  'afi' is the route's AFI,
 * not nhe->afi (parents are AFI_UNSPEC and would hash-collide with leaves).
 */
void zebra_nhg_rebuild_depends(struct nhg_hash_entry *nhe, afi_t afi)
{
	struct nexthop *nh;
	struct nhg_connected *rb_node_dep;
	uint32_t i;
	...
	/* Three cases, matching zebra_nhe_find(). */
	if (nh->next == NULL && CHECK_FLAG(nh->flags, NEXTHOP_FLAG_RECURSIVE) &&
	    nhe->id < ZEBRA_NHG_PROTO_LOWER) {
		/* Case 1: single recursive zebra-owned nexthop */
		handle_recursive_depend(&nhe->nhg_depends, nh->resolved, afi, nhe->type);
		SET_FLAG(nhe->flags, NEXTHOP_GROUP_RECURSIVE);
	} else if (!zebra_nhg_content_is_group(nhe)) {
		/*
		 * Case 2: leaf singleton - keep nhg_depends empty
		 */
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s: NHG %u leaf singleton NH %pNHv afi=%d", __func__, nhe->id,
				   nh, nhe->afi);
	} else {
		/* Case 3: build depends per nexthop */
		for (ALL_NEXTHOPS(nhe->nhg, nh))
			depends_find_add(&nhe->nhg_depends, nh, afi, nhe->type, false);
	}
 ...
 * and update the ifp binding for leaf singletons.
 * Caller must ensure nhe is currently OUT of the content hash.
 *
 * 'afi' is the AFI of the route driving the rework; see
 * zebra_nhg_rebuild_depends().
 */
				     ...
				     afi_t afi)
{
	...
	zebra_nhg_rebuild_depends(nhe, afi);

	/*
	 * Depends must match content shape or the dplane hands the kernel a
	 * structural change it rejects.  Should never fire.
	 */
	if (zebra_nhg_content_is_group(nhe) == zebra_nhg_depends_is_empty(nhe))
		zlog_warn("%s: NHG %u rework shape mismatch: content wants %s but depends %s",
			  __func__, nhe->id,
			  zebra_nhg_content_is_group(nhe) ? "group" : "singleton",
			  zebra_nhg_depends_is_empty(nhe) ? "empty" : "non-empty");
	...
	if (source_nhe->backup_info) {
		...
		zebra_nhg_backup_build_depends(nhe->backup_info->nhe, afi);
	}

	/*
	 * Update interface pointer for leaf singletons.  The new nexthop
	 * may use a different interface than the old one.  Remove from
	 * old zif->nhg_dependents and add to new.
	 * The afi check excludes parents (AFI_UNSPEC): only leaves bind to an interface.
	 */
	if (!PROTO_OWNED(nhe) && nhe->afi != AFI_UNSPEC && ZEBRA_NHG_IS_SINGLETON(nhe) &&
	    nhe->nhg.nexthop->ifindex &&
	    !CHECK_FLAG(nhe->nhg.nexthop->flags, NEXTHOP_FLAG_RECURSIVE)) {
					  ...
					  struct nhg_hash_entry *winner)
{
	...
	/* Retire from the content hash (up front: the loop below can free the loser). */
	hash_release(zrouter.nhgs, loser);
	UNSET_FLAG(loser->flags, NEXTHOP_GROUP_DUPLICATE);
	...
	frr_each_safe (nhe_re_tree, &loser->re_head, re) {
		...
		if (re->rn && CHECK_FLAG(re->status, ROUTE_ENTRY_INSTALLED | ROUTE_ENTRY_QUEUED)) {
			...
			frrtrace(5, frr_zebra, nhg_migrate, "migrate-skip", loser_id, winner->id,
				 re->status, 0);
		}
...
/*
 * Last tracker winner of a flush has drained off nhe.
 * Every path that takes tracker_pending_winners to zero ends here.
 */
void zebra_nhg_tracker_winners_drained(struct nhg_hash_entry *nhe)
{
	if (!nhe)
		return;
	...
	/*
	 * Phase 1 released nhe from the content hash and left it to a winner's
	 * rework to put it back.  If no winner did, reinsert it here.
...
void zebra_nhg_schedule_consolidate(struct nhg_hash_entry *nhe)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_TRACKER)
		zlog_debug("%s: NHG %u scheduled for consolidation (dup=%d pending_winners=%u)",
			   __func__, nhe->id,
			  ...
			  struct route_entry *old_re)
{
	...
	bool first_winner, parent_reusable;
...
backups_done:
	 ...
	 */
	if (parent_nhe) {
		/* Parent may have left the kernel during the flush. Reuse based on its state now. */
		parent_reusable = zebra_nhg_reusable(parent_nhe);
		...
		reuse_ok = route_map_ok && !shape_differs && parent_reusable;
		...
		if (first_winner) {
			if (!route_map_ok) {
				 ...
				 */
				if (IS_ZEBRA_DEBUG_NHG_TRACKER)
					zlog_debug("%s: NHG reuse skipped for old NHG %u: shape mismatch (parent_group=%d curr_group=%d)",
