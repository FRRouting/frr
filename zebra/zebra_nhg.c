...
void zebra_nhg_rebuild_depends(struct nhg_hash_entry *nhe)
{
	...
	zlog_info("%s: NHG %u (refcnt %d) rebuilt depends: old %u -> new %zu (drained/filled %d+1 per singleton)",
...
static void zebra_nhg_backup_build_depends(struct nhg_hash_entry *backup_nhe, afi_t afi)
{
	...
	if (ZEBRA_NHG_IS_SINGLETON(backup_nhe) && CHECK_FLAG(nh->flags, NEXTHOP_FLAG_RECURSIVE)) {
		...
		for (; nh; nh = nh->next) {
			if (IS_ZEBRA_DEBUG_NHG_DETAIL)
				zlog_debug("%s: backup_nhe %p depend NH %pNHv", __func__,
					   backup_nhe, nh);
...
{
	zlog_info("%s: NHG %u (refcnt %d flags 0x%x depends %zu backup %s) <- source NHG %u (backup %s)",
		  __func__, nhe->id, nhe->refcnt, nhe->flags,
		  nhg_connected_tree_count(&nhe->nhg_depends), nhe->backup_info ? "yes" : "no",
		  source_nhe->id, source_nhe->backup_info ? "yes" : "no");
	...
	if (source_nhe->backup_info) {
		...
		zlog_info("%s: NHG %u backup rebuilt: %zu backup-deps", __func__, nhe->id,
			  nhg_connected_tree_count(&nhe->backup_info->nhe->nhg_depends));
	}
...
void zebra_nhg_rework_content_rehash(struct nhg_hash_entry *nhe)
{
	...
	zlog_info("%s: input NHG %u (ptr=%p flags=0x%x) -> hash_get NHG %u (ptr=%p flags=0x%x) inserted=%d",
...
{
	uint32_t migrated __attribute__((unused)) = 0;
	...
	frr_each_safe (nhe_re_tree, &loser->re_head, re) {
		...
		if (re->rn && CHECK_FLAG(re->status, ROUTE_ENTRY_INSTALLED | ROUTE_ENTRY_QUEUED)) {
			...
			zlog_info("%s: NHG %u -> %u: calling rib_install_kernel for re %p prefix %pRN (status 0x%x)",
				  __func__, loser->id, winner->id, re, re->rn, re->status);
#endif
			rib_install_kernel(re->rn, re, NULL);
		}
#ifdef NHG_TRK_VERBOSE_LOG
		else {
			zlog_info("%s: NHG %u -> %u: NO rib_install_kernel for re %p (rn=%p installed=%d queued=%d status 0x%x)",
				  __func__, loser->id, winner->id, re, re->rn,
				  CHECK_FLAG(re->status, ROUTE_ENTRY_INSTALLED) ? 1 : 0,
				  CHECK_FLAG(re->status, ROUTE_ENTRY_QUEUED) ? 1 : 0, re->status);
		}
#endif
...
static void zebra_nhg_consolidate_event_handler(struct event *event)
{
	...
	if (nhe_busy) {
		zlog_info("%s: NHG %u busy (trackers=%zu pending_winners=%u); skipping consolidation, DUPLICATE retained",
			  __func__, nhe->id, nhg_event_tracker_list_count(&nhe->tracker_list),
	...
	for (i = 0; i < ctx.count; i++) {
		...
		if (nhg_event_tracker_list_count(&dup->tracker_list) > 0 ||
			...
			zlog_info("%s: NHG %u skipping busy dup NHG %u (trackers=%zu pending_winners=%u)",
	...
	if (ctx.count - skipped == 0) {
		 ...
		 * when they re-fire.
		 */
		zlog_info("%s: NHG %u no mergeable duplicate this round (dups found=%u, busy=%u%s) -- clearing DUPLICATE",
	...
	zlog_info("%s: NHG %u consolidation: %u dup(s) found (%u skipped busy), winner NHG %u (re_count=%u)",
		  __func__, nhe->id, ctx.count, skipped, winner->id, winner_re_count);
...
#ifdef NHG_TRK_VERBOSE_LOG
	zlog_info("pw-trk: clearing DUPLICATE post-merge: loser/nhe NHG %u, winner NHG %u",
		  nhe->id, winner->id);
#endif
	UNSET_FLAG(nhe->flags, NEXTHOP_GROUP_DUPLICATE);
...
void zebra_nhg_mark_duplicate(struct nhg_hash_entry *nhe)
{
	...
	zlog_info("%s: NHG %u has content-duplicate in the hash, consolidation will fire once winners drain",
		  __func__, nhe->id);
}
...
void zebra_nhg_mark_reuse(struct nhg_hash_entry *nhe)
{
	...
	zlog_info("%s: NHG %u marked as reuse-target", __func__, nhe->id);
}
...
backups_done:
	...
	if (parent_nhe) {
		...
		if (first_winner) {
			if (!route_map_ok) {
				 ...
				 */
#ifdef NHG_TRK_VERBOSE_LOG
				zlog_info("%s: NHG reuse skipped for old NHG %u (active=%u != incoming=%u), route-map mismatch -- leaving REUSE for another winner",
				...
				if (found == parent_nhe) {
#ifdef NHG_TRK_VERBOSE_LOG
					zlog_info("%s: NHG reuse fast-path: parent NHG %u already holds resolved content (active=%u incoming=%u)",
						  __func__, parent_nhe->id, resolved_active,
						  incoming_count);
#endif
					UNSET_FLAG(parent_nhe->flags, NEXTHOP_GROUP_TRACKER_REUSE);
...
#ifdef NHG_TRK_VERBOSE_LOG
					zlog_info("%s: NHG reuse: copying resolved state onto old NHG %u (active=%u incoming=%u)",
						  __func__, parent_nhe->id, resolved_active,
						  incoming_count);
#endif
		...
		if (parent_nhe->tracker_pending_winners > 0)
			...
			/* Leak detector: winner completing but parent counter already 0 (short-decrement) -- can strand pending_winners and block dup consolidation. */
			zlog_warn("%s: winner %pRN parent NHG %u pending_winners already 0 -- winner-counter short-decrement/leak",
...
#ifdef NHG_TRK_VERBOSE_LOG
		/* Single consolidated winner-drain trace (covers all cases). */
		zlog_info("pw-trk: winner drained %pRN parent NHG %u -> pending_winners=%u (reuse_ok=%d locked_id=%u dup=%d)%s",
			  rn, parent_nhe->id, parent_nhe->tracker_pending_winners,
			  reuse_ok ? 1 : 0, curr_nhe->id,
			  CHECK_FLAG(parent_nhe->flags, NEXTHOP_GROUP_DUPLICATE) ? 1 : 0,
			  (parent_nhe->tracker_pending_winners == 0)
				  ? (CHECK_FLAG(parent_nhe->flags, NEXTHOP_GROUP_DUPLICATE)
					     ? " [pending hit 0 -> consolidate scheduled]"
					     : " [pending hit 0]")
				  : "");
#endif
	} else if (is_tracker_winner) {
		/* Leak detector: winner with no installed peer (old_re NULL) -- parent_nhe unresolved, so pending_winners is never decremented and can strand (typically the old installed RE was drained/unlinked before this winner ran). */
		zlog_warn("%s: winner %pRN has no installed peer (old_re NULL) -- pending_winners DEC skipped, winner-counter leak",
			  __func__, rn);
	}
	...
	/*
	 * Custom log: dump the RE prefix and the final NHG mapped after
	 * resolution. Tag whether this came from the NHG reuse path or the
	 * normal hash-lookup path. Walk every nexthop (including recursively
	 * resolved children) so the full resolution tree is visible.
	 */
#ifdef NHG_TRK_VERBOSE_LOG
	if (re->nhe) {
		struct nexthop *_nhop;
		uint16_t _idx = 0;
		uint16_t _top_count = 0;

		for (_nhop = re->nhe->nhg.nexthop; _nhop; _nhop = _nhop->next)
			_top_count++;

		zlog_info("%s: re %pRN vrf %u type %d => NHG id %u (%pNG) flags 0x%x refcnt %d nh_count %u (%s path)",
			  __func__, rn, re->vrf_id, re->type, re->nhe->id, re->nhe, re->nhe->flags,
			  re->nhe->refcnt, _top_count, is_tracker_winner ? "winner" : "normal");

		for (ALL_NEXTHOPS_PTR(&re->nhe->nhg, _nhop)) {
			zlog_info("   %s: re %pRN nh[%u] %pNHv flags 0x%x%s%s", __func__, rn,
				  _idx++, _nhop, _nhop->flags,
				  CHECK_FLAG(_nhop->flags, NEXTHOP_FLAG_ACTIVE) ? " ACTIVE" : "",
				  CHECK_FLAG(_nhop->flags, NEXTHOP_FLAG_RECURSIVE) ? " RECURSIVE"
										   : "");
		}
	}
#endif
