...
/*
 * Zebra SRv6 L2 EVPN — Multi-VLAN-to-EVI mapping.
 *
 * Implements the "SRv6 L2 EVPN — Multi-VLAN-to-EVI Mapping" design: a per-EVI
 * dataplane-backend abstraction so the existing EVPN path is untouched while
 * SRv6 L2 EVPN instances are created directly from a VLAN->EVI binding in the
 * zebra SRv6 global block.
 *
 ...
 * VLAN-aware bridge + srl2 encap + an SRv6 locator.
 */
...
struct zevpn_dp_ops {
	...
	/* BUM / flood endpoint setup (HER vs srl2 DT2M/DT2U) */
...
#define SRV6_L2EVPN_LOCNAME_SIZE 256
