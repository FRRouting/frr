			    ...
			    enum srv6_headend_behavior *encap_behavior)
{
	...
	if (tb_encap[SEG6_IPTUNNEL_SRH]) {
		...
		nb_seg6 = ipt->srh[0].first_segment + 1;
		if (nb_seg6 > SRV6_MAX_SIDS) {
			nb_seg6 = SRV6_MAX_SIDS - 1;
			flog_err(EC_ZEBRA_NETLINK_SRV6_SID_NUMBER_ERROR,
				 "%s: nexthop unicast SRV6 Segment truncated from  %d to %d",
				 __func__, ipt->srh[0].first_segment, nb_seg6);
		}

		for (i = nb_seg6; i >= 0; i--)
			memcpy(&segs[nb_seg6 - i], &ipt->srh[0].segments[i],
