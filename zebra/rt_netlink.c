...
static int netlink_macfdb_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if (tb[NDA_PROTOCOL]) {
		proto = *(int *)RTA_DATA(tb[NDA_PROTOCOL]);
	}
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("Rx %s AF_BRIDGE IF %u%s st 0x%x fl 0x%x MAC %pEA%s nhg %d vni %d proto %d",
			   nl_msg_type_to_str(h->nlmsg_type), ndm->ndm_ifindex,
	...
	if (h->nlmsg_type == RTM_NEWNEIGH) {
		...
		if (IS_ZEBRA_IF_VXLAN(ifp)) {
			if (zebra_mac_ext_learn_mode()) {
				/* the Linux kernel should never give us an entry
				 * with VxLAN info
				 */
				return 0;
			} else {
				...
				return zebra_vxlan_dp_network_mac_add(ifp, br_if, &mac, vid, vni,
								      nhg_id, sticky,
								      !!(ndm->ndm_flags &
									 NTF_EXT_LEARNED));
			}
