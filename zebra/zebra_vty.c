		...
		if (routes) {
			if (uj) {
				...
				frr_each (nhe_re_tree, &nhe->re_head, re) {
					...
					json_object_string_addf(json_route, "prefix", "%s", buf);
					json_object_boolean_add(json_route, "installed",
								CHECK_FLAG(re->flags,
									   ZEBRA_FLAG_SELECTED));
					json_object_string_addf(json_route, "protocol", "%s",
								proto_name);
					if (info) {
						json_object_string_addf(json_route, "afi", "%s",
									afi2str(info->afi));
						json_object_string_addf(json_route, "safi", "%s",
									safi2str(info->safi));
					} else {
						json_object_string_add(json_route, "afi", "unknown");
