					      ...
					      json_object *json_routes, bool uj)
{
	...
	if (uj) {
		...
		snprintfrr(status_buf, sizeof(status_buf), "0x%x", re->status);
		snprintfrr(flags_buf, sizeof(flags_buf), "0x%x", re->flags);
      ...
      "Brief\n")
{
	...
	if (id) {
		...
		show_nexthop_group_id_cmd_helper(vty, id, uj ? json : NULL, brief);
		return CMD_SUCCESS;
	}
