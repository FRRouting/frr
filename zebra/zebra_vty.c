...
static void show_nhe_prefix_proto_nhgs_entry(struct hash_bucket *bucket, void *data)
{
	struct vty *vty;
	json_object *json = NULL;
	...
	if (json) {
		...
		json_object_string_addf(json_entry, "prefix", "%pFX", &entry->prefix);
		if (entry->prefix.family == AF_INET6 && entry->src_prefix.family != AF_UNSPEC)
			json_object_string_addf(json_entry, "srcPrefix", "%pFX", &entry->src_prefix);
		json_object_string_add(json_entry, "afi", afi2str(entry->afi));
		json_object_string_add(json_entry, "safi", safi2str(entry->safi));
		json_object_int_add(json_entry, "table", entry->table_id);
		json_object_int_add(json_entry, "vrfId", entry->vrf_id);
		json_object_string_add(json_entry, "vrfName", vrf_id_to_name(entry->vrf_id));
		...
		vty_out(vty, ", afi %s, safi %s, table %d (vrf %s)\n", afi2str(entry->afi),
			safi2str(entry->safi), entry->table_id, VRF_LOGNAME(vrf));
	}
}

static void show_nexthop_group_out(struct vty *vty, struct nhg_hash_entry *nhe, bool detail,
				   json_object *json_nhe_hdr)
{
	...
	if (detail && nhe->id >= ZEBRA_NHG_PROTO_LOWER) {
		...
		hash_iterate(nhe->prefix_proto_nhgs, show_nhe_prefix_proto_nhgs_entry, &ctx);
		if (json)
			json_object_object_add(json, "prefixList", json_prefix_list);
	}
}

static int show_nexthop_group_id_cmd_helper(struct vty *vty, uint32_t id, bool detail,
					    json_object *json)
{
...
static void show_nexthop_group_cmd_helper(struct vty *vty, struct zebra_vrf *zvrf, afi_t afi,
					  int type, bool detail, json_object *json)
{
