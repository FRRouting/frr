...
static struct nhg_backup_info *zapi_read_nexthop_group_backup(struct zserv *client,
							      ...
							      bool *err)
{
	...
	*err = false;
	...
	if (IS_ZEBRA_DEBUG_RECV)
		zlog_debug("%s: adding %d backup nexthop groups", __func__, nexthopgroup_num);
	...
	for (i = 0; i < nexthopgroup_num; i++) {
		...
		if (!nhe) {
			flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,
				  "%s: Nexthops Groups Specified: %u(%u) not found", __func__,
				  nexthopgroup_num, nhgid);
			zebra_nhg_backup_free(&bnhg);
			*err = true;
			return NULL;
		}
...
static struct nexthop_group *zapi_read_nexthop_group(struct zserv *client,
						     struct zapi_nhg_group *api_nhg_group)
{
	uint16_t nexthopgroup_num;
...
static int zapi_nhg_child_decode(struct stream *s, struct zapi_nhg_group *api_nhg_group)
{
	uint16_t i;
	...
	if (api_nhg_group->child_group_num == 0) {
		flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,
			  "%s: grp count should not be 0 when NHG_CHILD_ADD is used", __func__);
		return -1;
	}
...
stream_failure:
	flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,
		  "%s: Nexthop Group decode failed with some sort of stream read failure", __func__);
	return -1;
}
...
static void zread_nhg_add(ZAPI_HANDLER_ARGS)
{
	...
	if ((!zapi_read_nexthops(client, NULL, api_nhg.nexthops, 0, api_nhg.message,
				 api_nhg.nexthop_num, api_nhg.backup_nexthop_num, &nhg, NULL)) ||
	    (!zapi_read_nexthops(client, NULL, api_nhg.backup_nexthops, 0, 0,
				 api_nhg.backup_nexthop_num, api_nhg.backup_nexthop_num, NULL,
				 &bnhg))) {
		flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,
...
static void zread_nhg_child_add(ZAPI_HANDLER_ARGS)
{
	...
	if (zapi_nhg_child_decode(s, &api_nhg_group) < 0) {
		if (IS_ZEBRA_DEBUG_RECV)
			zlog_debug("%s: unable to decode the received zapi_nhg_group", __func__);
		return;
	}
