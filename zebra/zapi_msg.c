...
static void zserv_encode_vrf(struct stream *s, struct zebra_vrf *zvrf)
{
	...
	if (netns_name) {
		const char *basename_result = basename((char *)netns_name);
		if (basename_result)
			strlcpy(data.l.netns_name, basename_result, NS_NAMSIZ);
		else {
			zlog_warn("Failed to get basename from netns_name: %s", netns_name);
