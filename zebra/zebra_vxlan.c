...
static void zvni_send_mac_hash_entry_to_client(struct hash_bucket *bucket, void *arg)
{
	struct mac_walk_ctx *wctx = arg;
...
static void zvni_send_mac_to_client(zebra_vni_t *zvni)
{
        ...
        hash_iterate(zvni->mac_table, zvni_send_mac_hash_entry_to_client, &wctx);
}

/* Notify Neighbor entries to the Client, skips the GW entry */
static void zvni_send_neigh_hash_entry_to_client(struct hash_bucket *bucket, void *arg)
{
	struct mac_walk_ctx *wctx = arg;
