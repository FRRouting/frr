...
static int zl3vni_send_add_to_client(zebra_l3vni_t *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			"Send L3_VNI_ADD %u VRF %s RMAC %s local-ip %s to %s",
...
static void zebra_vxlan_process_l3vni_oper_up(zebra_l3vni_t *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("L3-VNI %u is UP - send add to BGP", zl3vni->vni);
...
static void zebra_vxlan_process_l3vni_oper_down(zebra_l3vni_t *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("L3-VNI %u is Down - Send del to BGP", zl3vni->vni);
