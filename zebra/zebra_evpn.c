...
int zebra_evpn_send_add_to_client(struct zebra_evpn *zevpn)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Send EVPN_ADD %u %pIA tenant vrf %s(%u) SVI index %u to %s",
			   zevpn->vni, &zevpn->local_vtep_ip, vrf_id_to_name(zevpn->vrf_id),
			   zevpn->vrf_id, (zevpn->svi_if ? zevpn->svi_if->ifindex : 0),
