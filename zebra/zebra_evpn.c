...
void zebra_evpn_print(struct zebra_evpn *zevpn, void **ctxt)
{
	 ...
	 * srl2/bum-srl2 backend replaces it), so "VxLAN interface" is not
...
int zebra_evpn_send_add_to_client(struct zebra_evpn *zevpn)
{
	 ...
	 */
	{
		...
		/*
		 * Trailing per-EVI locator METADATA.
		 *
		 ...
		 */
		{
			...
			if (meta_valid) {
				/* uSID if the EVI locator carries the uSID flag
				 * (`behavior usid`) OR a uSID sid-format
				 * (`format usid-fNNNN`).
				 */
