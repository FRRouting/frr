		...
		RNODE_FOREACH_RE (rn, newre) {
			if (IS_ZEBRA_DEBUG_RIB)
				zlog_debug("%s: client %s %pRN(%u:%u) checking: selected=%d, type=%s, instance=%u, distance=%d, metric=%d zebra_check_addr=%d",
					   ...
					   route_entry_get_proto_instance(newre), newre->distance,
					   newre->metric, zebra_check_addr(&rn->p));
			 ...
			 const struct route_entry *prev_re)
{
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("(%u:%u):%pRN(%u): Redist update re %p (%s), old %p (%s)", re->vrf_id,
			   re->table, rn, route_entry_get_proto_instance(re), re,
				 ...
				 struct route_entry *re, const char *rmap_name)
{
	...
	RNODE_FOREACH_RE (rn, same) {
		...
		if (same->type == re->type &&
		    ...
		    same->table == re->table &&
		    (same->type != ZEBRA_ROUTE_CONNECT && same->type != ZEBRA_ROUTE_LOCAL))
			break;
	}
