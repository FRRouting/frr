			 ...
			 struct rib *rib, struct rib *prev_rib)
{
	...
	if (IS_ZEBRA_DEBUG_RIB) {
		...
		zlog_debug(
			"%u:%s/%d: Redist update rib %p (type %d), old %p (type %d)",
	...
	for (ALL_LIST_ELEMENTS(zebrad.client_list, node, nnode, client)) {
		...
		if (is_default(p)
			...
			send_redistribute = 1;
					  ...
					  rib->vrf_id))
			send_redistribute = 1;
		else if (rib->instance
			 && redist_check_instance(
				    &client->mi_redist[afi][rib->type],
				    rib->instance))
			send_redistribute = 1;
		else if (vrf_bitmap_check(client->redist[afi][rib->type],
					  rib->vrf_id))
			send_redistribute = 1;
			...
			zsend_redistribute_route(1, client, p, src_p, rib);
			   ...
			   && ((rib->instance
				&& redist_check_instance(
					   ...
					   rib->instance))
			       || vrf_bitmap_check(
					  client->redist[afi][prev_rib->type],
					  rib->vrf_id))) {
	...
	for (ALL_LIST_ELEMENTS(zebrad.client_list, node, nnode, client)) {
		if ((is_default(p)
			...
			&& redist_check_instance(
				   &client->mi_redist[afi][rib->type],
				   rib->instance))
		    || vrf_bitmap_check(client->redist[afi][rib->type],
					rib->vrf_id)) {
				 ...
				 const char *rmap_name)
{
	...
	if (rmap_name)
		ret = zebra_import_table_route_map_check(
			AFI_IP, rib->type, &rn->p, rib->nexthop, rib->vrf_id,
			rib->tag, rmap_name);
	...
	if (ret == RMAP_MATCH) {
		if (rn->p.family == AF_INET) {
			...
			if (rib->nexthop_num == 1) {
				...
				rib_add(AFI_IP, SAFI_UNICAST, rib->vrf_id,
					ZEBRA_ROUTE_TABLE, rib->table, 0, &p,
					NULL, gate,
...
int zebra_del_import_table_entry(struct route_node *rn, struct rib *rib)
{
	...
	if (rn->p.family == AF_INET) {
		...
		rib_delete(AFI_IP, SAFI_UNICAST, rib->vrf_id, ZEBRA_ROUTE_TABLE,
			   rib->table, rib->flags, &p, NULL, NULL, 0,
