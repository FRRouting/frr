...
bgpd
   ...
   local decap. Advertisement is triggreed using the command
   ( ``advertise-srv6-evpn``).
...
With ``advertise-srv6-evpn`` selected for the EVPN address-family, bgpd:
...
``(bgp)->l2vpn_evpn_enabled``. With this change both VXLAN,SRv6 can coexist.
...
* ``vpws_build_attr()`` -- IPv4 nexthop = router-id; MPLS label =
  ...
  ``uDX2`` (``SRV6_ENDPOINT_BEHAVIOR_UDX2``) depending on whether the
