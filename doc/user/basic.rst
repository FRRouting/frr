...
For ``<user>`` substitute the name of the FRR user (usually ``frr``).
   ...
   .. note::
      ...
      If for some reason the key file is not available, encryption of
      protocol new passwords, authentication strings, keys will be
      disabled and decryption of previously encrypted strings will fail.
      Once the key file is restored, normal encryption and decryption
      operation can occur.  If the key file is lost, the encrypted
      strings can be regenerated by configuring the original unencrypted
      value.
