...
struct mgmt_commit_cfg_req {
	 ...
	 */
	struct mgmt_msg_cfg_req *msgs[MGMTD_BE_CLIENT_ID_MAX];
...
static void mgmt_txn_req_free(struct mgmt_txn_req **txn_req)
{
	...
	case MGMTD_TXN_PROC_COMMITCFG:
		...
		FOREACH_MGMTD_BE_CLIENT_ID (id) {
			 ...
			 */
			mgmt_msg_native_free_msg(ccreq->msgs[id]);
...
static int mgmt_txn_create_config_msgs(struct mgmt_txn_req *txn_req, struct nb_config_cbs *changes)
{
	...
	RB_FOREACH_SAFE (cb, nb_config_cbs, changes, nxt) {
		...
		FOREACH_BE_CLIENT_BITS (id, clients) {
			 ...
			 */
			char op = chg->cb.operation == NB_CB_DESTROY ? 'd' : 'm';

			darr_push(cmtcfg_req->cfg_actions[id], op);
			mgmt_msg_native_add_str(cmtcfg_req->msgs[id], xpath);
			if (op == 'm')
				mgmt_msg_native_add_str(cmtcfg_req->msgs[id], value);
				     ...
				     struct mgmt_be_client_adapter *adapter)
{
	...
	cmtcfg_req = &txn->commit_cfg_req->req.commit_cfg;
	assert(IS_IDBIT_SET(cmtcfg_req->clients, id));

	/* Add the final action string to the message */
	mgmt_msg_native_add_str(cmtcfg_req->msgs[id], cmtcfg_req->cfg_actions[id]);
	darr_free(cmtcfg_req->cfg_actions[id]);

	ret = mgmt_msg_native_send_msg(adapter->conn, cmtcfg_req->msgs[id], false);
	mgmt_msg_native_free_msg(cmtcfg_req->msgs[id]);
	if (ret) {
		(void)mgmt_txn_send_commit_cfg_reply(txn, MGMTD_INTERNAL_ERROR,
						     "Internal Error! Could not send config data to backend!");
		_log_err("Could not send CFG_REQ txn-id: %Lu to client '%s", txn->txn_id,
			 adapter->name);
		return -1;
	}
