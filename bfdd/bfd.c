...
void ptm_sbfd_init_sess_dn(struct bfd_session *bfd, uint8_t diag)
{
	 ...
	 * removed or VRF doesn't exist anymore).
	 */
	if (bfd->sock != -1 && (!CHECK_FLAG(bfd->flags, BFD_SESS_FLAG_REM_ADMIN_DOWN)))
...
void sbfd_init_recvtimer_cb(struct event *t)
{
	...
	case PTM_BFD_INIT:
	case PTM_BFD_UP:
		ptm_sbfd_init_sess_dn(bs, BD_SBFD_DETECT_FAILED);
		break;
...
void sbfd_echo_recvtimer_cb(struct event *t)
{
	...
	case PTM_BFD_INIT:
	case PTM_BFD_UP:
            ptm_sbfd_echo_sess_dn(bs, BD_SBFD_DETECT_FAILED);
		break;
	case PTM_BFD_DOWN:
		break;
	}
...
static void _sbfd_reflector_free(struct hash_bucket *hb,
		      void *arg __attribute__((__unused__)))
{
