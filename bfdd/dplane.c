 ...
 */
static enum bfd_dplane_key_class
bfd_dplane_classify_key(const struct bfd_session *bs, const struct key *key,
			time_t now, enum bfd_auth_type *type, size_t *keylen,
			bool warn)
{
	if (key->string == NULL)
	...
	if (*keylen == 0 || *keylen > BFDDP_AUTH_KEY_MAX) {
		if (warn)
			zlog_warn("%s: %s: key id %u is %zu bytes, outside 1..%u, not offloaded",
				  __func__, bs->kc->name, key->index, *keylen,
				  BFDDP_AUTH_KEY_MAX);
		return BFD_DPLANE_KEY_UNUSABLE;
	}
...
static int bfd_dplane_send_session_auth(const struct bfd_session *bs)
{
	...
	for (pass = 0; pass < 2; pass++) {
		enum bfd_dplane_key_class want =
			pass == 0 ? BFD_DPLANE_KEY_LIVE : BFD_DPLANE_KEY_FUTURE;

		for (ALL_LIST_ELEMENTS_RO(bs->kc->key, node, key)) {
			enum bfd_auth_type type;
			size_t keylen;

			if (bfd_dplane_classify_key(bs, key, now, &type,
						    &keylen, pass == 0) != want)
				continue;
			...
			keys[count].accept.start =
				htobe64((uint64_t)key->accept.start);
