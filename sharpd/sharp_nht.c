...
static void sharp_nhgroup_child_add_nexthop_cb(const struct nexthop_group_cmd *nhgc)
{
	struct listnode *node;
	struct sharp_nhg lookup;
	char *child_group;
	...
	for (ALL_LIST_ELEMENTS_RO(nhgc->nhg_child_group_list, node, child_group)) {
		...
		if (!snhg_tmp) {
			zlog_debug("%s() : nhg %s, child group %s not found", __func__, nhgc->name,
				   child_group);
			continue;
		}
		if (!snhg_tmp->id) {
			zlog_debug("%s() : nhg %s, child group %s has no valid id %p", __func__,
				   nhgc->name, child_group, snhg_tmp);
			continue;
		}
		if (!sharp_nhgroup_id_is_installed(snhg_tmp->id)) {
			zlog_debug("%s() : nhg %s, child group %s not installed (%u)", __func__,
				   nhgc->name, child_group, snhg_tmp->id);
			continue;
		}
...
static void sharp_nhgroup_child_del_nexthop_cb(const struct nexthop_group_cmd *nhgc)
{
	struct listnode *node;
	struct sharp_nhg lookup;
	char *child_group;
	...
	if (snhg)
		id = snhg->id;
	if (nh_num) {
		zlog_debug("%s() : nhg %s, id %u needs update, now has %u groups", __func__,
			   nhgc->name, id, nh_num);
		nhg_add(snhg->id, nhgc, NULL);
	} else if (sharp_nhgroup_id_is_installed(snhg->id)) {
		zlog_debug("%s() : nhg %s, id %u needs delete, no valid nh_num", __func__,
			   nhgc->name, id);
		nhg_del(snhg->id);
	}
}
					 ...
					 const struct nexthop *nhop)
{
	...
	if (!snhg->id) {
		zlog_debug("%s() : nexthop %s has no valid id %p", __func__, snhg->name, snhg);
		return;
	}
	if (!listcount(nhgc->nhg_child_group_list)) {
		if (nhgc->backup_list_name[0])
	...
	if (nhop) {
		nexthop_group_child_group_match(nhgc->name, sharp_nhgroup_child_add_nexthop_cb);
		return;
	}
					 ...
					 const struct nexthop *nhop)
{
	...
	/* lookup dependent nexthops */
	if (nhop) {
		nexthop_group_child_group_match(nhgc->name, sharp_nhgroup_child_del_nexthop_cb);
		return;
	}
...
void sharp_nhgroup_child_trigger_add_nexthop(uint32_t id)
{
	...
	if (!snhg)
		return;
	/* lookup dependent nexthops */
	nexthop_group_child_group_match(snhg->name, sharp_nhgroup_child_add_nexthop_cb);
}
