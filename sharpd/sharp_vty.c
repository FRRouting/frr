...
DEFPY (install_routes,
	  ...
	  (1-1000000)$routes [instance (0-255)$instance] [repeat (2-1000)$rpt] [opaque WORD] [no-recurse$norecurse] [use-nexthop-group$usenhg]",
       "Sharp routing Protocol\n"
       "install some routes\n"
       "Routes to install\n"
       ...
       "Use nexthop-groups when not installed\n")
{
	struct vrf *vrf;
	...
	if (nexthop_group) {
		...
		/* Only send via ID if nhgroup has been successfully installed or use-nexthop-group is used */
		if (nhgid && (sharp_nhgroup_id_is_installed(nhgid) || usenhg))
			sg.r.nhgid = nhgid;
		else {
			sg.r.nhgid = 0;
			nhgid = 0;
