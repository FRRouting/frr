...
/*
 * Parse a comma-separated list of nexthop IPs (IPv4 or IPv6) into a list
 * Returns the number of nexthops parsed, or -1 on error.
 */
static int sharp_parse_nexthops(const char *nhops_str, vrf_id_t vrf_id,
				struct nexthop_group *nhg, struct vty *vty)
{
	char buf[4096];
	char *saveptr = NULL;
	char *token;
...
DEFPY (install_routes,
	  ...
	  (1-1000000)$routes [instance (0-255)$instance] [table (0-4294967295)$table_id] [repeat (2-1000)$rpt] [opaque WORD] [no-recurse$norecurse]",
       ...
       "Multiple nexthops for ECMP\n"
       "Comma-separated nexthop IPs (e.g. 10.1.1.2,10.1.2.2,10.1.3.2)\n"
       ...
       "No recursive nexthops\n")
{
	...
	if (nexthop_group) {
		...
		nh_cnt = sharp_parse_nexthops(nexthop_str, vrf->vrf_id,
					      &sg.r.nhop_group, vty);
		if (nh_cnt < 0)
			return CMD_WARNING;
	} else {
