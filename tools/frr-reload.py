...
class Vtysh(object):
    ...
    def exec_file(self, filename):
        ...
        if child.wait() != 0:
            # vtysh reports the offending line on stderr ("line N: % Unknown
            # command: ..."). Without it the caller only sees an exit status
            # and cannot tell a rejected command from a daemon refusal.
            raise VtyshException(
                "vtysh (exec file) exited with status %d:\n%s"
                % (child.returncode, stderr.decode("UTF-8", "replace").strip())
            )
