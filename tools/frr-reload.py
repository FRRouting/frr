...
class Config(object):
    ...
    def _load_from_file(self, file_iter):
    ...
    def load_from_file(self, filename, marked=False):
        """
        Read configuration from specified file and slurp it into internal memory.
        If marked is True the file is assumed to already have been marked, otherwise
        we ensure the internal representation has been marked appropriately by passing it
        ...
        if marked:
            ...
            self._load_from_file(file_output.split("\n"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--marked",
        help="Assume provided input files are already appropriately marked",
        action="store_true",
        default=False,
    )
    ...
    try:
        newconf.load_from_file(args.filename, args.marked)
        reload_ok = True
    except OSError as err:
        log.error("Failed to load new config file {}: {}".format(args.filename, err))
        sys.exit(1)
    ...
    if args.test:
        ...
        if args.input:
            try:
                running.load_from_file(args.input, args.marked)
            except OSError as err:
                log.error(
                    "Failed to load running config file {}: {}".format(args.input, err)
                )
                sys.exit(1)
        else:
