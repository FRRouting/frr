/**EIGRPSendingandReceivingEIGRPSIA-ReplyPackets.*Copyright(C)2013-2014*Authors:
...
A-REPLYreadfunction*/voideigrp_siareply_receive(structeigrp*eigrp,structip*iph,s
...
istics.*/ei->siaReply_in++;/*getneighborstruct*/nbr=eigrp_nbr_get(ei,eigrph,iph)
...
_send_ack(nbr);}voideigrp_send_siareply(structeigrp_neighbor*nbr,structeigrp_pre
...
er_init(EIGRP_OPC_SIAREPLY,nbr->ei->eigrp,ep->s,0,nbr->ei->eigrp->sequence_numbe
r,0);//encodeAuthenticationTLV,ifneededif(nbr->ei->params.auth_type==EIGRP_AUTH_
TYPE_MD5&&nbr->ei->params.auth_keychain!=NULL){length+=eigrp_add_authTLV_MD5_to_
stream(ep->s,nbr->ei);}length+=eigrp_add_internalTLV_to_stream(ep->s,pe);if((nbr
->ei->params.auth_type==EIGRP_AUTH_TYPE_MD5)&&(nbr->ei->params.auth_keychain!=NU
LL)){eigrp_make_md5_digest(nbr->ei,ep->s,EIGRP_AUTH_UPDATE_FLAG);}/*EIGRPChecksu
m*/eigrp_packet_checksum(nbr->ei,ep->s,length);ep->length=length;ep->dst.s_addr=
nbr->src.s_addr;/*Thisacknumberweawaitfromneighbor*/ep->sequence_number=nbr->ei-
>eigrp->sequence_number;if(nbr->state==EIGRP_NEIGHBOR_UP){/*Putpackettoretransmi
ssionqueue*/eigrp_fifo_push(nbr->retrans_queue,ep);if(nbr->retrans_queue->count=
=1){eigrp_send_packet_reliably(nbr);}}elseeigrp_packet_free(ep);}