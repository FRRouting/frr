...
#define VRRP_XPATH_ENTRY VRRP_XPATH "[virtual-router-id='%lld']"
      ...
      VRRP_VERSION_STR)
{
	...
	snprintf(valbuf, sizeof(valbuf), "%lld", version ? version : vd.version);
      ...
      "Advertisement interval in milliseconds; must be multiple of 10\n")
{
	...
	snprintf(val, sizeof(val), "%lld", advertisement_interval);
